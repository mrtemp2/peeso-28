...
class Model_coaching extends CI_Model
{
    ...
    public function createCoaching($coaching){
        $created =  $this->db->insert('coaching',$coaching);
        if(!$created){
            return false;
        }
        ...
        if(count($students)){
            ...
            $created =  $this->db->insert_batch('coaching_students',$student_coaching);
            if(!$created){
                return false;
        ...
        if(count($seances)){
            ...
            return $this->db->where('id',$insert_id)->update('coaching',[
                'nb_seance'=>$nb_seance
            ]);
        }
        else{
    ...
    public function getCoachingById($id){
        ...
        return $this->db->select('*')->from('coaching')->where('id',$id)->get()->row_array();
    ...
    public function getDetailCoaching($coachingId){
            $coaching =$this->getCoachingById($coachingId);
            $domaines = $this->db->select('d.*')->from('coaching_domaines cd')->join('domaine d','cd.domaine_id=d.id')->where('cd.coaching_id',$coachingId)->get()->result_array();

            if(!$coaching){
                return null;
            }
            $referent = null;
            if($coaching['referent_id']){
                $referent = $this->getRefrentById($coaching['referent_id']);
            }
            return [
                'referent'=>$referent,
                'coaching'=>$coaching,
    ...
    public function prepareCreateCoachingForm(){
    ...
    public function getMembers($coachingId){
        return [
            ...
            ->join('etudiants e','cs.student_id=e.id')
             ->join('users u','e.id=u.etudiant_id')
             ->where('cs.coaching_id',$coachingId)               
             ->get()
             ->result_array()
        ];
    ...
    public function getAllCoaching(){
        return $this->db->select('c.*, u.photo as photo_ref, concat(e.prenom, " " ,e.nom ) as nom_ref, group_concat(d.nom separator " | ") as nom_dom')
                        ->from('coaching c')
                        ->join('enseignant e', 'referent_id = e.id','left')
                        ->join('users u','e.id = u.enseignant_id','left')
                        ->join('coaching_domaines cd','cd.coaching_id = c.id', 'left')
                        ->join('domaine d', 'cd.domaine_id = d.id', 'left')
                        ->group_by('c.id')
                        ->get()
                        ->result_array();
    }
