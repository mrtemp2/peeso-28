...
<div id="createSeanceModal" class="modal fade">
      ...
      <div class="modal-content bd-0 tx-14 loginarea__wraper" style="border:none !important;">
        ...
        <form method="post" id="createCoachingForm" action="<?=base_url()?>seance/createSeanceCoaching" enctype="multipart/form-data" accept-charset="utf-8">
                    <div class="row">               
                                        <div class="col-xl-12">
    ...
    var callbacks = {
        getDetailsMembers:(targetId)=>{
            return new Promise((resolve,reject)=>{
                $(targetId).load('<?=base_url('coaching/getMembers/'.$coaching['id'])?>',(complete)=>{
                    resolve()
                })
            })
        ...
        getDetailsActivity:(targetId)=>{
            ...
            return new Promise((resolve,reject)=>{
                $(targetId).load('<?=base_url('activity/getDetailsActivityCoaching')?>/'+seanceId,(complete)=>{
                    resolve()
                })
            })
        ...
        getSingleActivity:(targetId)=>{
            ...
            return new Promise((resolve,reject)=>{
                $(targetId).load('<?=base_url('activity/getSingleActivityReferentCoaching')?>/'+activityId,(complete)=>{
                    resolve()
                })
            })
        ...
        getDetailsModule:(targetId)=>{
            const seanceId = $(targetId).data('seance')
            return new Promise((resolve,reject)=>{
                $(targetId).load('<?=base_url('module/getDetailsModuleCoaching')?>/'+seanceId,(complete)=>{
                    resolve()
                })
            })
        ...
        getDetailsSeance:(targetId)=>{
            return new Promise((resolve,reject)=>{
                $(targetId).load('<?=base_url('seance/getDetailsSeanceCoaching/'.$coaching['id'])?>',(complete)=>{
                    resolve()
                })
            })
        },
        getSingleSeance:(targetId)=>{
            return new Promise((resolve,reject)=>{
                const seanceId = $(targetId).data('seance')
                $(targetId).load(`<?=base_url('seance/getSingleSeanceCoaching')?>/${seanceId}`,(complete)=>{
                    resolve()
                })
            })
        ...
        function reloadSeance(seanceId){
            ...
            $(targetId).load(`<?=base_url('seance/getSingleSeanceCoaching')?>/${seanceId}/<?=$_GET['progression_id']?>`,(complete)=>{
                    overlayOff()    
        ...
        function startSeance(seanceId){
                overlayOn()
                fetch(`<?=base_url('seance/startSeanceCoaching')?>/${seanceId}/<?=$_GET['progression_id']?>`).then(data=>{
                    return data.json()
                }).then(data=>{
                    if(data.success){
                        Swal.fire({
        ...
        function planifierSeance(groupe_seance_id){
            overlayOn()
            $('#createRandezVousContent').load('<?=base_url()?>randezvous/prepareRandezVousFormCoaching/'+groupe_seance_id,(complete)=>{
        ...
        function reloadMembers(){
            $('#member-content').load('<?=base_url('coaching/getMembers/'.$coaching['id'])?>',(complete)=>{
                    overlayOff()
            })
        ...
        function eliminateStudent(etudiantId){
            overlayOn()
            fetch(`<?=base_url()?>coaching/eliminateMember/<?=$coaching['id']?>/${etudiantId}`).then(data=>{
                return data.json()
            }).then(data=>{ 
                if(data.success){
                                Swal.fire({
        ...
        function reloadSeances(){
            ...
            $('#seance-content').load('<?=base_url('seance/getDetailsSeanceCoaching/'.$coaching['id'])?>',(complete)=>{
                    overlayOff()
           })
        ...
        function reloadModules(seanceId){
            ...
            $(`#module-content-${seanceId}`).load(`<?=base_url('module/getDetailsModuleCoaching')?>/${seanceId}`,(complete)=>{
                    overlayOff()
           })
        ...
        function reloadActivities(seanceId){
            ...
            $(`#activity-content-${seanceId}`).load('<?=base_url('activity/getDetailsActivityCoaching')?>/'+seanceId,(complete)=>{
                    overlayOff()
            })
        ...
        function ajouterSeance(){
               ...
               $('#createCoachingForm').unbind('submit').on('submit',function(e){
                    e.preventDefault()
                    overlayOn()
                    const action = $('#createCoachingForm').attr('action')
                    const formData = new FormData(this)
                       formData.append('coaching_id','<?=$coaching['id']?>')
                    fetch(action,{
                        body:formData,
                        method:'POST'
                    }).then(data=>{
