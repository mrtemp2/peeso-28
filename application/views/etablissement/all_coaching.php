...
<div class="col-xl-9 col-lg-9 col-md-12">
                            <div class="dashboard__content__wraper">
                                <div class="dashboard__section__title" style="display: flex;align-items:center;justify-content:space-between;">
                                    <h4>Liste Des Coaching</h4>
                                    <button class="default__button" onclick="ajouterCoaching()" href="#">Ajouter Coaching</button>
                                </div>
    ...
    <div id="updateCoaching" class="modal fade">
    ...
    <div id="createCoaching" class="modal fade">
      <div class="modal-dialog modal-lg" id="createCoachingContent" role="document">
      </div>
    </div>
    ...
    $('#createCoachingContent').load('<?=base_url()?>coaching/createCoachingContentDept',(complete)=>{
        ...
        setTimeout(()=>{
            ...
            $('.js-example-invited-multiple').select2({
              dropdownParent: $('#createCoaching'), // Ensure the dropdown is rendered within the modal
...
function updateCoaching(id){
    let updateReferentForm
    overlayOn()
    $('#updateModalContent').load('<?=base_url()?>coaching/getupdateCoachingForm/'+id,(complete)=>{
       
        setTimeout(()=>{
            $('.js-example-domaine-multiple').select2({
                dropdownParent: $('#updateCoaching'), // Ensure the dropdown is rendered within the modal
                placeholder: "Sélectionnez des domaines",
                width: '100%'
             });
            $('#updateCoaching').modal('show')
            overlayOff()

            $('#updateCoachingForm').unbind('submit').on('submit',function(e){
                e.preventDefault()
                overlayOn()
                const action  = $('#updateCoachingForm').attr('action')
    ...
    overlayOn()
    $('#inviteModalContent').load('<?=base_url()?>coaching/getUpdateInvitationForm/'+id,(complete)=>{
  ...
  overlayOn()
  $('#inviteGroupsContent').load('<?=base_url()?>coaching/getGroupInvitationForm/'+id,(complete)=>{
