...
class Formation extends CI_Controller
{
    ...
    public function updateFormation($id){
        ...
        if ($this->form_validation->run()) {
            ...
            $created = $this->model_formation->updateFormation($id,$updated_data);
            if($created){
                echo json_encode([
                    'success'=>true,
                    'message'=>'La Formation Est Crée avec succes'
    ...
    {
        $moduleData = $this->model_formation->fetchFormationData();
    ...
    private function sendEmailInvitation($formationId){
        ...
        $mails = $this->model_formation->getStudentsEmails($studentsIds);
        $formation = $this->model_formation->getFormationById($formationId);
        $this->load->library('email');
    ...
    public function getUpdateInvitationForm($id){
        $formation = $this->model_formation->getFormationById($id);
        $students = $this->model_formation->getIninvitedStudents($id);
        if(!$formation){
            show_404();
            return;
        }
    ...
    public function getFormationsStudent(){
        ...
        $formations = $this->model_formation->getFormationStudent($_POST,$this->getIdEtudiant());
    ...
    public function createDemandeFormation($formationId){
            $created = $this->model_formation->createDemandeFormation([
    ...
    public function getDemandesForReferents(){
        ...
        $moduleData = $this->model_formation->getDemandeForReferent($refrentId);
        $result = array('data' => array());
        foreach ($moduleData as $key => $value) {
            if($value['accepted']){
    ...
    public function getStudentDemandes(){
        ...
        $moduleData = $this->model_formation->getDemandeByStudent($studentId);
        $result = array('data' => array());
        foreach ($moduleData as $key => $value) {
            if($value['accepted']){
                $detailsButton = '<span class="dashboard__td dashboard__td--running">Accepté</span> ';	
    ...
    public function acceptDemand($demandeId){
            $accepted =     $this->model_formation->acceptDemande($demandeId); 
            $demande = $this->model_formation->getDemandeFormationById($demandeId);
    ...
    private function sendDemandAcceptationMail($formationId,$studentId){
        $etudiant = $this->model_formation->getStudentById($studentId);
        $formation = $this->model_formation->getFormationById($formationId);
        $this->load->library('email');
        // Send email invitation
        $this->email->set_mailtype('html');
    ...
    public function affecterFormateur(){
        ...
        $affected = $this->model_formation->affecterFormaionToRefrent($referentId,$formationId);
        if($affected){
            echo json_encode([
                'success'=>true,
    ...
    public function prepareAffectationForm($formationId){
        $formation = $this->model_formation->getFormationById($formationId);
        if($formation){
            $referents = $this->model_formation->getAllReferents();
    ...
    public function createFormationEnseignant(){
        $validate_data = array(
            array(
                'field' => 'nom',
                'label' => 'Nom',
    ...
    public function getFormationForEnseignant(){
            ...
            $formations = $this->model_formation->getFormationForEnseignant($connectedReferent);
            $result = array('data' => array());
    ...
    public function publishFormation($formationId){
            $publie = $this->model_formation->publishFormation($formationId);
            if($publie){
                echo json_encode([
                    'success'=>true,
    ...
    public function getGroupProgression($formationId): void{
        $progression = $this->model_formation->getGroupProgression($formationId);
    ...
    public function createFormationReferentContent(){
        $info = $this->model_formation->prepareCreateFormationForm();
        $this->load->view('enseignant/formation/createFormation',[
            'info'=>$info
        ]);
    }
    public function getMembers($formationId){
        $info = $this->model_formation->getMembers($formationId);
        $this->load->view('admin/gestion_formation/members',[
            'info'=>$info
        ]);
    ...
    public function eliminateMember($formationId,$etudiant_id){
        $deleted = $this->model_formation->eliminateMember($formationId,$etudiant_id);
        if($deleted){
            echo json_encode([
                'success'=>true,
