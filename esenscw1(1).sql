-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1:3306
-- Generation Time: Nov 28, 2024 at 02:35 PM
-- Server version: 8.3.0
-- PHP Version: 8.3.6

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `esenscw1`
--

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_2024_appel_a_candidature`
--

DROP TABLE IF EXISTS `28_2024_2024_appel_a_candidature`;
CREATE TABLE IF NOT EXISTS `28_2024_2024_appel_a_candidature` (
  `id` int NOT NULL,
  `id1` int NOT NULL,
  `id2` int NOT NULL,
  `id3` int NOT NULL,
  `etab_id` int DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_activities`
--

DROP TABLE IF EXISTS `28_2024_activities`;
CREATE TABLE IF NOT EXISTS `28_2024_activities` (
  `id` int NOT NULL AUTO_INCREMENT,
  `seance_id` int NOT NULL,
  `document` varchar(255) DEFAULT NULL,
  `titre` varchar(255) NOT NULL,
  `instructions` text NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_activities`
--

INSERT INTO `28_2024_activities` (`id`, `seance_id`, `document`, `titre`, `instructions`) VALUES
(3, 8, 'activity.pdf', 'Initiation CI/CD', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged'),
(4, 10, 'docker.pdf', 'Pratiquer Docker', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book'),
(5, 9, 'maintenance.pdf', 'Hands On', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged\r\n                                   \r\n\r\n                                '),
(6, 15, 'intro.pdf', 'Activité Peeso', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged.                                   \r\n\r\n                                ');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_appel_a_candidature`
--

DROP TABLE IF EXISTS `28_2024_appel_a_candidature`;
CREATE TABLE IF NOT EXISTS `28_2024_appel_a_candidature` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `sujet` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date NOT NULL,
  `domaine` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `photo` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_appel_a_candidature`
--

INSERT INTO `28_2024_appel_a_candidature` (`id`, `nom`, `sujet`, `date_debut`, `date_fin`, `domaine`, `description`, `photo`) VALUES
(1, 'Test', 'Sujett', '2024-10-02', '2024-11-21', '', 'test d\'un appel à candidature', 'https://esen-sci-compet.com/eesu/assets/assets/images/appel_a_candidature_dark7.jpg'),
(2, 'test2', 'test2', '2024-10-03', '2024-10-03', '', 'test n°2', 'https://esen-sci-compet.com/eesu/assets/assets/images/projectcycle-1024x6141.jpg'),
(3, 'appel à candidature', 'test', '2024-10-04', '2024-10-10', '', 'description test test', 'https://esen-sci-compet.com/eesu/assets/assets/images/projet1.jpg'),
(4, 'Energie renouvelable', 'Electricité', '2024-10-08', '2024-10-31', '', 'test de la photo ', 'https://esen-sci-compet.com/eesu/assets/assets/images/projet21.jpeg'),
(5, 'Intelligence artificielle', 'Application smart pour faciliter les tâches', '2024-10-14', '2024-10-21', '', 'Simplifier la tâche des étudiants entrepreneurs pour choisir un nom professionnel et adapté à leurs projets, pour corriger leurs rapports...', 'https://esen-sci-compet.com/eesu/assets/assets/images/projectcycle-1024x6143.jpg'),
(6, 'testttt', 'testtttttttt', '2024-10-28', '2024-11-10', '', 'tesstttttttttt testtttttt testttttttttttttttt', 'https://esen-sci-compet.com/eesu/assets/assets/images/projectcycle-1024x6145.jpg'),
(7, 'appel a candidature n2', 'appeeeeeel ', '2024-11-04', '2024-11-29', '', 'apeeeeeeellllllddd', 'https://esen-sci-compet.com/eesu/assets/assets/images/10d6ab35-8ccf-46c7-916c-8cc3943c143a.jpeg');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_appel_domaine`
--

DROP TABLE IF EXISTS `28_2024_appel_domaine`;
CREATE TABLE IF NOT EXISTS `28_2024_appel_domaine` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_appel` int NOT NULL,
  `id_domaine` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=51 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_appel_domaine`
--

INSERT INTO `28_2024_appel_domaine` (`id`, `id_appel`, `id_domaine`) VALUES
(22, 4, 1),
(26, 3, 8),
(31, 2, 1),
(32, 2, 3),
(40, 5, 6),
(41, 5, 7),
(42, 6, 3),
(43, 6, 13),
(44, 1, 1),
(45, 1, 2),
(48, 7, 2),
(50, 8, 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_attribution_deroulement_initiateur`
--

DROP TABLE IF EXISTS `28_2024_attribution_deroulement_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_attribution_deroulement_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `id_attribution_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=62 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_attribution_deroulement_initiateur`
--

INSERT INTO `28_2024_attribution_deroulement_initiateur` (`id`, `nom`, `id_attribution_seance`) VALUES
(1, 'Evaluation préalable des compétences\r\n', 1),
(2, 'Evaluation de la motivation de l’étudiant\r\n', 1),
(3, 'Identification du niveau d’avancement de l’idée\r\nVérification de l’adéquation entre le profil de l’étudiant et son idée de projet\r\n', 1),
(4, 'Vérification du degré de maitrise du contexte ', 1),
(5, 'économique et sociale de l’idée du\r\nprojet\r\n', 1),
(6, 'Présentation du planning d’accompagnement\r\n', 1),
(7, 'Explication de la démarche d’accompagnement', 1),
(8, ' Vérification et validation des requêtes de la séance 01', 2),
(9, 'Examen de l’étude documentaire du secteur d’activité\r\n', 2),
(10, 'Analyse de l’environnement PEST/ SWOT (facultatif)\r\n', 2),
(11, 'Identification du problème\r\n', 2),
(12, 'Présentation de la solution\r\n', 2),
(13, 'Description des segments clients (persona)\r\n', 2),
(14, 'Valider la proposition de valeur', 2),
(15, 'Vérification et validation des requêtes de la séance 02\r\n', 3),
(16, 'Elaborer las canaux de communication et de distribution\r\n', 3),
(17, 'Déterminer les relations clients\r\n', 3),
(18, 'Identifier les sources de revenus\r\n', 3),
(19, 'Préciser les différentes ressources clés\r\n', 3),
(20, 'Présenter les activités clés\r\n', 3),
(21, 'Identifier les partenaires stratégiques\r\n', 3),
(22, 'Déterminer la structure des coûts ', 3),
(23, 'Concevoir le Business Model Canvas/Business Lean Canvas (les différents blocs)', 3),
(24, 'Vérification et validation des requêtes de la séance 03\r\n', 4),
(25, 'Justifier le choix des clients cibles\r\n', 4),
(26, 'Enumérer les concurrents directs et indirects\r\n', 4),
(27, 'Choisir une technique d’étude de marché : qualitative/quantitative, focus groupe,\r\nenquête par questionnaire…', 4),
(28, 'Concevoir le questionnaire et/ou le guide d’entretien', 4),
(29, 'Vérification et validation des requêtes de la séance 04\r\n', 5),
(30, 'Justifier les hypothèses de base pour le calcul du chiffre d’affaires\r\n', 5),
(31, 'Compiler les données collectées (recherches documentaires &amp; enquête) ;\r\n', 5),
(32, 'Vérifier la cohérence de la stratégie marketing', 5),
(33, 'Valider l’étude de marché', 5),
(34, 'Vérification et validation des requêtes de la séance 05\r\n', 6),
(35, 'Examen du processus de fabrication ou de la chaine de valeur\r\n', 6),
(36, 'Identification des Besoins : Equipements, licences, brevets, ressources humaines\r\n', 6),
(37, 'Valider l’étude technique', 6),
(38, 'Vérification et validation des requêtes de la séance 06\r\n', 7),
(39, 'Examen de la liste des investissements\r\n', 7),
(40, 'Calcul du coût de l’investissement\r\n', 7),
(41, 'Choix des sources de financement adaptées aux besoins du projet\r\n', 7),
(42, 'Elaborer le schéma d’investissement et de financement\r\n', 7),
(43, 'Examiner les produits et des charges prévisionnelles', 7),
(44, 'Vérification et validation des requêtes de la séance 07\r\n', 8),
(45, 'Valider le tableau des produits et des charges prévisionnelles\r\n', 8),
(46, 'Elaborer le tableau de trésorerie prévisionnelle\r\n', 8),
(47, 'Calculer la rentabilité du projet en utilisant la VAN, le TRI et le DRCI\r\n', 8),
(48, 'Valider l’étude financière', 8),
(49, 'Vérification et validation des requêtes de la séance 08\r\n', 9),
(50, 'Justifier le choix de la forme juridique : Personne physique, SA, SUARL, SARL,\r\n', 9),
(51, 'Corriger les statuts de l’entreprise\r\n', 9),
(52, 'Valider les statuts de l’entreprise\r\n', 9),
(53, 'Préparer le dossier fiscal de l’entreprise', 9),
(54, 'Vérification et validation des requêtes de la séance 09\r\n', 10),
(55, 'Revoir toutes les parties du plan d’affaires\r\n', 10),
(56, 'Vérifier la cohérence du plan d’affaires\r\n', 10),
(57, 'Valider le plan d’affaires', 10),
(58, 'Vérification et validation des requêtes de la séance 10\r\n', 11),
(59, 'Analyse du pitch : verbale et non verbale\r\n', 11),
(60, 'Améliorer le PPT\r\n', 11),
(61, 'Présentation du projet en 3 minutes.', 11);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_attribution_deroulement_innovateur`
--

DROP TABLE IF EXISTS `28_2024_attribution_deroulement_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_attribution_deroulement_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `id_attribution_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=73 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_attribution_deroulement_innovateur`
--

INSERT INTO `28_2024_attribution_deroulement_innovateur` (`id`, `nom`, `id_attribution_seance`) VALUES
(1, 'Evaluation préalable des compétences\r\n', 1),
(2, 'Evaluation de la motivation de l’étudiant\r\n', 1),
(3, 'Identification du niveau d’avancement de l’idée\r\n', 1),
(4, 'Vérification de l’adéquation entre le profil de l’étudiant et son idée de projet\r\n', 1),
(5, 'Vérification du degré de maitrise du contexte économique et sociale de l’idée du projet\r\n', 1),
(6, 'Présentation du planning d’accompagnement', 1),
(7, 'Explication de la démarche d’accompagnement', 1),
(8, 'Vérification et validation des requêtes de la séance 01\r\n', 2),
(9, 'Examen de la démarche de la planification stratégique\r\n', 2),
(10, 'Diagnostic stratégique\r\n', 2),
(11, 'Identification des objectifs stratégiques et résultats clés à atteindre', 2),
(12, 'Elaboration du plan d’action et tableau de bord', 2),
(13, 'Vérification et validation des requêtes de la séance 02\r\n', 3),
(14, 'Identifier les compétences d’un bon leader\r\n', 3),
(15, 'Déterminer les relations de collaboration et de synergie des membres d’équipe', 3),
(16, 'Identifier et gérer les conflits de groupe', 3),
(17, 'Vérification et validation des requêtes de la séance 03 ', 4),
(18, 'Identification des types de risques\r\n', 4),
(19, 'Analyse des risques externes et internes\r\n', 4),
(20, 'Analyse et diagnostic des aspects des risques et problèmes', 4),
(21, 'Vérification et validation des requêtes de la séance 04\r\n', 5),
(22, 'Identifier les acteurs de l’écosystème entrepreneurial relatif au projet\r\n', 5),
(23, 'Analyser les opportunités et menaces de l’environnement externe et forces/faiblesses de\r\nl’environnement interne', 5),
(24, 'Vérification et validation des requêtes de la séance 05\r\n', 6),
(25, 'Analyse des besoins en communication des différents acteurs\r\n', 6),
(26, 'Constitution d’une base de données des différents besoins', 6),
(27, 'Formulation des objectifs de communication', 6),
(28, 'Vérification et validation des requêtes de la séance 06\r\n', 7),
(29, 'Présenter les différents types et formes de communication\r\n', 7),
(30, 'Présenter les différents canaux de communication', 7),
(31, 'Les étapes de l’elevator pitch', 7),
(32, 'Vérification et validation des requêtes de la séance 07\r\n', 8),
(33, 'Préciser les éléments du plan de communication \r\n', 8),
(34, 'Mise en œuvre des activités de communication', 8),
(35, 'Vérification et validation des requêtes de la séance 08\r\n', 9),
(36, 'Rédaction d’un questionnaire de satisfaction\r\n', 9),
(37, 'Collecter les informations\r\n', 9),
(38, 'Proposer des améliorations (au cas où il Ya une insatisfaction)', 9),
(39, 'Valider le plan de communication', 9),
(40, 'Vérification et validation des requêtes de la séance 09\r\n', 10),
(41, 'Présenter les techniques de prototypages\r\n', 10),
(42, 'Présenter des logiciels de prototypage', 10),
(43, 'Vérification et validation des requêtes de la séance 10\r\n', 11),
(44, 'Présenter les étapes du processus du test\r\n', 11),
(45, 'Présenter les techniques du test du prototype', 11),
(46, 'Vérification et validation des requêtes de la séance 11\r\n', 12),
(47, 'Déterminer l’étude qualitative de la clientèle cible\r\n', 12),
(48, 'Déterminer l’étude quantitative de la clientèle cible', 12),
(49, 'Vérification et validation des requêtes de la séance 12\r\n', 13),
(50, 'Identification des concurrents directs et indirects\r\n', 13),
(51, 'Choix des critères de comparaison entre ces concurrents\r\n', 13),
(52, 'Analyse comparative de la situation concurrentielle', 13),
(53, 'Vérification et validation des requêtes de la séance 13\r\n', 14),
(54, 'Présenter les stratégies commerciales', 14),
(55, 'Vérification et validation des requêtes de la séance 14\r\n', 15),
(56, 'Présenter le marketing Mix\r\n', 15),
(57, 'Définir les actions commerciales relatives au produit, au prix, à la promotion et à la place', 15),
(58, 'Vérification et validation des requêtes de la séance 15\r\n', 16),
(59, 'Présentation des différents types d’entreprises et statuts juridiques\r\n', 16),
(60, 'Présentation des instances nécessaires à la constitution de l’entreprise dans le contexte\r\nlocal/régional\r\n', 16),
(61, 'Présentation des opportunités et des avantages (fiscaux, accompagnements, subventions, etc.)', 16),
(62, 'Vérification et validation des requêtes de la séance 16\r\n', 17),
(63, 'Présentation des différents types d’entreprises et statuts juridiques\r\n', 17),
(64, 'Présentation des instances nécessaires à la constitution de l’entreprise dans le contexte\r\nlocal/régional\r\n', 17),
(65, 'Présentation des opportunités et des avantages (fiscaux, accompagnements, subventions, etc.)', 17),
(66, 'Vérification et validation des requêtes de la séance 17', 18),
(67, 'Présentation du schéma d’investissement et de financement\r\n', 18),
(68, 'Explication de la démarche de calcul du Chiffre d’affaires\r\n', 18),
(69, 'Présentation de l’estimation des charges d’exploitation et charges financières', 18),
(70, 'Présenter le compte d’exploitation prévisionnel\r\n', 19),
(71, 'Expliquer la démarche d’estimer la trésorerie prévisionnelle\r\n', 19),
(72, 'Exposer les différents Indicateurs de rentabilité ( Le Taux de Rentabilité Interne (TRI), la\r\nValeur Actuelle Nette (VAN), le Seuil de Rentabilité des premières années)', 19);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_attribution_deroulement_promoteur`
--

DROP TABLE IF EXISTS `28_2024_attribution_deroulement_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_attribution_deroulement_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `id_attribution_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_attribution_deroulement_promoteur`
--

INSERT INTO `28_2024_attribution_deroulement_promoteur` (`id`, `nom`, `id_attribution_seance`) VALUES
(1, 'Identifier les différentes parties prenantes à impliquer dans son projet,\r\n', 1),
(2, 'Mesurer le niveau d’intérêt et de pouvoir des parties prenantes sur le projet et discerner les\r\nnouvelles opportunités\r\n', 1),
(3, 'Classifier les parties prenantes', 1),
(4, 'Communiquer avec les parties prenantes afin de qualifier leurs attentes et les moyens d’action\r\nadaptés pour les influencer positivement\r\n', 2),
(5, 'Planifier une stratégie de négociation adaptée pour satisfaire les attentes communes (parties\r\nprenantes/ Entreprise)', 2),
(6, 'Vérification et validation des requêtes de la séance 2\r\n', 3),
(7, 'Définir le contenu des postes et les compétences requises\r\n', 3),
(8, 'Définir et choisir la structure organisationnelle (structure fonctionnelle, structure divisionnelle,\r\nstructure projet)', 3),
(9, 'Présentation des salaires de base, charges sociales et patronales\r\n', 4),
(10, 'définir les salaires et les autres composantes de rémunération\r\n', 4),
(11, 'Définir les étapes de recrutement', 4),
(12, 'Vérification et validation des requêtes de la séance 4\r\n', 5),
(13, 'Définir la charte de communication interne\r\n', 5),
(14, 'Présenter les différents types de motivation et structures incitatives\r\n', 5),
(15, 'Définir les types de conflits', 5),
(16, 'Vérification et validation des requêtes de la séance 5\r\n', 6),
(17, 'Présenter les outils d’identification des risques\r\n', 6),
(18, 'Définir les types de risques\r\n', 6),
(19, 'Évaluer et mesurer la criticité des risques', 6),
(20, 'Vérification et validation des requêtes de la séance 6\r\n', 7),
(21, 'Définir les approches de gestion de risques (évitement, réduction, transfert, acceptation)', 7),
(22, 'Vérification et validation des requêtes de la séance 7\r\n', 8),
(23, 'Présenter les outils de protection de la propriété intellectuelle (brevet, marque, certificat,\r\ndessins et modèles, secret)\r\n', 8),
(24, 'Présenter l’INNORPI (Institut national de la normalisation et de la propriété Intellectuelle) et\r\nses services', 8);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_attribution_seance_initiateur`
--

DROP TABLE IF EXISTS `28_2024_attribution_seance_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_attribution_seance_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `num` int NOT NULL,
  `phase_id` int NOT NULL,
  `seance_parent` int DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_attribution_seance_initiateur`
--

INSERT INTO `28_2024_attribution_seance_initiateur` (`id`, `nom`, `num`, `phase_id`, `seance_parent`) VALUES
(1, 'Prise de Contact (Brise Glace)', 1, 1, NULL),
(2, 'Bilan de Compétences', 2, 1, NULL),
(3, 'Valider l’idée du projet', 3, 2, NULL),
(6, ' Le Business Model (Exploration/Préparation)', 4, 3, NULL),
(7, 'Le Business Model (Validation/Confirmation)  ', 5, 3, NULL),
(8, 'L’étude de marché part(1)', 6, 4, NULL),
(9, 'L’étude de marché part(2)', 7, 4, NULL),
(10, 'L’étude technique', 8, 4, NULL),
(11, 'L’étude financière part(1) ', 9, 4, NULL),
(12, 'L’étude financière part(2) ', 10, 4, NULL),
(13, ' Le plan d’affaires (Validation/Confirmation)', 11, 4, NULL),
(14, 'Pitching', 12, 5, NULL);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_attribution_seance_innovateur`
--

DROP TABLE IF EXISTS `28_2024_attribution_seance_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_attribution_seance_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `num` int NOT NULL,
  `phase_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_attribution_seance_innovateur`
--

INSERT INTO `28_2024_attribution_seance_innovateur` (`id`, `nom`, `num`, `phase_id`) VALUES
(1, 'Prise de Contact', 1, 1),
(2, 'Planifier le projet', 2, 1),
(3, 'Être un bon leader', 3, 1),
(4, 'La gestion des risques', 4, 1),
(5, 'Analyse de l’écosystème et du context du projet', 5, 2),
(6, 'Définition des objectifs de communication', 6, 2),
(7, 'La formulation du message', 7, 2),
(8, 'Le plan de communication', 8, 2),
(9, 'Le suivi et l’évaluation', 9, 2),
(10, 'La conception du prototype', 10, 3),
(11, 'Le test du prototype', 11, 3),
(12, 'L’étude de la clientèle cible', 12, 4),
(13, 'L’analyse de la concurrence', 13, 4),
(14, 'la stratégie commerciale', 14, 4),
(15, 'Le plan d’action Marketing', 15, 4),
(16, 'Conception de l’ingénierie juridique', 16, 4),
(17, 'Conception de l’ingénierie juridique', 17, 4),
(18, 'Schéma d’investissement et de financement', 18, 5),
(19, 'L’analyse de rentabilité', 19, 5);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_attribution_seance_promoteur`
--

DROP TABLE IF EXISTS `28_2024_attribution_seance_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_attribution_seance_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `num` int NOT NULL,
  `phase_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_attribution_seance_promoteur`
--

INSERT INTO `28_2024_attribution_seance_promoteur` (`id`, `nom`, `num`, `phase_id`) VALUES
(1, 'Cartographie des parties prenantes du projet', 1, 1),
(2, 'Nouer des partenariats', 2, 1),
(3, 'Structure organisationnelle', 3, 2),
(4, 'Recrutement et rémunération', 4, 2),
(5, 'Politique de communication interne', 5, 2),
(6, 'Analyse des risques', 6, 3),
(7, 'Solutions des risques', 7, 3),
(8, 'La protection de la propriété intellectuelle et industrielle', 8, 3);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_bilan_competance_initiateur`
--

DROP TABLE IF EXISTS `28_2024_bilan_competance_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_bilan_competance_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `observation` text,
  `evaluation` int DEFAULT NULL,
  `parameter_id` int NOT NULL,
  `project_id` int DEFAULT NULL,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `updated` tinyint(1) NOT NULL DEFAULT '0',
  `valide` tinyint(1) DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=287 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_bilan_competance_initiateur`
--

INSERT INTO `28_2024_bilan_competance_initiateur` (`id`, `name`, `observation`, `evaluation`, `parameter_id`, `project_id`, `etudiant_id`, `enseignant_id`, `updated`, `valide`) VALUES
(105, 'Développer ses aptitudes de communication', NULL, 2, 1, 7, 1, NULL, 1, 0),
(106, 'Maîtriser les techniques de la communication verbale et non verbale', NULL, 2, 2, 7, 1, NULL, 1, 0),
(107, 'Gérer les conflits', NULL, 2, 3, 7, 1, NULL, 1, 0),
(108, 'Maîtriser la gestion du temps', NULL, 2, 4, 7, 1, NULL, 1, 0),
(109, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', NULL, 2, 5, 7, 1, NULL, 1, 0),
(110, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', NULL, 2, 6, 7, 1, NULL, 1, 0),
(111, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', NULL, 2, 7, 7, 1, NULL, 1, 0),
(112, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', NULL, 2, 8, 7, 1, NULL, 1, 0),
(113, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', NULL, 2, 9, 7, 1, NULL, 1, 0),
(114, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', NULL, 2, 10, 7, 1, NULL, 1, 0),
(115, 'Développer l’opportunité entrepreneuriale ', NULL, 2, 11, 7, 1, NULL, 1, 0),
(116, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', NULL, 2, 12, 7, 1, NULL, 1, 0),
(117, 'Développer l’opportunité entrepreneuriale \r\n\r\n', NULL, 2, 13, 7, 1, NULL, 1, 0),
(118, 'Développer ses aptitudes de communication', '                        \r\n                   Je L ai maitrise                               ', 3, 1, 7, NULL, 2, 1, 1),
(119, 'Maîtriser les techniques de la communication verbale et non verbale', '                        \r\n                   Je L ai maitrise                               ', 3, 2, 7, NULL, 2, 1, 1),
(120, 'Gérer les conflits', '                        \r\n                   Je L ai maitrise                               ', 3, 3, 7, NULL, 2, 1, 1),
(121, 'Maîtriser la gestion du temps', '                        \r\n                   Je L ai maitrise                               ', 3, 4, 7, NULL, 2, 1, 1),
(122, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', '                        \r\n                   Je L ai maitrise                               ', 3, 5, 7, NULL, 2, 1, 1),
(123, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', '                                ', 3, 6, 7, NULL, 2, 1, 1),
(124, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', '                                ', 3, 7, 7, NULL, 2, 1, 1),
(125, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', '                                ', 3, 8, 7, NULL, 2, 1, 1),
(126, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', '                                ', 3, 9, 7, NULL, 2, 1, 1),
(127, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', '                                ', 3, 10, 7, NULL, 2, 1, 0),
(128, 'Développer l’opportunité entrepreneuriale ', '                                ', 3, 11, 7, NULL, 2, 1, 1),
(129, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', '                                ', 3, 12, 7, NULL, 2, 1, 1),
(130, 'Développer l’opportunité entrepreneuriale \r\n\r\n', '                                ', 3, 13, 7, NULL, 2, 1, 1),
(144, 'Développer ses aptitudes de communication', 'Développer ses aptitudes de communication', 2, 1, 12, NULL, 5, 1, 1),
(145, 'Maîtriser les techniques de la communication verbale et non verbale', 'Développer ses aptitudes de communication', 2, 2, 12, NULL, 5, 1, 1),
(146, 'Gérer les conflits', 'Développer ses aptitudes de communication', 1, 3, 12, NULL, 5, 1, 0),
(147, 'Maîtriser la gestion du temps', 'Développer ses aptitudes de communication', 1, 4, 12, NULL, 5, 1, 0),
(148, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', 'Développer ses aptitudes de communication', 1, 5, 12, NULL, 5, 1, 0),
(149, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', 'Développer ses aptitudes de communication', 1, 6, 12, NULL, 5, 1, 0),
(150, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', 'Développer ses aptitudes de communicationDévelopper ses aptitudes de communication', 1, 7, 12, NULL, 5, 1, 0),
(151, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', 'Développer ses aptitudes de communication', 1, 8, 12, NULL, 5, 1, 1),
(152, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', 'Développer ses aptitudes de communication', 1, 9, 12, NULL, 5, 1, 1),
(153, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', 'Développer ses aptitudes de communication', 1, 10, 12, NULL, 5, 1, 1),
(154, 'Développer l’opportunité entrepreneuriale ', 'Développer ses aptitudes de communication', 2, 11, 12, NULL, 5, 1, 1),
(155, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', 'Développer ses aptitudes de communication', 3, 12, 12, NULL, 5, 1, 0),
(156, 'Développer l’opportunité entrepreneuriale \r\n\r\n', 'Développer ses aptitudes de communication', 2, 13, 12, NULL, 5, 1, 0),
(170, 'Développer ses aptitudes de communication', NULL, 3, 1, 12, 9, NULL, 1, 0),
(171, 'Maîtriser les techniques de la communication verbale et non verbale', NULL, 3, 2, 12, 9, NULL, 1, 0),
(172, 'Gérer les conflits', NULL, 3, 3, 12, 9, NULL, 1, 0),
(173, 'Maîtriser la gestion du temps', NULL, 3, 4, 12, 9, NULL, 1, 0),
(174, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', NULL, 3, 5, 12, 9, NULL, 1, 0),
(175, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', NULL, 2, 6, 12, 9, NULL, 1, 0),
(176, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', NULL, 2, 7, 12, 9, NULL, 1, 0),
(177, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', NULL, 3, 8, 12, 9, NULL, 1, 0),
(178, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', NULL, 2, 9, 12, 9, NULL, 1, 0),
(179, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', NULL, 2, 10, 12, 9, NULL, 1, 0),
(180, 'Développer l’opportunité entrepreneuriale ', NULL, 3, 11, 12, 9, NULL, 1, 0),
(181, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', NULL, 3, 12, 12, 9, NULL, 1, 0),
(182, 'Développer l’opportunité entrepreneuriale \r\n\r\n', NULL, 3, 13, 12, 9, NULL, 1, 0),
(196, 'Développer ses aptitudes de communication', 'elopper ses aptitudes de communication', 3, 1, 13, NULL, 5, 1, 1),
(197, 'Maîtriser les techniques de la communication verbale et non verbale', 'elopper ses aptitudes de communication', 3, 2, 13, NULL, 5, 1, 1),
(198, 'Gérer les conflits', 'elopper ses aptitudes de communication', 3, 3, 13, NULL, 5, 1, 1),
(199, 'Maîtriser la gestion du temps', 'elopper ses aptitudes de communication', 2, 4, 13, NULL, 5, 1, 0),
(200, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', 'elopper ses aptitudes de communication', 2, 5, 13, NULL, 5, 1, 0),
(201, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', 'elopper ses aptitudes de communication', 2, 6, 13, NULL, 5, 1, 1),
(202, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', '', 1, 7, 13, NULL, 5, 1, 0),
(203, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', '', 1, 8, 13, NULL, 5, 1, 0),
(204, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', '', 1, 9, 13, NULL, 5, 1, 0),
(205, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', '', 1, 10, 13, NULL, 5, 1, 0),
(206, 'Développer l’opportunité entrepreneuriale ', '', 1, 11, 13, NULL, 5, 1, 0),
(207, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', '', 1, 12, 13, NULL, 5, 1, 0),
(208, 'Développer l’opportunité entrepreneuriale \r\n\r\n', 'elopper ses aptitudes de communication', 3, 13, 13, NULL, 5, 1, 1),
(222, 'Développer ses aptitudes de communication', NULL, 3, 1, 13, 9, NULL, 1, 0),
(223, 'Maîtriser les techniques de la communication verbale et non verbale', NULL, 2, 2, 13, 9, NULL, 1, 0),
(224, 'Gérer les conflits', NULL, 2, 3, 13, 9, NULL, 1, 0),
(225, 'Maîtriser la gestion du temps', NULL, 2, 4, 13, 9, NULL, 1, 0),
(226, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', NULL, 2, 5, 13, 9, NULL, 1, 0),
(227, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', NULL, 3, 6, 13, 9, NULL, 1, 0),
(228, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', NULL, 2, 7, 13, 9, NULL, 1, 0),
(229, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', NULL, 2, 8, 13, 9, NULL, 1, 0),
(230, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', NULL, 2, 9, 13, 9, NULL, 1, 0),
(231, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', NULL, 1, 10, 13, 9, NULL, 1, 0),
(232, 'Développer l’opportunité entrepreneuriale ', NULL, 1, 11, 13, 9, NULL, 1, 0),
(233, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', NULL, 1, 12, 13, 9, NULL, 1, 0),
(234, 'Développer l’opportunité entrepreneuriale \r\n\r\n', NULL, 1, 13, 13, 9, NULL, 1, 0),
(248, 'Développer ses aptitudes de communication', NULL, 2, 1, 14, 13, NULL, 1, 0),
(249, 'Maîtriser les techniques de la communication verbale et non verbale', NULL, 2, 2, 14, 13, NULL, 1, 0),
(250, 'Gérer les conflits', NULL, 1, 3, 14, 13, NULL, 1, 0),
(251, 'Maîtriser la gestion du temps', NULL, 1, 4, 14, 13, NULL, 1, 0),
(252, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', NULL, 1, 5, 14, 13, NULL, 1, 0),
(253, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', NULL, 3, 6, 14, 13, NULL, 1, 0),
(254, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', NULL, 2, 7, 14, 13, NULL, 1, 0),
(255, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', NULL, 2, 8, 14, 13, NULL, 1, 0),
(256, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', NULL, 2, 9, 14, 13, NULL, 1, 0),
(257, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', NULL, 2, 10, 14, 13, NULL, 1, 0),
(258, 'Développer l’opportunité entrepreneuriale ', NULL, 2, 11, 14, 13, NULL, 1, 0),
(259, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', NULL, 2, 12, 14, 13, NULL, 1, 0),
(260, 'Développer l’opportunité entrepreneuriale \r\n\r\n', NULL, 2, 13, 14, 13, NULL, 1, 0),
(274, 'Développer ses aptitudes de communication', 'Bien', 2, 1, 14, NULL, 5, 1, 1),
(275, 'Maîtriser les techniques de la communication verbale et non verbale', 'Bien', 3, 2, 14, NULL, 5, 1, 1),
(276, 'Gérer les conflits', 'Bie', 2, 3, 14, NULL, 5, 1, 1),
(277, 'Maîtriser la gestion du temps', 'Bien', 2, 4, 14, NULL, 5, 1, 1),
(278, 'Appliquer les méthodes efficaces pour la gestion d\'équipe', 'cc', 1, 5, 14, NULL, 5, 1, 0),
(279, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi', '', 1, 6, 14, NULL, 5, 1, 0),
(280, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)', '', 1, 7, 14, NULL, 5, 1, 0),
(281, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)', '', 1, 8, 14, NULL, 5, 1, 1),
(282, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)', '', 3, 9, 14, NULL, 5, 1, 1),
(283, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)', '', 1, 10, 14, NULL, 5, 1, 0),
(284, 'Développer l’opportunité entrepreneuriale ', '', 1, 11, 14, NULL, 5, 1, 0),
(285, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales', '', 1, 12, 14, NULL, 5, 1, 0),
(286, 'Développer l’opportunité entrepreneuriale \r\n\r\n', 'l’opportunité entrepreneuriale', 3, 13, 14, NULL, 5, 1, 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_bilan_competance_innovateur`
--

DROP TABLE IF EXISTS `28_2024_bilan_competance_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_bilan_competance_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `observation` text CHARACTER SET latin1 COLLATE latin1_swedish_ci,
  `evaluation` int DEFAULT NULL,
  `parameter_id` int NOT NULL,
  `project_id` int DEFAULT NULL,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `updated` tinyint(1) NOT NULL DEFAULT '0',
  `valide` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=331 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_bilan_competance_innovateur`
--

INSERT INTO `28_2024_bilan_competance_innovateur` (`id`, `name`, `observation`, `evaluation`, `parameter_id`, `project_id`, `etudiant_id`, `enseignant_id`, `updated`, `valide`) VALUES
(67, 'Coordonner les diverses composantes d’un projet (les fonctions de planification et d’exécution du projet et de contrôle du changement)\r\nconformément à des normes de qualité, pour établir un juste milieu entre la durée, le coût et la qualité du projet', NULL, 2, 1, 7, 1, NULL, 1, 0),
(68, 'Veiller à ce que le projet soit réalisé dans les délais prescrits', NULL, 2, 2, 7, 1, NULL, 1, 0),
(69, 'Veiller à ce que le projet soit réalisé dans les limites du budget alloué', NULL, 2, 3, 7, 1, NULL, 1, 0),
(70, 'Veiller à ce que le produit/service réponde aux besoins', NULL, 2, 4, 7, 1, NULL, 1, 0),
(71, 'Faire preuve d’un leadership pour assurer un travail d’équipe de qualité', NULL, 2, 5, 7, 1, NULL, 1, 0),
(72, 'Identifier les éléments du contexte du projet utiles à la communication (parties prenantes, budget, temps alloué, les limitations, contraintes,\r\nopportunités...)', NULL, 3, 6, 7, 1, NULL, 1, 0),
(73, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', NULL, 3, 7, 7, 1, NULL, 1, 0),
(74, 'Analyser les différents groupes cibles (leurs importances, leurs implications dans le projet, leurs connaissances, leurs aptitudes, leurs attentes...)', NULL, 3, 8, 7, 1, NULL, 1, 0),
(75, 'Déterminer les objectifs de communication ( les résultats à atteindre, les objectifs formulés SMART, les indicateurs...)', NULL, 3, 9, 7, 1, NULL, 1, 0),
(76, 'Définir quels messages pour quels groupes cibles (les messages primaires, les messages secondaires adaptés en fonction des groupes cibles, Keep It\r\nShort and Simple, storytelling, elevator pitch...)', NULL, 3, 10, 7, 1, NULL, 1, 0),
(77, 'Etablir le plan d’actions (plan de communication visuel, planning de communication: tableau de bord et suivi des actions...)', NULL, 2, 11, 7, 1, NULL, 1, 0),
(78, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', NULL, 2, 12, 7, 1, NULL, 1, 0),
(79, 'Faire le suivi et adapter : outil de travail qui doit évoluer en permanence basé sur un plan-do-chek-act', NULL, 2, 13, 7, 1, NULL, 1, 0),
(80, 'Evaluer et débriefer (objectifs sont atteints partiellement ou totalement, échec du plan, pistes d’amélioration, mesures d\'impact des actions, tirer des\r\nconclusions...)', NULL, 2, 14, 7, 1, NULL, 1, 0),
(81, 'Décrire l’activité avec précision (utilité des produits et/ou services, leurs caractéristiques, cible, lieu de vente...)', NULL, 2, 15, 7, 1, NULL, 1, 0),
(82, 'Etudier le marché visé (zone géographique, saisonnier ou permanent, niveau de concentration , barrières à l’entrée, types de clientèles, état actuel du\r\nmarché et ses tendances...)', NULL, 2, 16, 7, 1, NULL, 1, 0),
(83, 'Etudier la compatibilité de l\'innovateur avec le besoin du son projet (ses motivations, ses objectifs, son savoir-faire, son potentiel, s’il y a des\r\ncompétences manquantes et trouver des associés qui pourront les apporter...)', NULL, 2, 17, 7, 1, NULL, 1, 0),
(84, 'Analyser les forces et les faiblesses de l’idée (avantages par rapport à la concurrence et les contraintes du projet...)', NULL, 2, 18, 7, 1, NULL, 1, 0),
(85, 'Tester l’idée de création d’entreprise (sélectionner les testeurs, tester le produit/service, analyser les retours...)', NULL, 2, 19, 7, 1, NULL, 1, 0),
(86, 'Choisir la structure juridique selon la nature et l\'importance de l’activité (entreprise individuelle, SUARL, SARL, SA...)', NULL, 2, 20, 7, 1, NULL, 1, 0),
(87, 'Déterminer la forme de l’entreprise selon la nature de l’activité, la volonté de s’associer, l’organisation patrimoniale, l’engagement financier et la\r\ncrédibilité vis-à-vis des partenaires', NULL, 2, 21, 7, 1, NULL, 1, 0),
(88, 'Finaliser les formalités de constitution (attestations de dépôt de déclaration auprès de l\'API, déclaration d’ouverture de patente au bureau des impôts,\r\nenregistrement des statuts à la recette d’enregistrement des actes de sociétés...)', NULL, 2, 22, 7, 1, NULL, 1, 0),
(89, 'Faire une analyse stratégique du projet, recenser les opportunités de l’entreprise et les éventuelles difficultés à court, moyen ou long terme', NULL, 3, 23, 7, 1, NULL, 1, 0),
(90, 'Elaborer un plan de financement du projet : évaluer les besoins financiers pour la mise en œuvre et trouver les moyens qui couvrent les besoins', NULL, 2, 24, 7, 1, NULL, 1, 0),
(91, 'Donner une appréciation du chiffre d’affaires (hypothèse du taux d’utilisation de la capacité, production, prévision du chiffre d\'affaires...)', NULL, 3, 25, 7, 1, NULL, 1, 0),
(92, 'Estimer les charges du projet : s’acquitter de plusieurs charges, recenser toutes ces charges sans restriction aucune et d’en faire une synthèse', NULL, 2, 26, 7, 1, NULL, 1, 0),
(93, 'Etablir un compte de résultat prévisionnel : capacité à rembourser les emprunts et savoir s\'il y a suffisamment de bénéfices pour vivre du projet', NULL, 3, 27, 7, 1, NULL, 1, 0),
(94, 'Faire une étude financière qui permet de surveiller l’évolution de la trésorerie (e.g. sur 12 mois)', NULL, 2, 28, 7, 1, NULL, 1, 0),
(95, 'Créer un design du produit', NULL, 3, 29, 7, 1, NULL, 1, 0),
(96, 'Créer un prototype non fonctionnel (concevoir des plans 3D, 2D, établir le cahier des charges...)', NULL, 2, 30, 7, 1, NULL, 1, 0),
(97, 'Créer un prototype de produit fonctionnel (le chiffrage selon la quantité et les caractéristiques techniques...)', NULL, 3, 31, 7, 1, NULL, 1, 0),
(98, 'Valider des différents éléments du projet', NULL, 2, 32, 7, 1, NULL, 1, 0),
(99, 'Vérifier l’adéquation aux exigences préétablies (géométrie des prototypes, dimensions, caractéristiques visuelles/physiques /mécaniques, finition)', NULL, 3, 33, 7, 1, NULL, 1, 0),
(133, 'Coordonner les diverses composantes d’un projet (les fonctions de planification et d’exécution du projet et de contrôle du changement)\r\nconformément à des normes de qualité, pour établir un juste milieu entre la durée, le coût et la qualité du projet', 'Tres Bien', 3, 1, 7, NULL, 2, 1, 1),
(134, 'Veiller à ce que le projet soit réalisé dans les délais prescrits', 'Tres Bien', 3, 2, 7, NULL, 2, 1, 1),
(135, 'Veiller à ce que le projet soit réalisé dans les limites du budget alloué', 'Tres Bien', 3, 3, 7, NULL, 2, 1, 1),
(136, 'Veiller à ce que le produit/service réponde aux besoins', 'Tres Bien', 3, 4, 7, NULL, 2, 1, 1),
(137, 'Faire preuve d’un leadership pour assurer un travail d’équipe de qualité', 'Tres Bien', 3, 5, 7, NULL, 2, 1, 1),
(138, 'Identifier les éléments du contexte du projet utiles à la communication (parties prenantes, budget, temps alloué, les limitations, contraintes,\r\nopportunités...)', 'Tres Bien', 3, 6, 7, NULL, 2, 1, 1),
(139, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', 'Tres Bien', 3, 7, 7, NULL, 2, 1, 1),
(140, 'Analyser les différents groupes cibles (leurs importances, leurs implications dans le projet, leurs connaissances, leurs aptitudes, leurs attentes...)', 'Tres Bien', 3, 8, 7, NULL, 2, 1, 1),
(141, 'Déterminer les objectifs de communication ( les résultats à atteindre, les objectifs formulés SMART, les indicateurs...)', 'Tres Bien', 2, 9, 7, NULL, 2, 1, 1),
(142, 'Définir quels messages pour quels groupes cibles (les messages primaires, les messages secondaires adaptés en fonction des groupes cibles, Keep It\r\nShort and Simple, storytelling, elevator pitch...)', 'Tres Bien', 3, 10, 7, NULL, 2, 1, 1),
(143, 'Etablir le plan d’actions (plan de communication visuel, planning de communication: tableau de bord et suivi des actions...)', 'Tres Bien', 3, 11, 7, NULL, 2, 1, 1),
(144, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', 'Tres Bien', 3, 12, 7, NULL, 2, 1, 1),
(145, 'Faire le suivi et adapter : outil de travail qui doit évoluer en permanence basé sur un plan-do-chek-act', 'Tres Bien', 1, 13, 7, NULL, 2, 1, 0),
(146, 'Evaluer et débriefer (objectifs sont atteints partiellement ou totalement, échec du plan, pistes d’amélioration, mesures d\'impact des actions, tirer des\r\nconclusions...)', 'Tres Bien', 1, 14, 7, NULL, 2, 1, 0),
(147, 'Décrire l’activité avec précision (utilité des produits et/ou services, leurs caractéristiques, cible, lieu de vente...)', 'Tres Bien', 1, 15, 7, NULL, 2, 1, 0),
(148, 'Etudier le marché visé (zone géographique, saisonnier ou permanent, niveau de concentration , barrières à l’entrée, types de clientèles, état actuel du\r\nmarché et ses tendances...)', 'Tres Bien', 1, 16, 7, NULL, 2, 1, 0),
(149, 'Etudier la compatibilité de l\'innovateur avec le besoin du son projet (ses motivations, ses objectifs, son savoir-faire, son potentiel, s’il y a des\r\ncompétences manquantes et trouver des associés qui pourront les apporter...)', 'Tres Bien', 1, 17, 7, NULL, 2, 1, 0),
(150, 'Analyser les forces et les faiblesses de l’idée (avantages par rapport à la concurrence et les contraintes du projet...)', 'Tres Bien', 1, 18, 7, NULL, 2, 1, 0),
(151, 'Tester l’idée de création d’entreprise (sélectionner les testeurs, tester le produit/service, analyser les retours...)', 'Tres Bien', 1, 19, 7, NULL, 2, 1, 0),
(152, 'Choisir la structure juridique selon la nature et l\'importance de l’activité (entreprise individuelle, SUARL, SARL, SA...)', 'Tres Bien', 1, 20, 7, NULL, 2, 1, 0),
(153, 'Déterminer la forme de l’entreprise selon la nature de l’activité, la volonté de s’associer, l’organisation patrimoniale, l’engagement financier et la\r\ncrédibilité vis-à-vis des partenaires', 'Tres Bien', 3, 21, 7, NULL, 2, 1, 1),
(154, 'Finaliser les formalités de constitution (attestations de dépôt de déclaration auprès de l\'API, déclaration d’ouverture de patente au bureau des impôts,\r\nenregistrement des statuts à la recette d’enregistrement des actes de sociétés...)', 'Tres Bien', 3, 22, 7, NULL, 2, 1, 1),
(155, 'Faire une analyse stratégique du projet, recenser les opportunités de l’entreprise et les éventuelles difficultés à court, moyen ou long terme', 'Tres Bien', 3, 23, 7, NULL, 2, 1, 1),
(156, 'Elaborer un plan de financement du projet : évaluer les besoins financiers pour la mise en œuvre et trouver les moyens qui couvrent les besoins', 'Tres Bien', 2, 24, 7, NULL, 2, 1, 1),
(157, 'Donner une appréciation du chiffre d’affaires (hypothèse du taux d’utilisation de la capacité, production, prévision du chiffre d\'affaires...)', 'Tres Bien', 3, 25, 7, NULL, 2, 1, 1),
(158, 'Estimer les charges du projet : s’acquitter de plusieurs charges, recenser toutes ces charges sans restriction aucune et d’en faire une synthèse', 'Tres Bien', 3, 26, 7, NULL, 2, 1, 1),
(159, 'Etablir un compte de résultat prévisionnel : capacité à rembourser les emprunts et savoir s\'il y a suffisamment de bénéfices pour vivre du projet', 'Tres Bien', 2, 27, 7, NULL, 2, 1, 1),
(160, 'Faire une étude financière qui permet de surveiller l’évolution de la trésorerie (e.g. sur 12 mois)', 'Tres Bien', 1, 28, 7, NULL, 2, 1, 1),
(161, 'Créer un design du produit', 'Tres Bien', 2, 29, 7, NULL, 2, 1, 1),
(162, 'Créer un prototype non fonctionnel (concevoir des plans 3D, 2D, établir le cahier des charges...)', 'Créer un prototyp', 2, 30, 7, NULL, 2, 1, 1),
(163, 'Créer un prototype de produit fonctionnel (le chiffrage selon la quantité et les caractéristiques techniques...)', 'Tres Bien', 3, 31, 7, NULL, 2, 1, 0),
(164, 'Valider des différents éléments du projet', 'Tres Bien', 3, 32, 7, NULL, 2, 1, 1),
(165, 'Vérifier l’adéquation aux exigences préétablies (géométrie des prototypes, dimensions, caractéristiques visuelles/physiques /mécaniques, finition)', 'Tres Bien', 3, 33, 7, NULL, 2, 1, 1),
(199, 'Coordonner les diverses composantes d’un projet (les fonctions de planification et d’exécution du projet et de contrôle du changement)\r\nconformément à des normes de qualité, pour établir un juste milieu entre la durée, le coût et la qualité du projet', NULL, 2, 1, 12, 9, NULL, 1, 0),
(200, 'Veiller à ce que le projet soit réalisé dans les délais prescrits', NULL, 2, 2, 12, 9, NULL, 1, 0),
(201, 'Veiller à ce que le projet soit réalisé dans les limites du budget alloué', NULL, 2, 3, 12, 9, NULL, 1, 0),
(202, 'Veiller à ce que le produit/service réponde aux besoins', NULL, 2, 4, 12, 9, NULL, 1, 0),
(203, 'Faire preuve d’un leadership pour assurer un travail d’équipe de qualité', NULL, 2, 5, 12, 9, NULL, 1, 0),
(204, 'Identifier les éléments du contexte du projet utiles à la communication (parties prenantes, budget, temps alloué, les limitations, contraintes,\r\nopportunités...)', NULL, 2, 6, 12, 9, NULL, 1, 0),
(205, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', NULL, 2, 7, 12, 9, NULL, 1, 0),
(206, 'Analyser les différents groupes cibles (leurs importances, leurs implications dans le projet, leurs connaissances, leurs aptitudes, leurs attentes...)', NULL, 2, 8, 12, 9, NULL, 1, 0),
(207, 'Déterminer les objectifs de communication ( les résultats à atteindre, les objectifs formulés SMART, les indicateurs...)', NULL, 2, 9, 12, 9, NULL, 1, 0),
(208, 'Définir quels messages pour quels groupes cibles (les messages primaires, les messages secondaires adaptés en fonction des groupes cibles, Keep It\r\nShort and Simple, storytelling, elevator pitch...)', NULL, 3, 10, 12, 9, NULL, 1, 0),
(209, 'Etablir le plan d’actions (plan de communication visuel, planning de communication: tableau de bord et suivi des actions...)', NULL, 3, 11, 12, 9, NULL, 1, 0),
(210, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', NULL, 3, 12, 12, 9, NULL, 1, 0),
(211, 'Faire le suivi et adapter : outil de travail qui doit évoluer en permanence basé sur un plan-do-chek-act', NULL, 3, 13, 12, 9, NULL, 1, 0),
(212, 'Evaluer et débriefer (objectifs sont atteints partiellement ou totalement, échec du plan, pistes d’amélioration, mesures d\'impact des actions, tirer des\r\nconclusions...)', NULL, 3, 14, 12, 9, NULL, 1, 0),
(213, 'Décrire l’activité avec précision (utilité des produits et/ou services, leurs caractéristiques, cible, lieu de vente...)', NULL, 3, 15, 12, 9, NULL, 1, 0),
(214, 'Etudier le marché visé (zone géographique, saisonnier ou permanent, niveau de concentration , barrières à l’entrée, types de clientèles, état actuel du\r\nmarché et ses tendances...)', NULL, 3, 16, 12, 9, NULL, 1, 0),
(215, 'Etudier la compatibilité de l\'innovateur avec le besoin du son projet (ses motivations, ses objectifs, son savoir-faire, son potentiel, s’il y a des\r\ncompétences manquantes et trouver des associés qui pourront les apporter...)', NULL, 3, 17, 12, 9, NULL, 1, 0),
(216, 'Analyser les forces et les faiblesses de l’idée (avantages par rapport à la concurrence et les contraintes du projet...)', NULL, 3, 18, 12, 9, NULL, 1, 0),
(217, 'Tester l’idée de création d’entreprise (sélectionner les testeurs, tester le produit/service, analyser les retours...)', NULL, 3, 19, 12, 9, NULL, 1, 0),
(218, 'Choisir la structure juridique selon la nature et l\'importance de l’activité (entreprise individuelle, SUARL, SARL, SA...)', NULL, 3, 20, 12, 9, NULL, 1, 0),
(219, 'Déterminer la forme de l’entreprise selon la nature de l’activité, la volonté de s’associer, l’organisation patrimoniale, l’engagement financier et la\r\ncrédibilité vis-à-vis des partenaires', NULL, 3, 21, 12, 9, NULL, 1, 0),
(220, 'Finaliser les formalités de constitution (attestations de dépôt de déclaration auprès de l\'API, déclaration d’ouverture de patente au bureau des impôts,\r\nenregistrement des statuts à la recette d’enregistrement des actes de sociétés...)', NULL, 3, 22, 12, 9, NULL, 1, 0),
(221, 'Faire une analyse stratégique du projet, recenser les opportunités de l’entreprise et les éventuelles difficultés à court, moyen ou long terme', NULL, 1, 23, 12, 9, NULL, 1, 0),
(222, 'Elaborer un plan de financement du projet : évaluer les besoins financiers pour la mise en œuvre et trouver les moyens qui couvrent les besoins', NULL, 1, 24, 12, 9, NULL, 1, 0),
(223, 'Donner une appréciation du chiffre d’affaires (hypothèse du taux d’utilisation de la capacité, production, prévision du chiffre d\'affaires...)', NULL, 1, 25, 12, 9, NULL, 1, 0),
(224, 'Estimer les charges du projet : s’acquitter de plusieurs charges, recenser toutes ces charges sans restriction aucune et d’en faire une synthèse', NULL, 1, 26, 12, 9, NULL, 1, 0),
(225, 'Etablir un compte de résultat prévisionnel : capacité à rembourser les emprunts et savoir s\'il y a suffisamment de bénéfices pour vivre du projet', NULL, 1, 27, 12, 9, NULL, 1, 0),
(226, 'Faire une étude financière qui permet de surveiller l’évolution de la trésorerie (e.g. sur 12 mois)', NULL, 1, 28, 12, 9, NULL, 1, 0),
(227, 'Créer un design du produit', NULL, 1, 29, 12, 9, NULL, 1, 0),
(228, 'Créer un prototype non fonctionnel (concevoir des plans 3D, 2D, établir le cahier des charges...)', NULL, 1, 30, 12, 9, NULL, 1, 0),
(229, 'Créer un prototype de produit fonctionnel (le chiffrage selon la quantité et les caractéristiques techniques...)', NULL, 1, 31, 12, 9, NULL, 1, 0),
(230, 'Valider des différents éléments du projet', NULL, 1, 32, 12, 9, NULL, 1, 0),
(231, 'Vérifier l’adéquation aux exigences préétablies (géométrie des prototypes, dimensions, caractéristiques visuelles/physiques /mécaniques, finition)', NULL, 1, 33, 12, 9, NULL, 1, 0),
(265, 'Coordonner les diverses composantes d’un projet (les fonctions de planification et d’exécution du projet et de contrôle du changement)\r\nconformément à des normes de qualité, pour établir un juste milieu entre la durée, le coût et la qualité du projet', 'xsxsxsxs', 2, 1, 12, NULL, 5, 1, 1),
(266, 'Veiller à ce que le projet soit réalisé dans les délais prescrits', 'xsxsxsx', 2, 2, 12, NULL, 5, 1, 1),
(267, 'Veiller à ce que le projet soit réalisé dans les limites du budget alloué', 'xsxsx', 2, 3, 12, NULL, 5, 1, 1),
(268, 'Veiller à ce que le produit/service réponde aux besoins', 'xsxs', 1, 4, 12, NULL, 5, 1, 0),
(269, 'Faire preuve d’un leadership pour assurer un travail d’équipe de qualité', '', 1, 5, 12, NULL, 5, 1, 0),
(270, 'Identifier les éléments du contexte du projet utiles à la communication (parties prenantes, budget, temps alloué, les limitations, contraintes,\r\nopportunités...)', '', 1, 6, 12, NULL, 5, 1, 0),
(271, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', '', 1, 7, 12, NULL, 5, 1, 0),
(272, 'Analyser les différents groupes cibles (leurs importances, leurs implications dans le projet, leurs connaissances, leurs aptitudes, leurs attentes...)', '', 1, 8, 12, NULL, 5, 1, 0),
(273, 'Déterminer les objectifs de communication ( les résultats à atteindre, les objectifs formulés SMART, les indicateurs...)', '', 1, 9, 12, NULL, 5, 1, 0),
(274, 'Définir quels messages pour quels groupes cibles (les messages primaires, les messages secondaires adaptés en fonction des groupes cibles, Keep It\r\nShort and Simple, storytelling, elevator pitch...)', '', 1, 10, 12, NULL, 5, 1, 0),
(275, 'Etablir le plan d’actions (plan de communication visuel, planning de communication: tableau de bord et suivi des actions...)', '', 1, 11, 12, NULL, 5, 1, 0),
(276, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', '', 1, 12, 12, NULL, 5, 1, 0),
(277, 'Faire le suivi et adapter : outil de travail qui doit évoluer en permanence basé sur un plan-do-chek-act', '', 1, 13, 12, NULL, 5, 1, 0),
(278, 'Evaluer et débriefer (objectifs sont atteints partiellement ou totalement, échec du plan, pistes d’amélioration, mesures d\'impact des actions, tirer des\r\nconclusions...)', '', 1, 14, 12, NULL, 5, 1, 0),
(279, 'Décrire l’activité avec précision (utilité des produits et/ou services, leurs caractéristiques, cible, lieu de vente...)', '', 3, 15, 12, NULL, 5, 1, 1),
(280, 'Etudier le marché visé (zone géographique, saisonnier ou permanent, niveau de concentration , barrières à l’entrée, types de clientèles, état actuel du\r\nmarché et ses tendances...)', '', 1, 16, 12, NULL, 5, 1, 0),
(281, 'Etudier la compatibilité de l\'innovateur avec le besoin du son projet (ses motivations, ses objectifs, son savoir-faire, son potentiel, s’il y a des\r\ncompétences manquantes et trouver des associés qui pourront les apporter...)', '', 1, 17, 12, NULL, 5, 1, 0),
(282, 'Analyser les forces et les faiblesses de l’idée (avantages par rapport à la concurrence et les contraintes du projet...)', '', 1, 18, 12, NULL, 5, 1, 0),
(283, 'Tester l’idée de création d’entreprise (sélectionner les testeurs, tester le produit/service, analyser les retours...)', '', 1, 19, 12, NULL, 5, 1, 0),
(284, 'Choisir la structure juridique selon la nature et l\'importance de l’activité (entreprise individuelle, SUARL, SARL, SA...)', '', 1, 20, 12, NULL, 5, 1, 0),
(285, 'Déterminer la forme de l’entreprise selon la nature de l’activité, la volonté de s’associer, l’organisation patrimoniale, l’engagement financier et la\r\ncrédibilité vis-à-vis des partenaires', '', 1, 21, 12, NULL, 5, 1, 0),
(286, 'Finaliser les formalités de constitution (attestations de dépôt de déclaration auprès de l\'API, déclaration d’ouverture de patente au bureau des impôts,\r\nenregistrement des statuts à la recette d’enregistrement des actes de sociétés...)', '', 1, 22, 12, NULL, 5, 1, 0),
(287, 'Faire une analyse stratégique du projet, recenser les opportunités de l’entreprise et les éventuelles difficultés à court, moyen ou long terme', '', 1, 23, 12, NULL, 5, 1, 0),
(288, 'Elaborer un plan de financement du projet : évaluer les besoins financiers pour la mise en œuvre et trouver les moyens qui couvrent les besoins', '', 1, 24, 12, NULL, 5, 1, 0),
(289, 'Donner une appréciation du chiffre d’affaires (hypothèse du taux d’utilisation de la capacité, production, prévision du chiffre d\'affaires...)', '', 1, 25, 12, NULL, 5, 1, 0),
(290, 'Estimer les charges du projet : s’acquitter de plusieurs charges, recenser toutes ces charges sans restriction aucune et d’en faire une synthèse', '', 1, 26, 12, NULL, 5, 1, 0),
(291, 'Etablir un compte de résultat prévisionnel : capacité à rembourser les emprunts et savoir s\'il y a suffisamment de bénéfices pour vivre du projet', '', 1, 27, 12, NULL, 5, 1, 0),
(292, 'Faire une étude financière qui permet de surveiller l’évolution de la trésorerie (e.g. sur 12 mois)', '', 1, 28, 12, NULL, 5, 1, 0),
(293, 'Créer un design du produit', '', 1, 29, 12, NULL, 5, 1, 0),
(294, 'Créer un prototype non fonctionnel (concevoir des plans 3D, 2D, établir le cahier des charges...)', '', 1, 30, 12, NULL, 5, 1, 0),
(295, 'Créer un prototype de produit fonctionnel (le chiffrage selon la quantité et les caractéristiques techniques...)', '', 1, 31, 12, NULL, 5, 1, 0),
(296, 'Valider des différents éléments du projet', '', 1, 32, 12, NULL, 5, 1, 0),
(297, 'Vérifier l’adéquation aux exigences préétablies (géométrie des prototypes, dimensions, caractéristiques visuelles/physiques /mécaniques, finition)', '', 1, 33, 12, NULL, 5, 1, 0),
(298, 'Coordonner les diverses composantes d’un projet (les fonctions de planification et d’exécution du projet et de contrôle du changement)\r\nconformément à des normes de qualité, pour établir un juste milieu entre la durée, le coût et la qualité du projet', NULL, 1, 1, 14, NULL, 5, 0, 0),
(299, 'Veiller à ce que le projet soit réalisé dans les délais prescrits', NULL, 1, 2, 14, NULL, 5, 0, 0),
(300, 'Veiller à ce que le projet soit réalisé dans les limites du budget alloué', NULL, 1, 3, 14, NULL, 5, 0, 0),
(301, 'Veiller à ce que le produit/service réponde aux besoins', NULL, 1, 4, 14, NULL, 5, 0, 0),
(302, 'Faire preuve d’un leadership pour assurer un travail d’équipe de qualité', NULL, 1, 5, 14, NULL, 5, 0, 0),
(303, 'Identifier les éléments du contexte du projet utiles à la communication (parties prenantes, budget, temps alloué, les limitations, contraintes,\r\nopportunités...)', NULL, 1, 6, 14, NULL, 5, 0, 0),
(304, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', NULL, 1, 7, 14, NULL, 5, 0, 0),
(305, 'Analyser les différents groupes cibles (leurs importances, leurs implications dans le projet, leurs connaissances, leurs aptitudes, leurs attentes...)', NULL, 1, 8, 14, NULL, 5, 0, 0),
(306, 'Déterminer les objectifs de communication ( les résultats à atteindre, les objectifs formulés SMART, les indicateurs...)', NULL, 1, 9, 14, NULL, 5, 0, 0),
(307, 'Définir quels messages pour quels groupes cibles (les messages primaires, les messages secondaires adaptés en fonction des groupes cibles, Keep It\r\nShort and Simple, storytelling, elevator pitch...)', NULL, 1, 10, 14, NULL, 5, 0, 0),
(308, 'Etablir le plan d’actions (plan de communication visuel, planning de communication: tableau de bord et suivi des actions...)', NULL, 1, 11, 14, NULL, 5, 0, 0),
(309, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', NULL, 1, 12, 14, NULL, 5, 0, 0),
(310, 'Faire le suivi et adapter : outil de travail qui doit évoluer en permanence basé sur un plan-do-chek-act', NULL, 1, 13, 14, NULL, 5, 0, 0),
(311, 'Evaluer et débriefer (objectifs sont atteints partiellement ou totalement, échec du plan, pistes d’amélioration, mesures d\'impact des actions, tirer des\r\nconclusions...)', NULL, 1, 14, 14, NULL, 5, 0, 0),
(312, 'Décrire l’activité avec précision (utilité des produits et/ou services, leurs caractéristiques, cible, lieu de vente...)', NULL, 1, 15, 14, NULL, 5, 0, 0),
(313, 'Etudier le marché visé (zone géographique, saisonnier ou permanent, niveau de concentration , barrières à l’entrée, types de clientèles, état actuel du\r\nmarché et ses tendances...)', NULL, 1, 16, 14, NULL, 5, 0, 0),
(314, 'Etudier la compatibilité de l\'innovateur avec le besoin du son projet (ses motivations, ses objectifs, son savoir-faire, son potentiel, s’il y a des\r\ncompétences manquantes et trouver des associés qui pourront les apporter...)', NULL, 1, 17, 14, NULL, 5, 0, 0),
(315, 'Analyser les forces et les faiblesses de l’idée (avantages par rapport à la concurrence et les contraintes du projet...)', NULL, 1, 18, 14, NULL, 5, 0, 0),
(316, 'Tester l’idée de création d’entreprise (sélectionner les testeurs, tester le produit/service, analyser les retours...)', NULL, 1, 19, 14, NULL, 5, 0, 0),
(317, 'Choisir la structure juridique selon la nature et l\'importance de l’activité (entreprise individuelle, SUARL, SARL, SA...)', NULL, 1, 20, 14, NULL, 5, 0, 0),
(318, 'Déterminer la forme de l’entreprise selon la nature de l’activité, la volonté de s’associer, l’organisation patrimoniale, l’engagement financier et la\r\ncrédibilité vis-à-vis des partenaires', NULL, 1, 21, 14, NULL, 5, 0, 0),
(319, 'Finaliser les formalités de constitution (attestations de dépôt de déclaration auprès de l\'API, déclaration d’ouverture de patente au bureau des impôts,\r\nenregistrement des statuts à la recette d’enregistrement des actes de sociétés...)', NULL, 1, 22, 14, NULL, 5, 0, 0),
(320, 'Faire une analyse stratégique du projet, recenser les opportunités de l’entreprise et les éventuelles difficultés à court, moyen ou long terme', NULL, 1, 23, 14, NULL, 5, 0, 0),
(321, 'Elaborer un plan de financement du projet : évaluer les besoins financiers pour la mise en œuvre et trouver les moyens qui couvrent les besoins', NULL, 1, 24, 14, NULL, 5, 0, 0),
(322, 'Donner une appréciation du chiffre d’affaires (hypothèse du taux d’utilisation de la capacité, production, prévision du chiffre d\'affaires...)', NULL, 1, 25, 14, NULL, 5, 0, 0),
(323, 'Estimer les charges du projet : s’acquitter de plusieurs charges, recenser toutes ces charges sans restriction aucune et d’en faire une synthèse', NULL, 1, 26, 14, NULL, 5, 0, 0),
(324, 'Etablir un compte de résultat prévisionnel : capacité à rembourser les emprunts et savoir s\'il y a suffisamment de bénéfices pour vivre du projet', NULL, 1, 27, 14, NULL, 5, 0, 0),
(325, 'Faire une étude financière qui permet de surveiller l’évolution de la trésorerie (e.g. sur 12 mois)', NULL, 1, 28, 14, NULL, 5, 0, 0),
(326, 'Créer un design du produit', NULL, 1, 29, 14, NULL, 5, 0, 0),
(327, 'Créer un prototype non fonctionnel (concevoir des plans 3D, 2D, établir le cahier des charges...)', NULL, 1, 30, 14, NULL, 5, 0, 0),
(328, 'Créer un prototype de produit fonctionnel (le chiffrage selon la quantité et les caractéristiques techniques...)', NULL, 1, 31, 14, NULL, 5, 0, 0),
(329, 'Valider des différents éléments du projet', NULL, 1, 32, 14, NULL, 5, 0, 0),
(330, 'Vérifier l’adéquation aux exigences préétablies (géométrie des prototypes, dimensions, caractéristiques visuelles/physiques /mécaniques, finition)', NULL, 1, 33, 14, NULL, 5, 0, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_bilan_competance_promoteur`
--

DROP TABLE IF EXISTS `28_2024_bilan_competance_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_bilan_competance_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `observation` text,
  `evaluation` int DEFAULT NULL,
  `parameter_id` int NOT NULL,
  `project_id` int DEFAULT NULL,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `updated` tinyint(1) NOT NULL DEFAULT '0',
  `valide` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=307 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_bilan_competance_promoteur`
--

INSERT INTO `28_2024_bilan_competance_promoteur` (`id`, `name`, `observation`, `evaluation`, `parameter_id`, `project_id`, `etudiant_id`, `enseignant_id`, `updated`, `valide`) VALUES
(35, 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', NULL, 2, 1, 7, 1, NULL, 1, 0),
(36, 'Mesurer le niveau d’intérêt et de pouvoir sur le projet de chaque partie prenante', NULL, 2, 2, 7, 1, NULL, 1, 0),
(37, 'Classifier les parties prenantes', NULL, 2, 3, 7, 1, NULL, 1, 0),
(38, 'Communiquer avec les différentes parties prenantes et partenaires, les persuader et entretenir des relations durables avec eux', NULL, 3, 4, 7, 1, NULL, 1, 0),
(39, 'Définir une stratégie de négociation appropriée pour chaque partie prenante afin de répondre aux attentes communes', NULL, 2, 5, 7, 1, NULL, 1, 0),
(40, 'Elaborer la matrice RACI (responsible, accountable, consulted et informed) et/ou RAM (responsibility assignment matrix) : répartir les tâches, organiser\r\nles circuits d’information, de décision, de validation', NULL, 3, 6, 7, 1, NULL, 1, 0),
(41, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', NULL, 3, 7, 7, 1, NULL, 1, 0),
(42, 'Mener un entretien de recrutement en utilisant les techniques adéquates', NULL, 3, 8, 7, 1, NULL, 1, 0),
(43, 'Installer un esprit et une dynamique d’équipe', NULL, 3, 9, 7, 1, NULL, 1, 0),
(44, 'Mettre en œuvre une politique de motivation des membres de l\'équipe : mettre en place des structures incitatives, motiver ses collaborateurs...', NULL, 3, 10, 7, 1, NULL, 1, 0),
(45, 'Mettre en œuvre une politique de rémunération en définissant les salaires et autres composantes de rémunération', NULL, 3, 11, 7, 1, NULL, 1, 0),
(46, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', NULL, 3, 12, 7, 1, NULL, 1, 0),
(47, 'Faire preuve d’écoute vis-à-vis des collaborateurs, être attentifs aux problèmes (gestion des conflits)', NULL, 3, 13, 7, 1, NULL, 1, 0),
(48, 'Mettre en œuvre une politique de communication interne efficace', NULL, 2, 14, 7, 1, NULL, 1, 0),
(49, 'Elaborer une matrice d’analyse des risques', NULL, 3, 15, 7, 1, NULL, 1, 0),
(50, 'Identifier tous les risques possibles (positifs et négatifs)', NULL, 3, 16, 7, 1, NULL, 1, 0),
(51, 'Mesurer l\'importance du risque et classifier les risques selon l’importance', NULL, 2, 17, 7, 1, NULL, 1, 0),
(52, 'Proposer les solutions pour affronter les risques (éliminer, réduire, transférer, accepter, pivoter...)', NULL, 3, 18, 7, 1, NULL, 1, 0),
(53, 'Mettre en œuvre les mesures de protection intellectuelle', NULL, 2, 19, 7, 1, NULL, 1, 0),
(54, 'Formaliser les contrats avec les partenaires d’affaire', NULL, 2, 20, 7, 1, NULL, 1, 0),
(55, 'Engager les formalités de création de l’entreprise', NULL, 1, 21, 7, 1, NULL, 1, 0),
(56, 'Identifier les différentes ressources de financement (autofinancement, dettes bancaires, fonds d’investissements, subventions, leasing...)', NULL, 1, 22, 7, 1, NULL, 1, 0),
(57, 'Identifier les incitations des investissements (code d’incitation des investissements, lois d\'investissements...)', NULL, 1, 23, 7, 1, NULL, 1, 0),
(58, 'Choisir la structure de financement optimale', NULL, 1, 24, 7, 1, NULL, 1, 0),
(59, 'Respecter le planing de projet.', NULL, 1, 25, 7, 1, NULL, 1, 0),
(60, 'Faire preuve d’efficience et d’efficacité: Définir et surveiller l’atteinte des indicateurs clés de performances du projet.', NULL, 1, 26, 7, 1, NULL, 1, 0),
(61, 'Ajuster le plan d\'action: déterminer les écarts et les mesures de corrections nécessaires.', NULL, 1, 27, 7, 1, NULL, 1, 0),
(62, 'Modifier le plan d\'action en présence de risques imprévus (covid, forces majeurs...).', NULL, 1, 28, 7, 1, NULL, 1, 0),
(63, 'Être en mesure d’identifier la stratégie commerciale convenable (en fonction du produit/service créé): domination par les coûts, différenciation,\r\nconcentration...', NULL, 1, 29, 7, 1, NULL, 1, 0),
(64, 'Savoir tenir un discours commercial efficace', NULL, 1, 30, 7, 1, NULL, 1, 0),
(65, 'Savoir créer un lien de confiance et fidéliser la clientèle...', NULL, 1, 31, 7, 1, NULL, 1, 0),
(66, 'Concevoir une stratégie de communication bien ciblée, cohérente avec des objectifs marketing clairs', NULL, 1, 32, 7, 1, NULL, 1, 0),
(67, 'Comprendre les usages et les valeurs ajoutées des réseaux sociaux pour augmenter sa visibilité', NULL, 1, 33, 7, 1, NULL, 1, 0),
(68, 'Valoriser ses profils et son activité en ligne : développer sa e-réputation; découvrir les notions d’identité numérique, d’empreinte numérique...', NULL, 3, 34, 7, 1, NULL, 1, 0),
(103, 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 'done', 3, 1, 7, NULL, 2, 1, 1),
(104, 'Mesurer le niveau d’intérêt et de pouvoir sur le projet de chaque partie prenante', 'done', 2, 2, 7, NULL, 2, 1, 1),
(105, 'Classifier les parties prenantes', 'done', 2, 3, 7, NULL, 2, 1, 1),
(106, 'Communiquer avec les différentes parties prenantes et partenaires, les persuader et entretenir des relations durables avec eux', 'done', 2, 4, 7, NULL, 2, 1, 1),
(107, 'Définir une stratégie de négociation appropriée pour chaque partie prenante afin de répondre aux attentes communes', 'tres bien', 3, 5, 7, NULL, 2, 1, 1),
(108, 'Elaborer la matrice RACI (responsible, accountable, consulted et informed) et/ou RAM (responsibility assignment matrix) : répartir les tâches, organiser\r\nles circuits d’information, de décision, de validation', '', 1, 6, 7, NULL, 2, 1, 0),
(109, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', '', 1, 7, 7, NULL, 2, 1, 0),
(110, 'Mener un entretien de recrutement en utilisant les techniques adéquates', '', 1, 8, 7, NULL, 2, 1, 0),
(111, 'Installer un esprit et une dynamique d’équipe', '', 1, 9, 7, NULL, 2, 1, 0),
(112, 'Mettre en œuvre une politique de motivation des membres de l\'équipe : mettre en place des structures incitatives, motiver ses collaborateurs...', '', 1, 10, 7, NULL, 2, 1, 0),
(113, 'Mettre en œuvre une politique de rémunération en définissant les salaires et autres composantes de rémunération', '', 1, 11, 7, NULL, 2, 1, 0),
(114, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', '', 1, 12, 7, NULL, 2, 1, 0),
(115, 'Faire preuve d’écoute vis-à-vis des collaborateurs, être attentifs aux problèmes (gestion des conflits)', '', 1, 13, 7, NULL, 2, 1, 0),
(116, 'Mettre en œuvre une politique de communication interne efficace', '', 1, 14, 7, NULL, 2, 1, 0),
(117, 'Elaborer une matrice d’analyse des risques', '', 1, 15, 7, NULL, 2, 1, 0),
(118, 'Identifier tous les risques possibles (positifs et négatifs)', '', 1, 16, 7, NULL, 2, 1, 0),
(119, 'Mesurer l\'importance du risque et classifier les risques selon l’importance', '', 1, 17, 7, NULL, 2, 1, 0),
(120, 'Proposer les solutions pour affronter les risques (éliminer, réduire, transférer, accepter, pivoter...)', '', 1, 18, 7, NULL, 2, 1, 0),
(121, 'Mettre en œuvre les mesures de protection intellectuelle', '', 1, 19, 7, NULL, 2, 1, 0),
(122, 'Formaliser les contrats avec les partenaires d’affaire', '', 1, 20, 7, NULL, 2, 1, 0),
(123, 'Engager les formalités de création de l’entreprise', '', 1, 21, 7, NULL, 2, 1, 0),
(124, 'Identifier les différentes ressources de financement (autofinancement, dettes bancaires, fonds d’investissements, subventions, leasing...)', '', 1, 22, 7, NULL, 2, 1, 0),
(125, 'Identifier les incitations des investissements (code d’incitation des investissements, lois d\'investissements...)', '', 1, 23, 7, NULL, 2, 1, 0),
(126, 'Choisir la structure de financement optimale', '', 1, 24, 7, NULL, 2, 1, 0),
(127, 'Respecter le planing de projet.', '', 1, 25, 7, NULL, 2, 1, 0),
(128, 'Faire preuve d’efficience et d’efficacité: Définir et surveiller l’atteinte des indicateurs clés de performances du projet.', '', 1, 26, 7, NULL, 2, 1, 0),
(129, 'Ajuster le plan d\'action: déterminer les écarts et les mesures de corrections nécessaires.', '', 1, 27, 7, NULL, 2, 1, 0),
(130, 'Modifier le plan d\'action en présence de risques imprévus (covid, forces majeurs...).', '', 1, 28, 7, NULL, 2, 1, 0),
(131, 'Être en mesure d’identifier la stratégie commerciale convenable (en fonction du produit/service créé): domination par les coûts, différenciation,\r\nconcentration...', '', 1, 29, 7, NULL, 2, 1, 0),
(132, 'Savoir tenir un discours commercial efficace', '', 1, 30, 7, NULL, 2, 1, 0),
(133, 'Savoir créer un lien de confiance et fidéliser la clientèle...', '', 1, 31, 7, NULL, 2, 1, 0),
(134, 'Concevoir une stratégie de communication bien ciblée, cohérente avec des objectifs marketing clairs', '', 1, 32, 7, NULL, 2, 1, 0),
(135, 'Comprendre les usages et les valeurs ajoutées des réseaux sociaux pour augmenter sa visibilité', '', 1, 33, 7, NULL, 2, 1, 0),
(136, 'Valoriser ses profils et son activité en ligne : développer sa e-réputation; découvrir les notions d’identité numérique, d’empreinte numérique...', 'Bien', 2, 34, 7, NULL, 2, 1, 1),
(171, 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', NULL, 2, 1, 12, 9, NULL, 1, 0),
(172, 'Mesurer le niveau d’intérêt et de pouvoir sur le projet de chaque partie prenante', NULL, 1, 2, 12, 9, NULL, 1, 0),
(173, 'Classifier les parties prenantes', NULL, 3, 3, 12, 9, NULL, 1, 0),
(174, 'Communiquer avec les différentes parties prenantes et partenaires, les persuader et entretenir des relations durables avec eux', NULL, 3, 4, 12, 9, NULL, 1, 0),
(175, 'Définir une stratégie de négociation appropriée pour chaque partie prenante afin de répondre aux attentes communes', NULL, 1, 5, 12, 9, NULL, 1, 0),
(176, 'Elaborer la matrice RACI (responsible, accountable, consulted et informed) et/ou RAM (responsibility assignment matrix) : répartir les tâches, organiser\r\nles circuits d’information, de décision, de validation', NULL, 1, 6, 12, 9, NULL, 1, 0),
(177, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', NULL, 1, 7, 12, 9, NULL, 1, 0),
(178, 'Mener un entretien de recrutement en utilisant les techniques adéquates', NULL, 1, 8, 12, 9, NULL, 1, 0),
(179, 'Installer un esprit et une dynamique d’équipe', NULL, 1, 9, 12, 9, NULL, 1, 0),
(180, 'Mettre en œuvre une politique de motivation des membres de l\'équipe : mettre en place des structures incitatives, motiver ses collaborateurs...', NULL, 1, 10, 12, 9, NULL, 1, 0),
(181, 'Mettre en œuvre une politique de rémunération en définissant les salaires et autres composantes de rémunération', NULL, 1, 11, 12, 9, NULL, 1, 0),
(182, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', NULL, 1, 12, 12, 9, NULL, 1, 0),
(183, 'Faire preuve d’écoute vis-à-vis des collaborateurs, être attentifs aux problèmes (gestion des conflits)', NULL, 1, 13, 12, 9, NULL, 1, 0),
(184, 'Mettre en œuvre une politique de communication interne efficace', NULL, 1, 14, 12, 9, NULL, 1, 0),
(185, 'Elaborer une matrice d’analyse des risques', NULL, 1, 15, 12, 9, NULL, 1, 0),
(186, 'Identifier tous les risques possibles (positifs et négatifs)', NULL, 1, 16, 12, 9, NULL, 1, 0),
(187, 'Mesurer l\'importance du risque et classifier les risques selon l’importance', NULL, 1, 17, 12, 9, NULL, 1, 0),
(188, 'Proposer les solutions pour affronter les risques (éliminer, réduire, transférer, accepter, pivoter...)', NULL, 1, 18, 12, 9, NULL, 1, 0),
(189, 'Mettre en œuvre les mesures de protection intellectuelle', NULL, 1, 19, 12, 9, NULL, 1, 0),
(190, 'Formaliser les contrats avec les partenaires d’affaire', NULL, 1, 20, 12, 9, NULL, 1, 0),
(191, 'Engager les formalités de création de l’entreprise', NULL, 1, 21, 12, 9, NULL, 1, 0),
(192, 'Identifier les différentes ressources de financement (autofinancement, dettes bancaires, fonds d’investissements, subventions, leasing...)', NULL, 1, 22, 12, 9, NULL, 1, 0),
(193, 'Identifier les incitations des investissements (code d’incitation des investissements, lois d\'investissements...)', NULL, 1, 23, 12, 9, NULL, 1, 0),
(194, 'Choisir la structure de financement optimale', NULL, 1, 24, 12, 9, NULL, 1, 0),
(195, 'Respecter le planing de projet.', NULL, 1, 25, 12, 9, NULL, 1, 0),
(196, 'Faire preuve d’efficience et d’efficacité: Définir et surveiller l’atteinte des indicateurs clés de performances du projet.', NULL, 1, 26, 12, 9, NULL, 1, 0),
(197, 'Ajuster le plan d\'action: déterminer les écarts et les mesures de corrections nécessaires.', NULL, 1, 27, 12, 9, NULL, 1, 0),
(198, 'Modifier le plan d\'action en présence de risques imprévus (covid, forces majeurs...).', NULL, 1, 28, 12, 9, NULL, 1, 0),
(199, 'Être en mesure d’identifier la stratégie commerciale convenable (en fonction du produit/service créé): domination par les coûts, différenciation,\r\nconcentration...', NULL, 1, 29, 12, 9, NULL, 1, 0),
(200, 'Savoir tenir un discours commercial efficace', NULL, 1, 30, 12, 9, NULL, 1, 0),
(201, 'Savoir créer un lien de confiance et fidéliser la clientèle...', NULL, 1, 31, 12, 9, NULL, 1, 0),
(202, 'Concevoir une stratégie de communication bien ciblée, cohérente avec des objectifs marketing clairs', NULL, 1, 32, 12, 9, NULL, 1, 0),
(203, 'Comprendre les usages et les valeurs ajoutées des réseaux sociaux pour augmenter sa visibilité', NULL, 1, 33, 12, 9, NULL, 1, 0),
(204, 'Valoriser ses profils et son activité en ligne : développer sa e-réputation; découvrir les notions d’identité numérique, d’empreinte numérique...', NULL, 2, 34, 12, 9, NULL, 1, 0),
(239, 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 1, 1, 12, NULL, 5, 1, 1),
(240, 'Mesurer le niveau d’intérêt et de pouvoir sur le projet de chaque partie prenante', 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 3, 2, 12, NULL, 5, 1, 1),
(241, 'Classifier les parties prenantes', 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 1, 3, 12, NULL, 5, 1, 0),
(242, 'Communiquer avec les différentes parties prenantes et partenaires, les persuader et entretenir des relations durables avec eux', '', 1, 4, 12, NULL, 5, 1, 0),
(243, 'Définir une stratégie de négociation appropriée pour chaque partie prenante afin de répondre aux attentes communes', '', 1, 5, 12, NULL, 5, 1, 0),
(244, 'Elaborer la matrice RACI (responsible, accountable, consulted et informed) et/ou RAM (responsibility assignment matrix) : répartir les tâches, organiser\r\nles circuits d’information, de décision, de validation', '', 1, 6, 12, NULL, 5, 1, 0),
(245, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', '', 1, 7, 12, NULL, 5, 1, 0),
(246, 'Mener un entretien de recrutement en utilisant les techniques adéquates', '', 1, 8, 12, NULL, 5, 1, 0),
(247, 'Installer un esprit et une dynamique d’équipe', '', 1, 9, 12, NULL, 5, 1, 0),
(248, 'Mettre en œuvre une politique de motivation des membres de l\'équipe : mettre en place des structures incitatives, motiver ses collaborateurs...', '', 1, 10, 12, NULL, 5, 1, 0),
(249, 'Mettre en œuvre une politique de rémunération en définissant les salaires et autres composantes de rémunération', '', 1, 11, 12, NULL, 5, 1, 0),
(250, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', '', 1, 12, 12, NULL, 5, 1, 0),
(251, 'Faire preuve d’écoute vis-à-vis des collaborateurs, être attentifs aux problèmes (gestion des conflits)', '', 1, 13, 12, NULL, 5, 1, 0),
(252, 'Mettre en œuvre une politique de communication interne efficace', '', 1, 14, 12, NULL, 5, 1, 0),
(253, 'Elaborer une matrice d’analyse des risques', '', 1, 15, 12, NULL, 5, 1, 0),
(254, 'Identifier tous les risques possibles (positifs et négatifs)', '', 1, 16, 12, NULL, 5, 1, 0),
(255, 'Mesurer l\'importance du risque et classifier les risques selon l’importance', '', 1, 17, 12, NULL, 5, 1, 0),
(256, 'Proposer les solutions pour affronter les risques (éliminer, réduire, transférer, accepter, pivoter...)', '', 1, 18, 12, NULL, 5, 1, 0),
(257, 'Mettre en œuvre les mesures de protection intellectuelle', '', 1, 19, 12, NULL, 5, 1, 0),
(258, 'Formaliser les contrats avec les partenaires d’affaire', '', 1, 20, 12, NULL, 5, 1, 0),
(259, 'Engager les formalités de création de l’entreprise', '', 1, 21, 12, NULL, 5, 1, 0),
(260, 'Identifier les différentes ressources de financement (autofinancement, dettes bancaires, fonds d’investissements, subventions, leasing...)', '', 1, 22, 12, NULL, 5, 1, 0),
(261, 'Identifier les incitations des investissements (code d’incitation des investissements, lois d\'investissements...)', 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 2, 23, 12, NULL, 5, 1, 1),
(262, 'Choisir la structure de financement optimale', '', 1, 24, 12, NULL, 5, 1, 0),
(263, 'Respecter le planing de projet.', '', 1, 25, 12, NULL, 5, 1, 0),
(264, 'Faire preuve d’efficience et d’efficacité: Définir et surveiller l’atteinte des indicateurs clés de performances du projet.', '', 1, 26, 12, NULL, 5, 1, 0),
(265, 'Ajuster le plan d\'action: déterminer les écarts et les mesures de corrections nécessaires.', '', 1, 27, 12, NULL, 5, 1, 0),
(266, 'Modifier le plan d\'action en présence de risques imprévus (covid, forces majeurs...).', '', 1, 28, 12, NULL, 5, 1, 0),
(267, 'Être en mesure d’identifier la stratégie commerciale convenable (en fonction du produit/service créé): domination par les coûts, différenciation,\r\nconcentration...', '', 1, 29, 12, NULL, 5, 1, 0),
(268, 'Savoir tenir un discours commercial efficace', 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 1, 30, 12, NULL, 5, 1, 0),
(269, 'Savoir créer un lien de confiance et fidéliser la clientèle...', '', 1, 31, 12, NULL, 5, 1, 0),
(270, 'Concevoir une stratégie de communication bien ciblée, cohérente avec des objectifs marketing clairs', '', 1, 32, 12, NULL, 5, 1, 0),
(271, 'Comprendre les usages et les valeurs ajoutées des réseaux sociaux pour augmenter sa visibilité', '', 1, 33, 12, NULL, 5, 1, 0),
(272, 'Valoriser ses profils et son activité en ligne : développer sa e-réputation; découvrir les notions d’identité numérique, d’empreinte numérique...', '', 1, 34, 12, NULL, 5, 1, 0),
(273, 'Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', NULL, 1, 1, 13, NULL, 5, 0, 0),
(274, 'Mesurer le niveau d’intérêt et de pouvoir sur le projet de chaque partie prenante', NULL, 1, 2, 13, NULL, 5, 0, 0),
(275, 'Classifier les parties prenantes', NULL, 1, 3, 13, NULL, 5, 0, 0),
(276, 'Communiquer avec les différentes parties prenantes et partenaires, les persuader et entretenir des relations durables avec eux', NULL, 1, 4, 13, NULL, 5, 0, 0),
(277, 'Définir une stratégie de négociation appropriée pour chaque partie prenante afin de répondre aux attentes communes', NULL, 1, 5, 13, NULL, 5, 0, 0),
(278, 'Elaborer la matrice RACI (responsible, accountable, consulted et informed) et/ou RAM (responsibility assignment matrix) : répartir les tâches, organiser\r\nles circuits d’information, de décision, de validation', NULL, 1, 6, 13, NULL, 5, 0, 0),
(279, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', NULL, 1, 7, 13, NULL, 5, 0, 0),
(280, 'Mener un entretien de recrutement en utilisant les techniques adéquates', NULL, 1, 8, 13, NULL, 5, 0, 0),
(281, 'Installer un esprit et une dynamique d’équipe', NULL, 1, 9, 13, NULL, 5, 0, 0),
(282, 'Mettre en œuvre une politique de motivation des membres de l\'équipe : mettre en place des structures incitatives, motiver ses collaborateurs...', NULL, 1, 10, 13, NULL, 5, 0, 0),
(283, 'Mettre en œuvre une politique de rémunération en définissant les salaires et autres composantes de rémunération', NULL, 1, 11, 13, NULL, 5, 0, 0),
(284, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', NULL, 1, 12, 13, NULL, 5, 0, 0),
(285, 'Faire preuve d’écoute vis-à-vis des collaborateurs, être attentifs aux problèmes (gestion des conflits)', NULL, 1, 13, 13, NULL, 5, 0, 0),
(286, 'Mettre en œuvre une politique de communication interne efficace', NULL, 1, 14, 13, NULL, 5, 0, 0),
(287, 'Elaborer une matrice d’analyse des risques', NULL, 1, 15, 13, NULL, 5, 0, 0),
(288, 'Identifier tous les risques possibles (positifs et négatifs)', NULL, 1, 16, 13, NULL, 5, 0, 0),
(289, 'Mesurer l\'importance du risque et classifier les risques selon l’importance', NULL, 1, 17, 13, NULL, 5, 0, 0),
(290, 'Proposer les solutions pour affronter les risques (éliminer, réduire, transférer, accepter, pivoter...)', NULL, 1, 18, 13, NULL, 5, 0, 0),
(291, 'Mettre en œuvre les mesures de protection intellectuelle', NULL, 1, 19, 13, NULL, 5, 0, 0),
(292, 'Formaliser les contrats avec les partenaires d’affaire', NULL, 1, 20, 13, NULL, 5, 0, 0),
(293, 'Engager les formalités de création de l’entreprise', NULL, 1, 21, 13, NULL, 5, 0, 0),
(294, 'Identifier les différentes ressources de financement (autofinancement, dettes bancaires, fonds d’investissements, subventions, leasing...)', NULL, 1, 22, 13, NULL, 5, 0, 0),
(295, 'Identifier les incitations des investissements (code d’incitation des investissements, lois d\'investissements...)', NULL, 1, 23, 13, NULL, 5, 0, 0),
(296, 'Choisir la structure de financement optimale', NULL, 1, 24, 13, NULL, 5, 0, 0),
(297, 'Respecter le planing de projet.', NULL, 1, 25, 13, NULL, 5, 0, 0),
(298, 'Faire preuve d’efficience et d’efficacité: Définir et surveiller l’atteinte des indicateurs clés de performances du projet.', NULL, 1, 26, 13, NULL, 5, 0, 0),
(299, 'Ajuster le plan d\'action: déterminer les écarts et les mesures de corrections nécessaires.', NULL, 1, 27, 13, NULL, 5, 0, 0),
(300, 'Modifier le plan d\'action en présence de risques imprévus (covid, forces majeurs...).', NULL, 1, 28, 13, NULL, 5, 0, 0),
(301, 'Être en mesure d’identifier la stratégie commerciale convenable (en fonction du produit/service créé): domination par les coûts, différenciation,\r\nconcentration...', NULL, 1, 29, 13, NULL, 5, 0, 0),
(302, 'Savoir tenir un discours commercial efficace', NULL, 1, 30, 13, NULL, 5, 0, 0),
(303, 'Savoir créer un lien de confiance et fidéliser la clientèle...', NULL, 1, 31, 13, NULL, 5, 0, 0),
(304, 'Concevoir une stratégie de communication bien ciblée, cohérente avec des objectifs marketing clairs', NULL, 1, 32, 13, NULL, 5, 0, 0),
(305, 'Comprendre les usages et les valeurs ajoutées des réseaux sociaux pour augmenter sa visibilité', NULL, 1, 33, 13, NULL, 5, 0, 0),
(306, 'Valoriser ses profils et son activité en ligne : développer sa e-réputation; découvrir les notions d’identité numérique, d’empreinte numérique...', NULL, 1, 34, 13, NULL, 5, 0, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_cache_users`
--

DROP TABLE IF EXISTS `28_2024_cache_users`;
CREATE TABLE IF NOT EXISTS `28_2024_cache_users` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `prenom` varchar(255) NOT NULL,
  `etablissement_id` int NOT NULL,
  `email` varchar(255) NOT NULL,
  `phone` varchar(255) NOT NULL,
  `niveau` varchar(255) NOT NULL,
  `old_promotion` varchar(255) DEFAULT NULL,
  `old_niveau` varchar(255) DEFAULT NULL,
  `appel_id` int NOT NULL,
  `addedBy` int NOT NULL,
  `cv` varchar(255) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_capsules_videos`
--

DROP TABLE IF EXISTS `28_2024_capsules_videos`;
CREATE TABLE IF NOT EXISTS `28_2024_capsules_videos` (
  `id` int NOT NULL AUTO_INCREMENT,
  `titre` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `titre-en` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `id_video` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `created_at` date NOT NULL,
  `published` tinyint(1) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_capsules_videos`
--

INSERT INTO `28_2024_capsules_videos` (`id`, `titre`, `titre-en`, `id_video`, `created_at`, `published`) VALUES
(3, 'Inauguration du nouveau Pole e lEtudiant Entrepreneur de luniversité de Sousse', 'Inauguration of the new Student Entrepreneur Center at the University of Sousse ', '7BOKK1qBXK0', '2024-11-07', 1),
(4, '1er regroupement national des Pôles Étudiant-Entrepreneur en Tunisie', '1st national grouping of Student-Entrepreneur Poles in Tunisia', 'jvUll7xbKbs', '2024-11-07', 1),
(5, 'Appel à candidature du Pôle Étudiant Entrepreneur', 'Call for applications from the Student Entrepreneur Center', 'A6K6orBxMa8', '2024-11-07', 1),
(6, 'UNE JOURNÉE DANS MA VIE D\'ÉTUDIANTE', 'A DAY IN MY LIFE AS A STUDENT', 'EN9lEZgyymI', '2024-11-07', 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_certificats`
--

DROP TABLE IF EXISTS `28_2024_certificats`;
CREATE TABLE IF NOT EXISTS `28_2024_certificats` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date NOT NULL,
  `etudiant_id` int DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_certificats`
--

INSERT INTO `28_2024_certificats` (`id`, `nom`, `date_debut`, `date_fin`, `etudiant_id`) VALUES
(1, 'certificat cybersecurity updated', '2024-10-17', '2024-12-25', 1),
(3, '3D Painting', '2022-03-05', '2022-06-04', 8),
(4, 'wordpress', '2024-03-01', '2024-09-06', 15);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_coaching`
--

DROP TABLE IF EXISTS `28_2024_coaching`;
CREATE TABLE IF NOT EXISTS `28_2024_coaching` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `thumbnail` varchar(255) DEFAULT NULL,
  `rating` int NOT NULL DEFAULT '0',
  `created_at` date NOT NULL,
  `referent_id` int DEFAULT NULL,
  `publie` tinyint(1) NOT NULL DEFAULT '0',
  `nb_seance` int NOT NULL DEFAULT '0',
  `date_overture` date DEFAULT NULL,
  `date_debut` date DEFAULT NULL,
  `date_fin` date DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_coaching`
--

INSERT INTO `28_2024_coaching` (`id`, `nom`, `thumbnail`, `rating`, `created_at`, `referent_id`, `publie`, `nb_seance`, `date_overture`, `date_debut`, `date_fin`) VALUES
(1, 'test coaching', NULL, 0, '2024-11-28', 1, 0, 2, NULL, '2024-11-30', '2024-12-01');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_coaching_domaines`
--

DROP TABLE IF EXISTS `28_2024_coaching_domaines`;
CREATE TABLE IF NOT EXISTS `28_2024_coaching_domaines` (
  `id` int NOT NULL AUTO_INCREMENT,
  `coaching_id` int NOT NULL,
  `domaine_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_coaching_groupes`
--

DROP TABLE IF EXISTS `28_2024_coaching_groupes`;
CREATE TABLE IF NOT EXISTS `28_2024_coaching_groupes` (
  `id` int NOT NULL AUTO_INCREMENT,
  `coaching_id` int NOT NULL,
  `groupe_id` int NOT NULL,
  `progression` int NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_coaching_students`
--

DROP TABLE IF EXISTS `28_2024_coaching_students`;
CREATE TABLE IF NOT EXISTS `28_2024_coaching_students` (
  `id` int NOT NULL AUTO_INCREMENT,
  `coaching_id` int NOT NULL,
  `student_id` int NOT NULL,
  `progression` int NOT NULL DEFAULT '0',
  `fiche_chaude_ready` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_coaching_students`
--

INSERT INTO `28_2024_coaching_students` (`id`, `coaching_id`, `student_id`, `progression`, `fiche_chaude_ready`) VALUES
(1, 1, 8, 0, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_comite_rende_vous`
--

DROP TABLE IF EXISTS `28_2024_comite_rende_vous`;
CREATE TABLE IF NOT EXISTS `28_2024_comite_rende_vous` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_projet` int NOT NULL,
  `date_comite` date NOT NULL,
  `heure_comite` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `lieu_comite` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_comite_rende_vous`
--

INSERT INTO `28_2024_comite_rende_vous` (`id`, `id_projet`, `date_comite`, `heure_comite`, `lieu_comite`) VALUES
(1, 7, '2024-11-13', '10:00', 'Faculté des Lettres et des Sciences Humaines de Sousse'),
(2, 9, '2024-11-13', '10:00', 'Faculté des Lettres et des Sciences Humaines de Sousse'),
(17, 12, '2024-11-15', '09:00', 'Chott Mariem'),
(18, 10, '2024-11-15', '09:00', 'Chott Mariem'),
(19, 13, '2024-11-09', '09:30', 'Tunis'),
(20, 8, '2024-11-20', '10:30', 'Sousse'),
(21, 8, '2024-11-19', '10:30', 'Sousse');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_competition`
--

DROP TABLE IF EXISTS `28_2024_competition`;
CREATE TABLE IF NOT EXISTS `28_2024_competition` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date NOT NULL,
  `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `photo` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_competition`
--

INSERT INTO `28_2024_competition` (`id`, `nom`, `date_debut`, `date_fin`, `description`, `photo`) VALUES
(1, 'testtt compétition', '2024-10-29', '2024-11-10', 'testttttttttttttttt testttttttttttttttttttt', 'https://esen-sci-compet.com/eesu/assets/assets/images/competition3.jpg'),
(3, 'creation didentite visuelle', '2024-11-05', '2024-11-30', 'fjghjhgjhvjh', 'https://esen-sci-compet.com/eesu/assets/assets/images/15.png');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_demandes_coaching`
--

DROP TABLE IF EXISTS `28_2024_demandes_coaching`;
CREATE TABLE IF NOT EXISTS `28_2024_demandes_coaching` (
  `id` int NOT NULL AUTO_INCREMENT,
  `coaching_id` int NOT NULL,
  `etudiant_id` int NOT NULL,
  `accepted` tinyint(1) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_demandes_formation`
--

DROP TABLE IF EXISTS `28_2024_demandes_formation`;
CREATE TABLE IF NOT EXISTS `28_2024_demandes_formation` (
  `id` int NOT NULL AUTO_INCREMENT,
  `formation_id` int NOT NULL,
  `etudiant_id` int NOT NULL,
  `accepted` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_demandes_formation`
--

INSERT INTO `28_2024_demandes_formation` (`id`, `formation_id`, `etudiant_id`, `accepted`) VALUES
(3, 2, 16, 0),
(4, 7, 16, 1),
(5, 4, 16, 0),
(6, 7, 8, 1),
(7, 8, 8, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_deroulement`
--

DROP TABLE IF EXISTS `28_2024_deroulement`;
CREATE TABLE IF NOT EXISTS `28_2024_deroulement` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_attribution_deroulement` int NOT NULL,
  `status` tinyint(1) NOT NULL DEFAULT '0',
  `id_projet` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1726 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_deroulement`
--

INSERT INTO `28_2024_deroulement` (`id`, `id_attribution_deroulement`, `status`, `id_projet`) VALUES
(24, 24, 0, 7),
(25, 25, 0, 7),
(26, 26, 0, 7),
(27, 27, 0, 7),
(28, 28, 0, 7),
(29, 29, 0, 7),
(30, 30, 0, 7),
(31, 31, 0, 7),
(32, 32, 0, 7),
(33, 33, 0, 7),
(99, 17, 1, 7),
(100, 18, 1, 7),
(101, 19, 1, 7),
(102, 20, 1, 7),
(103, 21, 1, 7),
(104, 22, 1, 7),
(105, 23, 1, 7),
(106, 34, 1, 7),
(107, 35, 1, 7),
(108, 36, 1, 7),
(109, 37, 1, 7),
(110, 38, 1, 7),
(111, 39, 1, 7),
(112, 40, 1, 7),
(113, 41, 1, 7),
(114, 42, 1, 7),
(115, 43, 1, 7),
(116, 44, 1, 7),
(117, 45, 1, 7),
(118, 46, 1, 7),
(119, 47, 1, 7),
(120, 48, 1, 7),
(121, 49, 1, 7),
(122, 50, 1, 7),
(123, 51, 1, 7),
(124, 52, 1, 7),
(125, 53, 1, 7),
(126, 54, 1, 7),
(127, 55, 1, 7),
(128, 56, 0, 7),
(129, 57, 0, 7),
(130, 58, 1, 7),
(131, 59, 1, 7),
(132, 60, 0, 7),
(133, 61, 0, 7),
(134, 1, 0, 10),
(135, 2, 0, 10),
(136, 3, 0, 10),
(137, 4, 0, 10),
(138, 5, 0, 10),
(139, 6, 0, 10),
(140, 7, 0, 10),
(141, 8, 0, 10),
(142, 9, 0, 10),
(143, 10, 0, 10),
(144, 11, 0, 10),
(145, 12, 0, 10),
(146, 13, 0, 10),
(147, 14, 0, 10),
(148, 15, 0, 10),
(149, 16, 0, 10),
(150, 17, 0, 10),
(151, 18, 0, 10),
(152, 19, 0, 10),
(153, 20, 0, 10),
(154, 21, 0, 10),
(155, 22, 0, 10),
(156, 23, 0, 10),
(157, 24, 0, 10),
(158, 25, 0, 10),
(159, 26, 0, 10),
(160, 27, 0, 10),
(161, 28, 0, 10),
(162, 29, 0, 10),
(163, 30, 0, 10),
(164, 31, 0, 10),
(165, 32, 0, 10),
(166, 33, 0, 10),
(167, 34, 0, 10),
(168, 35, 0, 10),
(169, 36, 0, 10),
(170, 37, 0, 10),
(171, 38, 0, 10),
(172, 39, 0, 10),
(173, 40, 0, 10),
(174, 41, 0, 10),
(175, 42, 0, 10),
(176, 43, 0, 10),
(177, 44, 0, 10),
(178, 45, 0, 10),
(179, 46, 0, 10),
(180, 47, 0, 10),
(181, 48, 0, 10),
(182, 49, 0, 10),
(183, 50, 0, 10),
(184, 51, 0, 10),
(185, 52, 0, 10),
(186, 53, 0, 10),
(187, 54, 0, 10),
(188, 55, 0, 10),
(189, 56, 0, 10),
(190, 57, 0, 10),
(191, 58, 0, 10),
(192, 59, 0, 10),
(193, 60, 0, 10),
(194, 61, 0, 10),
(218, 24, 0, 12),
(219, 25, 0, 12),
(220, 26, 0, 12),
(221, 27, 0, 12),
(222, 28, 0, 12),
(223, 29, 0, 12),
(224, 30, 0, 12),
(225, 31, 0, 12),
(226, 32, 0, 12),
(227, 33, 0, 12),
(228, 34, 0, 12),
(229, 35, 0, 12),
(230, 36, 0, 12),
(231, 37, 0, 12),
(232, 38, 0, 12),
(233, 39, 0, 12),
(234, 40, 0, 12),
(235, 41, 0, 12),
(236, 42, 0, 12),
(237, 43, 0, 12),
(238, 44, 0, 12),
(239, 45, 0, 12),
(240, 46, 0, 12),
(241, 47, 0, 12),
(242, 48, 0, 12),
(243, 49, 0, 12),
(244, 50, 0, 12),
(245, 51, 0, 12),
(246, 52, 0, 12),
(247, 53, 0, 12),
(248, 54, 0, 12),
(249, 55, 0, 12),
(250, 56, 0, 12),
(251, 57, 0, 12),
(252, 58, 0, 12),
(253, 59, 0, 12),
(254, 60, 0, 12),
(255, 61, 0, 12),
(263, 8, 0, 11),
(264, 9, 0, 11),
(265, 10, 0, 11),
(266, 11, 0, 11),
(267, 12, 0, 11),
(268, 13, 0, 11),
(269, 14, 0, 11),
(270, 15, 0, 11),
(271, 16, 0, 11),
(272, 17, 0, 11),
(273, 18, 0, 11),
(274, 19, 0, 11),
(275, 20, 0, 11),
(276, 21, 0, 11),
(277, 22, 0, 11),
(278, 23, 0, 11),
(279, 24, 0, 11),
(280, 25, 0, 11),
(281, 26, 0, 11),
(282, 27, 0, 11),
(283, 28, 0, 11),
(284, 29, 0, 11),
(285, 30, 0, 11),
(286, 31, 0, 11),
(287, 32, 0, 11),
(288, 33, 0, 11),
(289, 34, 0, 11),
(290, 35, 0, 11),
(291, 36, 0, 11),
(292, 37, 0, 11),
(293, 38, 0, 11),
(294, 39, 0, 11),
(295, 40, 0, 11),
(296, 41, 0, 11),
(297, 42, 0, 11),
(298, 43, 0, 11),
(299, 44, 0, 11),
(300, 45, 0, 11),
(301, 46, 0, 11),
(302, 47, 0, 11),
(303, 48, 0, 11),
(304, 49, 0, 11),
(305, 50, 0, 11),
(306, 51, 0, 11),
(307, 52, 0, 11),
(308, 53, 0, 11),
(309, 54, 0, 11),
(310, 55, 0, 11),
(311, 56, 0, 11),
(312, 57, 0, 11),
(313, 58, 0, 11),
(314, 59, 0, 11),
(315, 60, 0, 11),
(316, 61, 0, 11),
(317, 1, 1, 11),
(318, 2, 1, 11),
(319, 3, 1, 11),
(320, 4, 1, 11),
(321, 5, 1, 11),
(322, 6, 1, 11),
(323, 7, 1, 11),
(324, 1, 1, 12),
(325, 2, 1, 12),
(326, 3, 1, 12),
(327, 4, 1, 12),
(328, 5, 1, 12),
(329, 6, 1, 12),
(330, 7, 1, 12),
(338, 8, 1, 12),
(339, 9, 1, 12),
(340, 10, 1, 12),
(341, 11, 1, 12),
(342, 12, 1, 12),
(343, 13, 1, 12),
(344, 14, 1, 12),
(354, 15, 1, 12),
(355, 16, 1, 12),
(356, 17, 1, 12),
(357, 18, 1, 12),
(358, 19, 1, 12),
(359, 20, 1, 12),
(360, 21, 1, 12),
(361, 22, 1, 12),
(362, 23, 1, 12),
(370, 8, 0, 13),
(371, 9, 0, 13),
(372, 10, 0, 13),
(373, 11, 0, 13),
(374, 12, 0, 13),
(375, 13, 0, 13),
(376, 14, 0, 13),
(377, 15, 0, 13),
(378, 16, 0, 13),
(379, 17, 0, 13),
(380, 18, 0, 13),
(381, 19, 0, 13),
(382, 20, 0, 13),
(383, 21, 0, 13),
(384, 22, 0, 13),
(385, 23, 0, 13),
(386, 24, 0, 13),
(387, 25, 0, 13),
(388, 26, 0, 13),
(389, 27, 0, 13),
(390, 28, 0, 13),
(391, 29, 0, 13),
(392, 30, 0, 13),
(393, 31, 0, 13),
(394, 32, 0, 13),
(395, 33, 0, 13),
(396, 34, 0, 13),
(397, 35, 0, 13),
(398, 36, 0, 13),
(399, 37, 0, 13),
(400, 38, 0, 13),
(401, 39, 0, 13),
(402, 40, 0, 13),
(403, 41, 0, 13),
(404, 42, 0, 13),
(405, 43, 0, 13),
(406, 44, 0, 13),
(407, 45, 0, 13),
(408, 46, 0, 13),
(409, 47, 0, 13),
(410, 48, 0, 13),
(411, 49, 0, 13),
(412, 50, 0, 13),
(413, 51, 0, 13),
(414, 52, 0, 13),
(415, 53, 0, 13),
(416, 54, 0, 13),
(417, 55, 0, 13),
(418, 56, 0, 13),
(419, 57, 0, 13),
(420, 58, 0, 13),
(421, 59, 0, 13),
(422, 60, 0, 13),
(423, 61, 0, 13),
(431, 1, 1, 13),
(432, 2, 1, 13),
(433, 3, 1, 13),
(434, 4, 1, 13),
(435, 5, 1, 13),
(436, 6, 1, 13),
(437, 7, 1, 13),
(445, 8, 0, 14),
(446, 9, 0, 14),
(447, 10, 0, 14),
(448, 11, 0, 14),
(449, 12, 0, 14),
(450, 13, 0, 14),
(451, 14, 0, 14),
(452, 15, 0, 14),
(453, 16, 0, 14),
(454, 17, 0, 14),
(455, 18, 0, 14),
(456, 19, 0, 14),
(457, 20, 0, 14),
(458, 21, 0, 14),
(459, 22, 0, 14),
(460, 23, 0, 14),
(461, 24, 0, 14),
(462, 25, 0, 14),
(463, 26, 0, 14),
(464, 27, 0, 14),
(465, 28, 0, 14),
(466, 29, 0, 14),
(467, 30, 0, 14),
(468, 31, 0, 14),
(469, 32, 0, 14),
(470, 33, 0, 14),
(471, 34, 0, 14),
(472, 35, 0, 14),
(473, 36, 0, 14),
(474, 37, 0, 14),
(475, 38, 0, 14),
(476, 39, 0, 14),
(477, 40, 0, 14),
(478, 41, 0, 14),
(479, 42, 0, 14),
(480, 43, 0, 14),
(481, 44, 0, 14),
(482, 45, 0, 14),
(483, 46, 0, 14),
(484, 47, 0, 14),
(485, 48, 0, 14),
(486, 49, 0, 14),
(487, 50, 0, 14),
(488, 51, 0, 14),
(489, 52, 0, 14),
(490, 53, 0, 14),
(491, 54, 0, 14),
(492, 55, 0, 14),
(493, 56, 0, 14),
(494, 57, 0, 14),
(495, 58, 0, 14),
(496, 59, 0, 14),
(497, 60, 0, 14),
(498, 61, 0, 14),
(499, 1, 1, 14),
(500, 2, 1, 14),
(501, 3, 1, 14),
(502, 4, 1, 14),
(503, 5, 1, 14),
(504, 6, 1, 14),
(505, 7, 1, 14),
(506, 1, 0, 10),
(507, 2, 0, 10),
(508, 3, 0, 10),
(509, 4, 0, 10),
(510, 5, 0, 10),
(511, 6, 0, 10),
(512, 7, 0, 10),
(513, 8, 0, 10),
(514, 9, 0, 10),
(515, 10, 0, 10),
(516, 11, 0, 10),
(517, 12, 0, 10),
(518, 13, 0, 10),
(519, 14, 0, 10),
(520, 15, 0, 10),
(521, 16, 0, 10),
(522, 17, 0, 10),
(523, 18, 0, 10),
(524, 19, 0, 10),
(525, 20, 0, 10),
(526, 21, 0, 10),
(527, 22, 0, 10),
(528, 23, 0, 10),
(529, 24, 0, 10),
(530, 25, 0, 10),
(531, 26, 0, 10),
(532, 27, 0, 10),
(533, 28, 0, 10),
(534, 29, 0, 10),
(535, 30, 0, 10),
(536, 31, 0, 10),
(537, 32, 0, 10),
(538, 33, 0, 10),
(539, 34, 0, 10),
(540, 35, 0, 10),
(541, 36, 0, 10),
(542, 37, 0, 10),
(543, 38, 0, 10),
(544, 39, 0, 10),
(545, 40, 0, 10),
(546, 41, 0, 10),
(547, 42, 0, 10),
(548, 43, 0, 10),
(549, 44, 0, 10),
(550, 45, 0, 10),
(551, 46, 0, 10),
(552, 47, 0, 10),
(553, 48, 0, 10),
(554, 49, 0, 10),
(555, 50, 0, 10),
(556, 51, 0, 10),
(557, 52, 0, 10),
(558, 53, 0, 10),
(559, 54, 0, 10),
(560, 55, 0, 10),
(561, 56, 0, 10),
(562, 57, 0, 10),
(563, 58, 0, 10),
(564, 59, 0, 10),
(565, 60, 0, 10),
(566, 61, 0, 10),
(567, 1, 0, 11),
(568, 2, 0, 11),
(569, 3, 0, 11),
(570, 4, 0, 11),
(571, 5, 0, 11),
(572, 6, 0, 11),
(573, 7, 0, 11),
(574, 8, 0, 11),
(575, 9, 0, 11),
(576, 10, 0, 11),
(577, 11, 0, 11),
(578, 12, 0, 11),
(579, 13, 0, 11),
(580, 14, 0, 11),
(581, 15, 0, 11),
(582, 16, 0, 11),
(583, 17, 0, 11),
(584, 18, 0, 11),
(585, 19, 0, 11),
(586, 20, 0, 11),
(587, 21, 0, 11),
(588, 22, 0, 11),
(589, 23, 0, 11),
(590, 24, 0, 11),
(591, 25, 0, 11),
(592, 26, 0, 11),
(593, 27, 0, 11),
(594, 28, 0, 11),
(595, 29, 0, 11),
(596, 30, 0, 11),
(597, 31, 0, 11),
(598, 32, 0, 11),
(599, 33, 0, 11),
(600, 34, 0, 11),
(601, 35, 0, 11),
(602, 36, 0, 11),
(603, 37, 0, 11),
(604, 38, 0, 11),
(605, 39, 0, 11),
(606, 40, 0, 11),
(607, 41, 0, 11),
(608, 42, 0, 11),
(609, 43, 0, 11),
(610, 44, 0, 11),
(611, 45, 0, 11),
(612, 46, 0, 11),
(613, 47, 0, 11),
(614, 48, 0, 11),
(615, 49, 0, 11),
(616, 50, 0, 11),
(617, 51, 0, 11),
(618, 52, 0, 11),
(619, 53, 0, 11),
(620, 54, 0, 11),
(621, 55, 0, 11),
(622, 56, 0, 11),
(623, 57, 0, 11),
(624, 58, 0, 11),
(625, 59, 0, 11),
(626, 60, 0, 11),
(627, 61, 0, 11),
(628, 1, 0, 11),
(629, 2, 0, 11),
(630, 3, 0, 11),
(631, 4, 0, 11),
(632, 5, 0, 11),
(633, 6, 0, 11),
(634, 7, 0, 11),
(635, 8, 0, 11),
(636, 9, 0, 11),
(637, 10, 0, 11),
(638, 11, 0, 11),
(639, 12, 0, 11),
(640, 13, 0, 11),
(641, 14, 0, 11),
(642, 15, 0, 11),
(643, 16, 0, 11),
(644, 17, 0, 11),
(645, 18, 0, 11),
(646, 19, 0, 11),
(647, 20, 0, 11),
(648, 21, 0, 11),
(649, 22, 0, 11),
(650, 23, 0, 11),
(651, 24, 0, 11),
(652, 25, 0, 11),
(653, 26, 0, 11),
(654, 27, 0, 11),
(655, 28, 0, 11),
(656, 29, 0, 11),
(657, 30, 0, 11),
(658, 31, 0, 11),
(659, 32, 0, 11),
(660, 33, 0, 11),
(661, 34, 0, 11),
(662, 35, 0, 11),
(663, 36, 0, 11),
(664, 37, 0, 11),
(665, 38, 0, 11),
(666, 39, 0, 11),
(667, 40, 0, 11),
(668, 41, 0, 11),
(669, 42, 0, 11),
(670, 43, 0, 11),
(671, 44, 0, 11),
(672, 45, 0, 11),
(673, 46, 0, 11),
(674, 47, 0, 11),
(675, 48, 0, 11),
(676, 49, 0, 11),
(677, 50, 0, 11),
(678, 51, 0, 11),
(679, 52, 0, 11),
(680, 53, 0, 11),
(681, 54, 0, 11),
(682, 55, 0, 11),
(683, 56, 0, 11),
(684, 57, 0, 11),
(685, 58, 0, 11),
(686, 59, 0, 11),
(687, 60, 0, 11),
(688, 61, 0, 11),
(689, 1, 0, 11),
(690, 2, 0, 11),
(691, 3, 0, 11),
(692, 4, 0, 11),
(693, 5, 0, 11),
(694, 6, 0, 11),
(695, 7, 0, 11),
(696, 8, 0, 11),
(697, 9, 0, 11),
(698, 10, 0, 11),
(699, 11, 0, 11),
(700, 12, 0, 11),
(701, 13, 0, 11),
(702, 14, 0, 11),
(703, 15, 0, 11),
(704, 16, 0, 11),
(705, 17, 0, 11),
(706, 18, 0, 11),
(707, 19, 0, 11),
(708, 20, 0, 11),
(709, 21, 0, 11),
(710, 22, 0, 11),
(711, 23, 0, 11),
(712, 24, 0, 11),
(713, 25, 0, 11),
(714, 26, 0, 11),
(715, 27, 0, 11),
(716, 28, 0, 11),
(717, 29, 0, 11),
(718, 30, 0, 11),
(719, 31, 0, 11),
(720, 32, 0, 11),
(721, 33, 0, 11),
(722, 34, 0, 11),
(723, 35, 0, 11),
(724, 36, 0, 11),
(725, 37, 0, 11),
(726, 38, 0, 11),
(727, 39, 0, 11),
(728, 40, 0, 11),
(729, 41, 0, 11),
(730, 42, 0, 11),
(731, 43, 0, 11),
(732, 44, 0, 11),
(733, 45, 0, 11),
(734, 46, 0, 11),
(735, 47, 0, 11),
(736, 48, 0, 11),
(737, 49, 0, 11),
(738, 50, 0, 11),
(739, 51, 0, 11),
(740, 52, 0, 11),
(741, 53, 0, 11),
(742, 54, 0, 11),
(743, 55, 0, 11),
(744, 56, 0, 11),
(745, 57, 0, 11),
(746, 58, 0, 11),
(747, 59, 0, 11),
(748, 60, 0, 11),
(749, 61, 0, 11),
(750, 1, 0, 11),
(751, 2, 0, 11),
(752, 3, 0, 11),
(753, 4, 0, 11),
(754, 5, 0, 11),
(755, 6, 0, 11),
(756, 7, 0, 11),
(757, 8, 0, 11),
(758, 9, 0, 11),
(759, 10, 0, 11),
(760, 11, 0, 11),
(761, 12, 0, 11),
(762, 13, 0, 11),
(763, 14, 0, 11),
(764, 15, 0, 11),
(765, 16, 0, 11),
(766, 17, 0, 11),
(767, 18, 0, 11),
(768, 19, 0, 11),
(769, 20, 0, 11),
(770, 21, 0, 11),
(771, 22, 0, 11),
(772, 23, 0, 11),
(773, 24, 0, 11),
(774, 25, 0, 11),
(775, 26, 0, 11),
(776, 27, 0, 11),
(777, 28, 0, 11),
(778, 29, 0, 11),
(779, 30, 0, 11),
(780, 31, 0, 11),
(781, 32, 0, 11),
(782, 33, 0, 11),
(783, 34, 0, 11),
(784, 35, 0, 11),
(785, 36, 0, 11),
(786, 37, 0, 11),
(787, 38, 0, 11),
(788, 39, 0, 11),
(789, 40, 0, 11),
(790, 41, 0, 11),
(791, 42, 0, 11),
(792, 43, 0, 11),
(793, 44, 0, 11),
(794, 45, 0, 11),
(795, 46, 0, 11),
(796, 47, 0, 11),
(797, 48, 0, 11),
(798, 49, 0, 11),
(799, 50, 0, 11),
(800, 51, 0, 11),
(801, 52, 0, 11),
(802, 53, 0, 11),
(803, 54, 0, 11),
(804, 55, 0, 11),
(805, 56, 0, 11),
(806, 57, 0, 11),
(807, 58, 0, 11),
(808, 59, 0, 11),
(809, 60, 0, 11),
(810, 61, 0, 11),
(811, 1, 0, 11),
(812, 2, 0, 11),
(813, 3, 0, 11),
(814, 4, 0, 11),
(815, 5, 0, 11),
(816, 6, 0, 11),
(817, 7, 0, 11),
(818, 8, 0, 11),
(819, 9, 0, 11),
(820, 10, 0, 11),
(821, 11, 0, 11),
(822, 12, 0, 11),
(823, 13, 0, 11),
(824, 14, 0, 11),
(825, 15, 0, 11),
(826, 16, 0, 11),
(827, 17, 0, 11),
(828, 18, 0, 11),
(829, 19, 0, 11),
(830, 20, 0, 11),
(831, 21, 0, 11),
(832, 22, 0, 11),
(833, 23, 0, 11),
(834, 24, 0, 11),
(835, 25, 0, 11),
(836, 26, 0, 11),
(837, 27, 0, 11),
(838, 28, 0, 11),
(839, 29, 0, 11),
(840, 30, 0, 11),
(841, 31, 0, 11),
(842, 32, 0, 11),
(843, 33, 0, 11),
(844, 34, 0, 11),
(845, 35, 0, 11),
(846, 36, 0, 11),
(847, 37, 0, 11),
(848, 38, 0, 11),
(849, 39, 0, 11),
(850, 40, 0, 11),
(851, 41, 0, 11),
(852, 42, 0, 11),
(853, 43, 0, 11),
(854, 44, 0, 11),
(855, 45, 0, 11),
(856, 46, 0, 11),
(857, 47, 0, 11),
(858, 48, 0, 11),
(859, 49, 0, 11),
(860, 50, 0, 11),
(861, 51, 0, 11),
(862, 52, 0, 11),
(863, 53, 0, 11),
(864, 54, 0, 11),
(865, 55, 0, 11),
(866, 56, 0, 11),
(867, 57, 0, 11),
(868, 58, 0, 11),
(869, 59, 0, 11),
(870, 60, 0, 11),
(871, 61, 0, 11),
(872, 1, 0, 11),
(873, 2, 0, 11),
(874, 3, 0, 11),
(875, 4, 0, 11),
(876, 5, 0, 11),
(877, 6, 0, 11),
(878, 7, 0, 11),
(879, 8, 0, 11),
(880, 9, 0, 11),
(881, 10, 0, 11),
(882, 11, 0, 11),
(883, 12, 0, 11),
(884, 13, 0, 11),
(885, 14, 0, 11),
(886, 15, 0, 11),
(887, 16, 0, 11),
(888, 17, 0, 11),
(889, 18, 0, 11),
(890, 19, 0, 11),
(891, 20, 0, 11),
(892, 21, 0, 11),
(893, 22, 0, 11),
(894, 23, 0, 11),
(895, 24, 0, 11),
(896, 25, 0, 11),
(897, 26, 0, 11),
(898, 27, 0, 11),
(899, 28, 0, 11),
(900, 29, 0, 11),
(901, 30, 0, 11),
(902, 31, 0, 11),
(903, 32, 0, 11),
(904, 33, 0, 11),
(905, 34, 0, 11),
(906, 35, 0, 11),
(907, 36, 0, 11),
(908, 37, 0, 11),
(909, 38, 0, 11),
(910, 39, 0, 11),
(911, 40, 0, 11),
(912, 41, 0, 11),
(913, 42, 0, 11),
(914, 43, 0, 11),
(915, 44, 0, 11),
(916, 45, 0, 11),
(917, 46, 0, 11),
(918, 47, 0, 11),
(919, 48, 0, 11),
(920, 49, 0, 11),
(921, 50, 0, 11),
(922, 51, 0, 11),
(923, 52, 0, 11),
(924, 53, 0, 11),
(925, 54, 0, 11),
(926, 55, 0, 11),
(927, 56, 0, 11),
(928, 57, 0, 11),
(929, 58, 0, 11),
(930, 59, 0, 11),
(931, 60, 0, 11),
(932, 61, 0, 11),
(933, 1, 0, 11),
(934, 2, 0, 11),
(935, 3, 0, 11),
(936, 4, 0, 11),
(937, 5, 0, 11),
(938, 6, 0, 11),
(939, 7, 0, 11),
(940, 8, 0, 11),
(941, 9, 0, 11),
(942, 10, 0, 11),
(943, 11, 0, 11),
(944, 12, 0, 11),
(945, 13, 0, 11),
(946, 14, 0, 11),
(947, 15, 0, 11),
(948, 16, 0, 11),
(949, 17, 0, 11),
(950, 18, 0, 11),
(951, 19, 0, 11),
(952, 20, 0, 11),
(953, 21, 0, 11),
(954, 22, 0, 11),
(955, 23, 0, 11),
(956, 24, 0, 11),
(957, 25, 0, 11),
(958, 26, 0, 11),
(959, 27, 0, 11),
(960, 28, 0, 11),
(961, 29, 0, 11),
(962, 30, 0, 11),
(963, 31, 0, 11),
(964, 32, 0, 11),
(965, 33, 0, 11),
(966, 34, 0, 11),
(967, 35, 0, 11),
(968, 36, 0, 11),
(969, 37, 0, 11),
(970, 38, 0, 11),
(971, 39, 0, 11),
(972, 40, 0, 11),
(973, 41, 0, 11),
(974, 42, 0, 11),
(975, 43, 0, 11),
(976, 44, 0, 11),
(977, 45, 0, 11),
(978, 46, 0, 11),
(979, 47, 0, 11),
(980, 48, 0, 11),
(981, 49, 0, 11),
(982, 50, 0, 11),
(983, 51, 0, 11),
(984, 52, 0, 11),
(985, 53, 0, 11),
(986, 54, 0, 11),
(987, 55, 0, 11),
(988, 56, 0, 11),
(989, 57, 0, 11),
(990, 58, 0, 11),
(991, 59, 0, 11),
(992, 60, 0, 11),
(993, 61, 0, 11),
(994, 1, 0, 11),
(995, 2, 0, 11),
(996, 3, 0, 11),
(997, 4, 0, 11),
(998, 5, 0, 11),
(999, 6, 0, 11),
(1000, 7, 0, 11),
(1001, 8, 0, 11),
(1002, 9, 0, 11),
(1003, 10, 0, 11),
(1004, 11, 0, 11),
(1005, 12, 0, 11),
(1006, 13, 0, 11),
(1007, 14, 0, 11),
(1008, 15, 0, 11),
(1009, 16, 0, 11),
(1010, 17, 0, 11),
(1011, 18, 0, 11),
(1012, 19, 0, 11),
(1013, 20, 0, 11),
(1014, 21, 0, 11),
(1015, 22, 0, 11),
(1016, 23, 0, 11),
(1017, 24, 0, 11),
(1018, 25, 0, 11),
(1019, 26, 0, 11),
(1020, 27, 0, 11),
(1021, 28, 0, 11),
(1022, 29, 0, 11),
(1023, 30, 0, 11),
(1024, 31, 0, 11),
(1025, 32, 0, 11),
(1026, 33, 0, 11),
(1027, 34, 0, 11),
(1028, 35, 0, 11),
(1029, 36, 0, 11),
(1030, 37, 0, 11),
(1031, 38, 0, 11),
(1032, 39, 0, 11),
(1033, 40, 0, 11),
(1034, 41, 0, 11),
(1035, 42, 0, 11),
(1036, 43, 0, 11),
(1037, 44, 0, 11),
(1038, 45, 0, 11),
(1039, 46, 0, 11),
(1040, 47, 0, 11),
(1041, 48, 0, 11),
(1042, 49, 0, 11),
(1043, 50, 0, 11),
(1044, 51, 0, 11),
(1045, 52, 0, 11),
(1046, 53, 0, 11),
(1047, 54, 0, 11),
(1048, 55, 0, 11),
(1049, 56, 0, 11),
(1050, 57, 0, 11),
(1051, 58, 0, 11),
(1052, 59, 0, 11),
(1053, 60, 0, 11),
(1054, 61, 0, 11),
(1055, 1, 0, 11),
(1056, 2, 0, 11),
(1057, 3, 0, 11),
(1058, 4, 0, 11),
(1059, 5, 0, 11),
(1060, 6, 0, 11),
(1061, 7, 0, 11),
(1062, 8, 0, 11),
(1063, 9, 0, 11),
(1064, 10, 0, 11),
(1065, 11, 0, 11),
(1066, 12, 0, 11),
(1067, 13, 0, 11),
(1068, 14, 0, 11),
(1069, 15, 0, 11),
(1070, 16, 0, 11),
(1071, 17, 0, 11),
(1072, 18, 0, 11),
(1073, 19, 0, 11),
(1074, 20, 0, 11),
(1075, 21, 0, 11),
(1076, 22, 0, 11),
(1077, 23, 0, 11),
(1078, 24, 0, 11),
(1079, 25, 0, 11),
(1080, 26, 0, 11),
(1081, 27, 0, 11),
(1082, 28, 0, 11),
(1083, 29, 0, 11),
(1084, 30, 0, 11),
(1085, 31, 0, 11),
(1086, 32, 0, 11),
(1087, 33, 0, 11),
(1088, 34, 0, 11),
(1089, 35, 0, 11),
(1090, 36, 0, 11),
(1091, 37, 0, 11),
(1092, 38, 0, 11),
(1093, 39, 0, 11),
(1094, 40, 0, 11),
(1095, 41, 0, 11),
(1096, 42, 0, 11),
(1097, 43, 0, 11),
(1098, 44, 0, 11),
(1099, 45, 0, 11),
(1100, 46, 0, 11),
(1101, 47, 0, 11),
(1102, 48, 0, 11),
(1103, 49, 0, 11),
(1104, 50, 0, 11),
(1105, 51, 0, 11),
(1106, 52, 0, 11),
(1107, 53, 0, 11),
(1108, 54, 0, 11),
(1109, 55, 0, 11),
(1110, 56, 0, 11),
(1111, 57, 0, 11),
(1112, 58, 0, 11),
(1113, 59, 0, 11),
(1114, 60, 0, 11),
(1115, 61, 0, 11),
(1116, 1, 0, 11),
(1117, 2, 0, 11),
(1118, 3, 0, 11),
(1119, 4, 0, 11),
(1120, 5, 0, 11),
(1121, 6, 0, 11),
(1122, 7, 0, 11),
(1123, 8, 0, 11),
(1124, 9, 0, 11),
(1125, 10, 0, 11),
(1126, 11, 0, 11),
(1127, 12, 0, 11),
(1128, 13, 0, 11),
(1129, 14, 0, 11),
(1130, 15, 0, 11),
(1131, 16, 0, 11),
(1132, 17, 0, 11),
(1133, 18, 0, 11),
(1134, 19, 0, 11),
(1135, 20, 0, 11),
(1136, 21, 0, 11),
(1137, 22, 0, 11),
(1138, 23, 0, 11),
(1139, 24, 0, 11),
(1140, 25, 0, 11),
(1141, 26, 0, 11),
(1142, 27, 0, 11),
(1143, 28, 0, 11),
(1144, 29, 0, 11),
(1145, 30, 0, 11),
(1146, 31, 0, 11),
(1147, 32, 0, 11),
(1148, 33, 0, 11),
(1149, 34, 0, 11),
(1150, 35, 0, 11),
(1151, 36, 0, 11),
(1152, 37, 0, 11),
(1153, 38, 0, 11),
(1154, 39, 0, 11),
(1155, 40, 0, 11),
(1156, 41, 0, 11),
(1157, 42, 0, 11),
(1158, 43, 0, 11),
(1159, 44, 0, 11),
(1160, 45, 0, 11),
(1161, 46, 0, 11),
(1162, 47, 0, 11),
(1163, 48, 0, 11),
(1164, 49, 0, 11),
(1165, 50, 0, 11),
(1166, 51, 0, 11),
(1167, 52, 0, 11),
(1168, 53, 0, 11),
(1169, 54, 0, 11),
(1170, 55, 0, 11),
(1171, 56, 0, 11),
(1172, 57, 0, 11),
(1173, 58, 0, 11),
(1174, 59, 0, 11),
(1175, 60, 0, 11),
(1176, 61, 0, 11),
(1177, 1, 0, 11),
(1178, 2, 0, 11),
(1179, 3, 0, 11),
(1180, 4, 0, 11),
(1181, 5, 0, 11),
(1182, 6, 0, 11),
(1183, 7, 0, 11),
(1184, 8, 0, 11),
(1185, 9, 0, 11),
(1186, 10, 0, 11),
(1187, 11, 0, 11),
(1188, 12, 0, 11),
(1189, 13, 0, 11),
(1190, 14, 0, 11),
(1191, 15, 0, 11),
(1192, 16, 0, 11),
(1193, 17, 0, 11),
(1194, 18, 0, 11),
(1195, 19, 0, 11),
(1196, 20, 0, 11),
(1197, 21, 0, 11),
(1198, 22, 0, 11),
(1199, 23, 0, 11),
(1200, 24, 0, 11),
(1201, 25, 0, 11),
(1202, 26, 0, 11),
(1203, 27, 0, 11),
(1204, 28, 0, 11),
(1205, 29, 0, 11),
(1206, 30, 0, 11),
(1207, 31, 0, 11),
(1208, 32, 0, 11),
(1209, 33, 0, 11),
(1210, 34, 0, 11),
(1211, 35, 0, 11),
(1212, 36, 0, 11),
(1213, 37, 0, 11),
(1214, 38, 0, 11),
(1215, 39, 0, 11),
(1216, 40, 0, 11),
(1217, 41, 0, 11),
(1218, 42, 0, 11),
(1219, 43, 0, 11),
(1220, 44, 0, 11),
(1221, 45, 0, 11),
(1222, 46, 0, 11),
(1223, 47, 0, 11),
(1224, 48, 0, 11),
(1225, 49, 0, 11),
(1226, 50, 0, 11),
(1227, 51, 0, 11),
(1228, 52, 0, 11),
(1229, 53, 0, 11),
(1230, 54, 0, 11),
(1231, 55, 0, 11),
(1232, 56, 0, 11),
(1233, 57, 0, 11),
(1234, 58, 0, 11),
(1235, 59, 0, 11),
(1236, 60, 0, 11),
(1237, 61, 0, 11),
(1238, 1, 0, 11),
(1239, 2, 0, 11),
(1240, 3, 0, 11),
(1241, 4, 0, 11),
(1242, 5, 0, 11),
(1243, 6, 0, 11),
(1244, 7, 0, 11),
(1245, 8, 0, 11),
(1246, 9, 0, 11),
(1247, 10, 0, 11),
(1248, 11, 0, 11),
(1249, 12, 0, 11),
(1250, 13, 0, 11),
(1251, 14, 0, 11),
(1252, 15, 0, 11),
(1253, 16, 0, 11),
(1254, 17, 0, 11),
(1255, 18, 0, 11),
(1256, 19, 0, 11),
(1257, 20, 0, 11),
(1258, 21, 0, 11),
(1259, 22, 0, 11),
(1260, 23, 0, 11),
(1261, 24, 0, 11),
(1262, 25, 0, 11),
(1263, 26, 0, 11),
(1264, 27, 0, 11),
(1265, 28, 0, 11),
(1266, 29, 0, 11),
(1267, 30, 0, 11),
(1268, 31, 0, 11),
(1269, 32, 0, 11),
(1270, 33, 0, 11),
(1271, 34, 0, 11),
(1272, 35, 0, 11),
(1273, 36, 0, 11),
(1274, 37, 0, 11),
(1275, 38, 0, 11),
(1276, 39, 0, 11),
(1277, 40, 0, 11),
(1278, 41, 0, 11),
(1279, 42, 0, 11),
(1280, 43, 0, 11),
(1281, 44, 0, 11),
(1282, 45, 0, 11),
(1283, 46, 0, 11),
(1284, 47, 0, 11),
(1285, 48, 0, 11),
(1286, 49, 0, 11),
(1287, 50, 0, 11),
(1288, 51, 0, 11),
(1289, 52, 0, 11),
(1290, 53, 0, 11),
(1291, 54, 0, 11),
(1292, 55, 0, 11),
(1293, 56, 0, 11),
(1294, 57, 0, 11),
(1295, 58, 0, 11),
(1296, 59, 0, 11),
(1297, 60, 0, 11),
(1298, 61, 0, 11),
(1299, 1, 0, 11),
(1300, 2, 0, 11),
(1301, 3, 0, 11),
(1302, 4, 0, 11),
(1303, 5, 0, 11),
(1304, 6, 0, 11),
(1305, 7, 0, 11),
(1306, 8, 0, 11),
(1307, 9, 0, 11),
(1308, 10, 0, 11),
(1309, 11, 0, 11),
(1310, 12, 0, 11),
(1311, 13, 0, 11),
(1312, 14, 0, 11),
(1313, 15, 0, 11),
(1314, 16, 0, 11),
(1315, 17, 0, 11),
(1316, 18, 0, 11),
(1317, 19, 0, 11),
(1318, 20, 0, 11),
(1319, 21, 0, 11),
(1320, 22, 0, 11),
(1321, 23, 0, 11),
(1322, 24, 0, 11),
(1323, 25, 0, 11),
(1324, 26, 0, 11),
(1325, 27, 0, 11),
(1326, 28, 0, 11),
(1327, 29, 0, 11),
(1328, 30, 0, 11),
(1329, 31, 0, 11),
(1330, 32, 0, 11),
(1331, 33, 0, 11),
(1332, 34, 0, 11),
(1333, 35, 0, 11),
(1334, 36, 0, 11),
(1335, 37, 0, 11),
(1336, 38, 0, 11),
(1337, 39, 0, 11),
(1338, 40, 0, 11),
(1339, 41, 0, 11),
(1340, 42, 0, 11),
(1341, 43, 0, 11),
(1342, 44, 0, 11),
(1343, 45, 0, 11),
(1344, 46, 0, 11),
(1345, 47, 0, 11),
(1346, 48, 0, 11),
(1347, 49, 0, 11),
(1348, 50, 0, 11),
(1349, 51, 0, 11),
(1350, 52, 0, 11),
(1351, 53, 0, 11),
(1352, 54, 0, 11),
(1353, 55, 0, 11),
(1354, 56, 0, 11),
(1355, 57, 0, 11),
(1356, 58, 0, 11),
(1357, 59, 0, 11),
(1358, 60, 0, 11),
(1359, 61, 0, 11),
(1360, 1, 0, 11),
(1361, 2, 0, 11),
(1362, 3, 0, 11),
(1363, 4, 0, 11),
(1364, 5, 0, 11),
(1365, 6, 0, 11),
(1366, 7, 0, 11),
(1367, 8, 0, 11),
(1368, 9, 0, 11),
(1369, 10, 0, 11),
(1370, 11, 0, 11),
(1371, 12, 0, 11),
(1372, 13, 0, 11),
(1373, 14, 0, 11),
(1374, 15, 0, 11),
(1375, 16, 0, 11),
(1376, 17, 0, 11),
(1377, 18, 0, 11),
(1378, 19, 0, 11),
(1379, 20, 0, 11),
(1380, 21, 0, 11),
(1381, 22, 0, 11),
(1382, 23, 0, 11),
(1383, 24, 0, 11),
(1384, 25, 0, 11),
(1385, 26, 0, 11),
(1386, 27, 0, 11),
(1387, 28, 0, 11),
(1388, 29, 0, 11),
(1389, 30, 0, 11),
(1390, 31, 0, 11),
(1391, 32, 0, 11),
(1392, 33, 0, 11),
(1393, 34, 0, 11),
(1394, 35, 0, 11),
(1395, 36, 0, 11),
(1396, 37, 0, 11),
(1397, 38, 0, 11),
(1398, 39, 0, 11),
(1399, 40, 0, 11),
(1400, 41, 0, 11),
(1401, 42, 0, 11),
(1402, 43, 0, 11),
(1403, 44, 0, 11),
(1404, 45, 0, 11),
(1405, 46, 0, 11),
(1406, 47, 0, 11),
(1407, 48, 0, 11),
(1408, 49, 0, 11),
(1409, 50, 0, 11),
(1410, 51, 0, 11),
(1411, 52, 0, 11),
(1412, 53, 0, 11),
(1413, 54, 0, 11),
(1414, 55, 0, 11),
(1415, 56, 0, 11),
(1416, 57, 0, 11),
(1417, 58, 0, 11),
(1418, 59, 0, 11),
(1419, 60, 0, 11),
(1420, 61, 0, 11),
(1421, 1, 0, 11),
(1422, 2, 0, 11),
(1423, 3, 0, 11),
(1424, 4, 0, 11),
(1425, 5, 0, 11),
(1426, 6, 0, 11),
(1427, 7, 0, 11),
(1428, 8, 0, 11),
(1429, 9, 0, 11),
(1430, 10, 0, 11),
(1431, 11, 0, 11),
(1432, 12, 0, 11),
(1433, 13, 0, 11),
(1434, 14, 0, 11),
(1435, 15, 0, 11),
(1436, 16, 0, 11),
(1437, 17, 0, 11),
(1438, 18, 0, 11),
(1439, 19, 0, 11),
(1440, 20, 0, 11),
(1441, 21, 0, 11),
(1442, 22, 0, 11),
(1443, 23, 0, 11),
(1444, 24, 0, 11),
(1445, 25, 0, 11),
(1446, 26, 0, 11),
(1447, 27, 0, 11),
(1448, 28, 0, 11),
(1449, 29, 0, 11),
(1450, 30, 0, 11),
(1451, 31, 0, 11),
(1452, 32, 0, 11),
(1453, 33, 0, 11),
(1454, 34, 0, 11),
(1455, 35, 0, 11),
(1456, 36, 0, 11),
(1457, 37, 0, 11),
(1458, 38, 0, 11),
(1459, 39, 0, 11),
(1460, 40, 0, 11),
(1461, 41, 0, 11),
(1462, 42, 0, 11),
(1463, 43, 0, 11),
(1464, 44, 0, 11),
(1465, 45, 0, 11),
(1466, 46, 0, 11),
(1467, 47, 0, 11),
(1468, 48, 0, 11),
(1469, 49, 0, 11),
(1470, 50, 0, 11),
(1471, 51, 0, 11),
(1472, 52, 0, 11),
(1473, 53, 0, 11),
(1474, 54, 0, 11),
(1475, 55, 0, 11),
(1476, 56, 0, 11),
(1477, 57, 0, 11),
(1478, 58, 0, 11),
(1479, 59, 0, 11),
(1480, 60, 0, 11),
(1481, 61, 0, 11),
(1482, 1, 0, 11),
(1483, 2, 0, 11),
(1484, 3, 0, 11),
(1485, 4, 0, 11),
(1486, 5, 0, 11),
(1487, 6, 0, 11),
(1488, 7, 0, 11),
(1489, 8, 0, 11),
(1490, 9, 0, 11),
(1491, 10, 0, 11),
(1492, 11, 0, 11),
(1493, 12, 0, 11),
(1494, 13, 0, 11),
(1495, 14, 0, 11),
(1496, 15, 0, 11),
(1497, 16, 0, 11),
(1498, 17, 0, 11),
(1499, 18, 0, 11),
(1500, 19, 0, 11),
(1501, 20, 0, 11),
(1502, 21, 0, 11),
(1503, 22, 0, 11),
(1504, 23, 0, 11),
(1505, 24, 0, 11),
(1506, 25, 0, 11),
(1507, 26, 0, 11),
(1508, 27, 0, 11),
(1509, 28, 0, 11),
(1510, 29, 0, 11),
(1511, 30, 0, 11),
(1512, 31, 0, 11),
(1513, 32, 0, 11),
(1514, 33, 0, 11),
(1515, 34, 0, 11),
(1516, 35, 0, 11),
(1517, 36, 0, 11),
(1518, 37, 0, 11),
(1519, 38, 0, 11),
(1520, 39, 0, 11),
(1521, 40, 0, 11),
(1522, 41, 0, 11),
(1523, 42, 0, 11),
(1524, 43, 0, 11),
(1525, 44, 0, 11),
(1526, 45, 0, 11),
(1527, 46, 0, 11),
(1528, 47, 0, 11),
(1529, 48, 0, 11),
(1530, 49, 0, 11),
(1531, 50, 0, 11),
(1532, 51, 0, 11),
(1533, 52, 0, 11),
(1534, 53, 0, 11),
(1535, 54, 0, 11),
(1536, 55, 0, 11),
(1537, 56, 0, 11),
(1538, 57, 0, 11),
(1539, 58, 0, 11),
(1540, 59, 0, 11),
(1541, 60, 0, 11),
(1542, 61, 0, 11),
(1543, 1, 0, 11),
(1544, 2, 0, 11),
(1545, 3, 0, 11),
(1546, 4, 0, 11),
(1547, 5, 0, 11),
(1548, 6, 0, 11),
(1549, 7, 0, 11),
(1550, 8, 0, 11),
(1551, 9, 0, 11),
(1552, 10, 0, 11),
(1553, 11, 0, 11),
(1554, 12, 0, 11),
(1555, 13, 0, 11),
(1556, 14, 0, 11),
(1557, 15, 0, 11),
(1558, 16, 0, 11),
(1559, 17, 0, 11),
(1560, 18, 0, 11),
(1561, 19, 0, 11),
(1562, 20, 0, 11),
(1563, 21, 0, 11),
(1564, 22, 0, 11),
(1565, 23, 0, 11),
(1566, 24, 0, 11),
(1567, 25, 0, 11),
(1568, 26, 0, 11),
(1569, 27, 0, 11),
(1570, 28, 0, 11),
(1571, 29, 0, 11),
(1572, 30, 0, 11),
(1573, 31, 0, 11),
(1574, 32, 0, 11),
(1575, 33, 0, 11),
(1576, 34, 0, 11),
(1577, 35, 0, 11),
(1578, 36, 0, 11),
(1579, 37, 0, 11),
(1580, 38, 0, 11),
(1581, 39, 0, 11),
(1582, 40, 0, 11),
(1583, 41, 0, 11),
(1584, 42, 0, 11),
(1585, 43, 0, 11),
(1586, 44, 0, 11),
(1587, 45, 0, 11),
(1588, 46, 0, 11),
(1589, 47, 0, 11),
(1590, 48, 0, 11),
(1591, 49, 0, 11),
(1592, 50, 0, 11),
(1593, 51, 0, 11),
(1594, 52, 0, 11),
(1595, 53, 0, 11),
(1596, 54, 0, 11),
(1597, 55, 0, 11),
(1598, 56, 0, 11),
(1599, 57, 0, 11),
(1600, 58, 0, 11),
(1601, 59, 0, 11),
(1602, 60, 0, 11),
(1603, 61, 0, 11),
(1604, 1, 0, 11),
(1605, 2, 0, 11),
(1606, 3, 0, 11),
(1607, 4, 0, 11),
(1608, 5, 0, 11),
(1609, 6, 0, 11),
(1610, 7, 0, 11),
(1611, 8, 0, 11),
(1612, 9, 0, 11),
(1613, 10, 0, 11),
(1614, 11, 0, 11),
(1615, 12, 0, 11),
(1616, 13, 0, 11),
(1617, 14, 0, 11),
(1618, 15, 0, 11),
(1619, 16, 0, 11),
(1620, 17, 0, 11),
(1621, 18, 0, 11),
(1622, 19, 0, 11),
(1623, 20, 0, 11),
(1624, 21, 0, 11),
(1625, 22, 0, 11),
(1626, 23, 0, 11),
(1627, 24, 0, 11),
(1628, 25, 0, 11),
(1629, 26, 0, 11),
(1630, 27, 0, 11),
(1631, 28, 0, 11),
(1632, 29, 0, 11),
(1633, 30, 0, 11),
(1634, 31, 0, 11),
(1635, 32, 0, 11),
(1636, 33, 0, 11),
(1637, 34, 0, 11),
(1638, 35, 0, 11),
(1639, 36, 0, 11),
(1640, 37, 0, 11),
(1641, 38, 0, 11),
(1642, 39, 0, 11),
(1643, 40, 0, 11),
(1644, 41, 0, 11),
(1645, 42, 0, 11),
(1646, 43, 0, 11),
(1647, 44, 0, 11),
(1648, 45, 0, 11),
(1649, 46, 0, 11),
(1650, 47, 0, 11),
(1651, 48, 0, 11),
(1652, 49, 0, 11),
(1653, 50, 0, 11),
(1654, 51, 0, 11),
(1655, 52, 0, 11),
(1656, 53, 0, 11),
(1657, 54, 0, 11),
(1658, 55, 0, 11),
(1659, 56, 0, 11),
(1660, 57, 0, 11),
(1661, 58, 0, 11),
(1662, 59, 0, 11),
(1663, 60, 0, 11),
(1664, 61, 0, 11),
(1665, 1, 0, 11),
(1666, 2, 0, 11),
(1667, 3, 0, 11),
(1668, 4, 0, 11),
(1669, 5, 0, 11),
(1670, 6, 0, 11),
(1671, 7, 0, 11),
(1672, 8, 0, 11),
(1673, 9, 0, 11),
(1674, 10, 0, 11),
(1675, 11, 0, 11),
(1676, 12, 0, 11),
(1677, 13, 0, 11),
(1678, 14, 0, 11),
(1679, 15, 0, 11),
(1680, 16, 0, 11),
(1681, 17, 0, 11),
(1682, 18, 0, 11),
(1683, 19, 0, 11),
(1684, 20, 0, 11),
(1685, 21, 0, 11),
(1686, 22, 0, 11),
(1687, 23, 0, 11),
(1688, 24, 0, 11),
(1689, 25, 0, 11),
(1690, 26, 0, 11),
(1691, 27, 0, 11),
(1692, 28, 0, 11),
(1693, 29, 0, 11),
(1694, 30, 0, 11),
(1695, 31, 0, 11),
(1696, 32, 0, 11),
(1697, 33, 0, 11),
(1698, 34, 0, 11),
(1699, 35, 0, 11),
(1700, 36, 0, 11),
(1701, 37, 0, 11),
(1702, 38, 0, 11),
(1703, 39, 0, 11),
(1704, 40, 0, 11),
(1705, 41, 0, 11),
(1706, 42, 0, 11),
(1707, 43, 0, 11),
(1708, 44, 0, 11),
(1709, 45, 0, 11),
(1710, 46, 0, 11),
(1711, 47, 0, 11),
(1712, 48, 0, 11),
(1713, 49, 0, 11),
(1714, 50, 0, 11),
(1715, 51, 0, 11),
(1716, 52, 0, 11),
(1717, 53, 0, 11),
(1718, 54, 0, 11),
(1719, 55, 0, 11),
(1720, 56, 0, 11),
(1721, 57, 0, 11),
(1722, 58, 0, 11),
(1723, 59, 0, 11),
(1724, 60, 0, 11),
(1725, 61, 0, 11);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_deroulement_innovateur`
--

DROP TABLE IF EXISTS `28_2024_deroulement_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_deroulement_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_attribution_deroulement` int NOT NULL,
  `status` tinyint(1) NOT NULL,
  `id_projet` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=315 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_deroulement_innovateur`
--

INSERT INTO `28_2024_deroulement_innovateur` (`id`, `id_attribution_deroulement`, `status`, `id_projet`) VALUES
(1, 1, 0, 7),
(2, 2, 0, 7),
(3, 3, 0, 7),
(4, 4, 0, 7),
(5, 5, 0, 7),
(6, 6, 0, 7),
(7, 7, 0, 7),
(8, 8, 0, 7),
(9, 9, 0, 7),
(10, 10, 0, 7),
(11, 11, 0, 7),
(12, 12, 0, 7),
(13, 13, 0, 7),
(14, 14, 0, 7),
(15, 15, 0, 7),
(16, 16, 0, 7),
(21, 21, 0, 7),
(22, 22, 0, 7),
(23, 23, 0, 7),
(24, 24, 0, 7),
(25, 25, 0, 7),
(26, 26, 0, 7),
(27, 27, 0, 7),
(28, 28, 0, 7),
(29, 29, 0, 7),
(30, 30, 0, 7),
(31, 31, 0, 7),
(32, 32, 0, 7),
(33, 33, 0, 7),
(34, 34, 0, 7),
(35, 35, 0, 7),
(36, 36, 0, 7),
(37, 37, 0, 7),
(38, 38, 0, 7),
(39, 39, 0, 7),
(40, 40, 0, 7),
(41, 41, 0, 7),
(42, 42, 0, 7),
(43, 43, 0, 7),
(44, 44, 0, 7),
(45, 45, 0, 7),
(46, 46, 0, 7),
(47, 47, 0, 7),
(48, 48, 0, 7),
(49, 49, 0, 7),
(50, 50, 0, 7),
(51, 51, 0, 7),
(52, 52, 0, 7),
(53, 53, 0, 7),
(54, 54, 0, 7),
(55, 55, 0, 7),
(56, 56, 0, 7),
(57, 57, 0, 7),
(58, 58, 0, 7),
(59, 59, 0, 7),
(60, 60, 0, 7),
(61, 61, 0, 7),
(62, 62, 0, 7),
(63, 63, 0, 7),
(64, 64, 0, 7),
(65, 65, 0, 7),
(66, 66, 0, 7),
(67, 67, 0, 7),
(68, 68, 0, 7),
(69, 69, 0, 7),
(70, 70, 0, 7),
(71, 71, 0, 7),
(72, 72, 0, 7),
(73, 1, 1, 7),
(74, 2, 1, 7),
(75, 3, 1, 7),
(76, 4, 1, 7),
(77, 5, 1, 7),
(78, 6, 1, 7),
(79, 7, 1, 7),
(80, 8, 1, 7),
(81, 9, 1, 7),
(82, 10, 1, 7),
(83, 11, 1, 7),
(84, 12, 1, 7),
(85, 13, 1, 7),
(86, 14, 0, 7),
(87, 15, 0, 7),
(88, 16, 0, 7),
(89, 17, 1, 7),
(90, 18, 1, 7),
(91, 19, 1, 7),
(92, 20, 1, 7),
(93, 1, 1, 7),
(94, 2, 1, 7),
(95, 3, 1, 7),
(96, 1, 1, 7),
(97, 2, 1, 7),
(98, 3, 1, 7),
(106, 8, 0, 12),
(107, 9, 0, 12),
(108, 10, 0, 12),
(109, 11, 0, 12),
(110, 12, 0, 12),
(111, 13, 0, 12),
(112, 14, 0, 12),
(113, 15, 0, 12),
(114, 16, 0, 12),
(115, 17, 0, 12),
(116, 18, 0, 12),
(117, 19, 0, 12),
(118, 20, 0, 12),
(119, 21, 0, 12),
(120, 22, 0, 12),
(121, 23, 0, 12),
(122, 24, 0, 12),
(123, 25, 0, 12),
(124, 26, 0, 12),
(125, 27, 0, 12),
(126, 28, 0, 12),
(127, 29, 0, 12),
(128, 30, 0, 12),
(129, 31, 0, 12),
(130, 32, 0, 12),
(131, 33, 0, 12),
(132, 34, 0, 12),
(133, 35, 0, 12),
(134, 36, 0, 12),
(135, 37, 0, 12),
(136, 38, 0, 12),
(137, 39, 0, 12),
(138, 40, 0, 12),
(139, 41, 0, 12),
(140, 42, 0, 12),
(141, 43, 0, 12),
(142, 44, 0, 12),
(143, 45, 0, 12),
(144, 46, 0, 12),
(145, 47, 0, 12),
(146, 48, 0, 12),
(147, 49, 0, 12),
(148, 50, 0, 12),
(149, 51, 0, 12),
(150, 52, 0, 12),
(151, 53, 0, 12),
(152, 54, 0, 12),
(153, 55, 0, 12),
(154, 56, 0, 12),
(155, 57, 0, 12),
(156, 58, 0, 12),
(157, 59, 0, 12),
(158, 60, 0, 12),
(159, 61, 0, 12),
(160, 62, 0, 12),
(161, 63, 0, 12),
(162, 64, 0, 12),
(163, 65, 0, 12),
(164, 66, 0, 12),
(165, 67, 0, 12),
(166, 68, 0, 12),
(167, 69, 0, 12),
(168, 70, 0, 12),
(169, 71, 0, 12),
(170, 72, 0, 12),
(178, 8, 0, 13),
(179, 9, 0, 13),
(180, 10, 0, 13),
(181, 11, 0, 13),
(182, 12, 0, 13),
(183, 13, 0, 13),
(184, 14, 0, 13),
(185, 15, 0, 13),
(186, 16, 0, 13),
(187, 17, 0, 13),
(188, 18, 0, 13),
(189, 19, 0, 13),
(190, 20, 0, 13),
(191, 21, 0, 13),
(192, 22, 0, 13),
(193, 23, 0, 13),
(194, 24, 0, 13),
(195, 25, 0, 13),
(196, 26, 0, 13),
(197, 27, 0, 13),
(198, 28, 0, 13),
(199, 29, 0, 13),
(200, 30, 0, 13),
(201, 31, 0, 13),
(202, 32, 0, 13),
(203, 33, 0, 13),
(204, 34, 0, 13),
(205, 35, 0, 13),
(206, 36, 0, 13),
(207, 37, 0, 13),
(208, 38, 0, 13),
(209, 39, 0, 13),
(210, 40, 0, 13),
(211, 41, 0, 13),
(212, 42, 0, 13),
(213, 43, 0, 13),
(214, 44, 0, 13),
(215, 45, 0, 13),
(216, 46, 0, 13),
(217, 47, 0, 13),
(218, 48, 0, 13),
(219, 49, 0, 13),
(220, 50, 0, 13),
(221, 51, 0, 13),
(222, 52, 0, 13),
(223, 53, 0, 13),
(224, 54, 0, 13),
(225, 55, 0, 13),
(226, 56, 0, 13),
(227, 57, 0, 13),
(228, 58, 0, 13),
(229, 59, 0, 13),
(230, 60, 0, 13),
(231, 61, 0, 13),
(232, 62, 0, 13),
(233, 63, 0, 13),
(234, 64, 0, 13),
(235, 65, 0, 13),
(236, 66, 0, 13),
(237, 67, 0, 13),
(238, 68, 0, 13),
(239, 69, 0, 13),
(240, 70, 0, 13),
(241, 71, 0, 13),
(242, 72, 0, 13),
(243, 1, 0, 14),
(244, 2, 0, 14),
(245, 3, 0, 14),
(246, 4, 0, 14),
(247, 5, 0, 14),
(248, 6, 0, 14),
(249, 7, 0, 14),
(250, 8, 0, 14),
(251, 9, 0, 14),
(252, 10, 0, 14),
(253, 11, 0, 14),
(254, 12, 0, 14),
(255, 13, 0, 14),
(256, 14, 0, 14),
(257, 15, 0, 14),
(258, 16, 0, 14),
(259, 17, 0, 14),
(260, 18, 0, 14),
(261, 19, 0, 14),
(262, 20, 0, 14),
(263, 21, 0, 14),
(264, 22, 0, 14),
(265, 23, 0, 14),
(266, 24, 0, 14),
(267, 25, 0, 14),
(268, 26, 0, 14),
(269, 27, 0, 14),
(270, 28, 0, 14),
(271, 29, 0, 14),
(272, 30, 0, 14),
(273, 31, 0, 14),
(274, 32, 0, 14),
(275, 33, 0, 14),
(276, 34, 0, 14),
(277, 35, 0, 14),
(278, 36, 0, 14),
(279, 37, 0, 14),
(280, 38, 0, 14),
(281, 39, 0, 14),
(282, 40, 0, 14),
(283, 41, 0, 14),
(284, 42, 0, 14),
(285, 43, 0, 14),
(286, 44, 0, 14),
(287, 45, 0, 14),
(288, 46, 0, 14),
(289, 47, 0, 14),
(290, 48, 0, 14),
(291, 49, 0, 14),
(292, 50, 0, 14),
(293, 51, 0, 14),
(294, 52, 0, 14),
(295, 53, 0, 14),
(296, 54, 0, 14),
(297, 55, 0, 14),
(298, 56, 0, 14),
(299, 57, 0, 14),
(300, 58, 0, 14),
(301, 59, 0, 14),
(302, 60, 0, 14),
(303, 61, 0, 14),
(304, 62, 0, 14),
(305, 63, 0, 14),
(306, 64, 0, 14),
(307, 65, 0, 14),
(308, 66, 0, 14),
(309, 67, 0, 14),
(310, 68, 0, 14),
(311, 69, 0, 14),
(312, 70, 0, 14),
(313, 71, 0, 14),
(314, 72, 0, 14);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_deroulement_promoteur`
--

DROP TABLE IF EXISTS `28_2024_deroulement_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_deroulement_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_attribution_deroulement` int NOT NULL,
  `id_projet` int NOT NULL,
  `status` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=194 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_deroulement_promoteur`
--

INSERT INTO `28_2024_deroulement_promoteur` (`id`, `id_attribution_deroulement`, `id_projet`, `status`) VALUES
(57, 9, 7, 0),
(58, 10, 7, 0),
(59, 11, 7, 0),
(60, 12, 7, 0),
(61, 13, 7, 0),
(62, 14, 7, 0),
(63, 15, 7, 0),
(64, 16, 7, 0),
(65, 17, 7, 0),
(66, 18, 7, 0),
(67, 19, 7, 0),
(68, 20, 7, 0),
(69, 21, 7, 0),
(70, 22, 7, 0),
(71, 23, 7, 0),
(72, 24, 7, 0),
(73, 1, 7, 1),
(74, 2, 7, 1),
(75, 3, 7, 1),
(76, 1, 12, 1),
(77, 2, 12, 1),
(78, 3, 12, 1),
(79, 4, 12, 1),
(80, 5, 12, 1),
(81, 6, 12, 1),
(82, 7, 12, 1),
(83, 1, 12, 0),
(84, 2, 12, 0),
(85, 3, 12, 0),
(86, 4, 12, 0),
(87, 5, 12, 0),
(88, 6, 12, 0),
(89, 7, 12, 0),
(90, 8, 12, 0),
(91, 9, 12, 0),
(92, 10, 12, 0),
(93, 11, 12, 0),
(94, 12, 12, 0),
(95, 13, 12, 0),
(96, 14, 12, 0),
(97, 15, 12, 0),
(98, 16, 12, 0),
(99, 17, 12, 0),
(100, 18, 12, 0),
(101, 19, 12, 0),
(102, 20, 12, 0),
(103, 21, 12, 0),
(104, 22, 12, 0),
(105, 23, 12, 0),
(106, 24, 12, 0),
(138, 4, 7, 1),
(139, 5, 7, 1),
(143, 6, 7, 1),
(144, 7, 7, 1),
(145, 8, 7, 1),
(146, 1, 13, 0),
(147, 2, 13, 0),
(148, 3, 13, 0),
(149, 4, 13, 0),
(150, 5, 13, 0),
(151, 6, 13, 0),
(152, 7, 13, 0),
(153, 8, 13, 0),
(154, 9, 13, 0),
(155, 10, 13, 0),
(156, 11, 13, 0),
(157, 12, 13, 0),
(158, 13, 13, 0),
(159, 14, 13, 0),
(160, 15, 13, 0),
(161, 16, 13, 0),
(162, 17, 13, 0),
(163, 18, 13, 0),
(164, 19, 13, 0),
(165, 20, 13, 0),
(166, 21, 13, 0),
(167, 22, 13, 0),
(168, 23, 13, 0),
(169, 24, 13, 0),
(170, 1, 14, 0),
(171, 2, 14, 0),
(172, 3, 14, 0),
(173, 4, 14, 0),
(174, 5, 14, 0),
(175, 6, 14, 0),
(176, 7, 14, 0),
(177, 8, 14, 0),
(178, 9, 14, 0),
(179, 10, 14, 0),
(180, 11, 14, 0),
(181, 12, 14, 0),
(182, 13, 14, 0),
(183, 14, 14, 0),
(184, 15, 14, 0),
(185, 16, 14, 0),
(186, 17, 14, 0),
(187, 18, 14, 0),
(188, 19, 14, 0),
(189, 20, 14, 0),
(190, 21, 14, 0),
(191, 22, 14, 0),
(192, 23, 14, 0),
(193, 24, 14, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_domaine`
--

DROP TABLE IF EXISTS `28_2024_domaine`;
CREATE TABLE IF NOT EXISTS `28_2024_domaine` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_domaine`
--

INSERT INTO `28_2024_domaine` (`id`, `nom`) VALUES
(1, 'Agriculture'),
(2, 'Agroalimentaire'),
(3, 'Artisanat'),
(4, 'Biotechnologie'),
(5, 'Culture'),
(6, 'Digital (TIC)'),
(7, 'Education'),
(8, 'Energie'),
(9, 'Média'),
(10, 'Nouvelles Technologies'),
(11, 'Santé'),
(12, 'Société'),
(13, 'Tourisme'),
(14, 'Transport');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_echanges`
--

DROP TABLE IF EXISTS `28_2024_echanges`;
CREATE TABLE IF NOT EXISTS `28_2024_echanges` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `seance_id` int NOT NULL,
  `comment` text NOT NULL,
  `rapport` varchar(255) DEFAULT NULL,
  `created_at` date NOT NULL,
  `sender` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_echanges`
--

INSERT INTO `28_2024_echanges` (`id`, `etudiant_id`, `enseignant_id`, `seance_id`, `comment`, `rapport`, `created_at`, `sender`) VALUES
(9, NULL, 2, 1, 'Hello guys i am pleased to hear from you in our first  seance               ', '', '2024-10-30', 4),
(10, 1, NULL, 1, 'hello here is my first business model                ', 'fiche_accompagnementaal_rom_seance_2.pdf', '2024-10-30', 1),
(11, 4, NULL, 1, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.', '', '2024-10-30', 6),
(12, 1, NULL, 1, '                hghfgvhg', '', '2024-10-31', 1),
(13, 9, NULL, 39, '  hello for our first sessioj              ', '', '2024-11-07', 14),
(14, NULL, 5, 75, '        Bonjour C\'est La SEnace Numero 1 ', 'fiche_accompagnementetudiant_detest_seance_1 (4).pdf', '2024-11-08', 13),
(15, 13, NULL, 75, 'Bonjour merci                ', '', '2024-11-08', 18);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_echanges_innovateur`
--

DROP TABLE IF EXISTS `28_2024_echanges_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_echanges_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `seance_id` int NOT NULL,
  `comment` text NOT NULL,
  `rapport` varchar(255) DEFAULT NULL,
  `created_at` date NOT NULL,
  `sender` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_echanges_innovateur`
--

INSERT INTO `28_2024_echanges_innovateur` (`id`, `etudiant_id`, `enseignant_id`, `seance_id`, `comment`, `rapport`, `created_at`, `sender`) VALUES
(1, NULL, 2, 1, ' Bonjour C est la seance 1 dans le phase 2 vous pouvez me dire si vous avez des questions               ', '', '2024-11-04', 4),
(2, 1, NULL, 1, 'Merci :) :)                ', '', '2024-11-04', 1),
(3, 1, NULL, 2, 'hi we have a new seance                ', 'fiche_suivie_aal_rom (8).pdf', '2024-11-05', 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_echanges_promoteur`
--

DROP TABLE IF EXISTS `28_2024_echanges_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_echanges_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `seance_id` int NOT NULL,
  `comment` text NOT NULL,
  `rapport` varchar(255) DEFAULT NULL,
  `created_at` date NOT NULL,
  `sender` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_echanges_promoteur`
--

INSERT INTO `28_2024_echanges_promoteur` (`id`, `etudiant_id`, `enseignant_id`, `seance_id`, `comment`, `rapport`, `created_at`, `sender`) VALUES
(1, 1, NULL, 17, ' hello welcome to the first seance of the 3rd phase promoteur               ', '', '2024-11-05', 1),
(2, NULL, 2, 17, '   welcome i am ala             ', '', '2024-11-05', 4);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_enseignant`
--

DROP TABLE IF EXISTS `28_2024_enseignant`;
CREATE TABLE IF NOT EXISTS `28_2024_enseignant` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `prenom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `email` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `affiliation` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `role` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `id_etab` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_enseignant`
--

INSERT INTO `28_2024_enseignant` (`id`, `nom`, `prenom`, `email`, `affiliation`, `role`, `id_etab`) VALUES
(1, 'Nidhal', 'Nidhal', 'nidhalabbassi9@gmail.com', 'PEESo', '', 8),
(2, 'Ala', 'Ala', 'ala.rom0311@gmail.com', 'PEESo', '', 7),
(5, 'refrent', 'detest', 'ala.romdhani@esprit.tn', 'peeso-22', '', 11);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_etablissements`
--

DROP TABLE IF EXISTS `28_2024_etablissements`;
CREATE TABLE IF NOT EXISTS `28_2024_etablissements` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `active` tinyint(1) NOT NULL DEFAULT '1',
  `logo` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=23 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_etablissements`
--

INSERT INTO `28_2024_etablissements` (`id`, `nom`, `active`, `logo`) VALUES
(1, 'Faculté de Droit et des Sciences Economiques et Politiques de Sousse', 1, 'Capture_décran_2024-11-26_141000.png'),
(2, 'Faculté de Médecine de Sousse', 1, 'Capture_décran_2024-11-26_143024.png'),
(3, 'Faculté des Lettres et des Sciences Humaines de Sousse', 1, 'Capture_décran_2024-11-26_142842.png'),
(5, 'Institut Supérieur de Finances et de Fiscalité de Sousse', 1, 'Capture_décran_2024-11-26_144441.png'),
(6, 'Institut Supérieur du Transport et de la Logistique de Sousse', 1, 'Capture_décran_2024-11-26_144142.png'),
(7, 'Institut Supérieur des Beaux Arts de Sousse', 1, 'Capture_décran_2024-11-26_143905.png'),
(8, 'Ecole Supérieure des Sciences et Techniques de la Santé de Sousse', 1, 'Capture_décran_2024-11-26_143631.png'),
(9, 'Institut Supérieur de Gestion de Sousse', 1, 'Capture_décran_2024-11-26_143736.png'),
(10, 'Institut des Hautes Etudes Commerciales de Sousse', 1, 'Capture_décran_2024-11-26_144109.png'),
(11, 'Institut Supérieur de Musique de Sousse', 1, 'Capture_décran_2024-11-26_144031.png'),
(12, 'Institut Supérieur des Sciences Appliquées et de la Technologie de Sousse', 1, 'Capture_décran_2024-11-26_144257.png'),
(13, 'Ecole Nationale des Ingénieurs de Sousse', 1, 'Capture_décran_2024-11-26_143442.png'),
(14, 'Institut Supérieur d Agronomie de Chott Meriem', 1, 'Capture_décran_2024-11-26_144756.png'),
(15, 'Institut Supérieur des Sciences infirmières de Sousse', 1, 'Capture_décran_2024-11-26_144344.png'),
(16, 'Ecole Supérieure des Sciences et des technologies de Hammam Sousse', 1, 'Capture_décran_2024-11-26_143538.png'),
(20, 'test2', 0, 'teacher1.png'),
(21, 'Faculté des Sciences Economiques et de Gestion de  Sousse', 1, 'Capture_décran_2024-11-26_143150.png'),
(22, 'Institut Supérieur d Informatique et des Technologies de Communication de Sousse', 1, 'Capture_décran_2024-11-26_144611.png');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_etudiants`
--

DROP TABLE IF EXISTS `28_2024_etudiants`;
CREATE TABLE IF NOT EXISTS `28_2024_etudiants` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `prenom` varchar(255) NOT NULL,
  `etablissement_id` int NOT NULL,
  `cin` varchar(255) NOT NULL,
  `email` varchar(255) NOT NULL,
  `phone` varchar(255) NOT NULL,
  `ville` varchar(255) NOT NULL,
  `adresse` varchar(255) NOT NULL,
  `code_postal` varchar(255) NOT NULL,
  `niveau` varchar(255) NOT NULL DEFAULT 'master1',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_etudiants`
--

INSERT INTO `28_2024_etudiants` (`id`, `nom`, `prenom`, `etablissement_id`, `cin`, `email`, `phone`, `ville`, `adresse`, `code_postal`, `niveau`) VALUES
(1, 'Ala', 'Romdhani', 4, '11955435', 'ala@gmail.com', '7894564', 'kairouan', 'adresse', '3100', 'master1'),
(2, 'ali', 'massoudi', 13, '22446688', 'ala.romhani@esprit.tn', '29208660', 'Tunis', 'adresse', '3100', 'master1'),
(4, '7ama', '7ama', 7, '64220128', '7ama@gmail.com', '77448855', 'aucun', 'aucun', '', 'Master1'),
(5, 'membre1', 'test', 9, '99186482', 'ala.romdhani@esprit.tn', '55733554', 'aucun', 'aucun', '', 'Master1'),
(6, 'membre2', 'test', 5, '76095142', 'alaromdh0311@gmail.com', '29208660', 'aucun', 'aucun', '', '3ème année'),
(7, 'ala ', 'test', 2, '18609872', 'benromdhane.medali2013@gmail.com', '22334466', 'aucun', 'aucun', '', 'Doctorat'),
(8, 'Mohammed', 'Mohammed', 14, '11111111', '1215', '20202020', 'Chott Mériem', 'adresse', '1215', 'master1'),
(9, 'etudiant', 'detest', 11, '11955440', 'alaromdhani0311@gmail.co', '25202025', 'ville', 'adresse', '3100', 'master1'),
(10, 'membre1', 'membre1', 10, '51115305', 'alaromdhani0311@gmail.co', '77440011', 'aucun', 'aucun', '', 'Master1'),
(11, 'romdhani', 'membre', 13, '75109136', 'alaromdhani0311@gmail.co', '22955468', 'aucun', 'aucun', '', 'Doctorat'),
(12, 'qssq', 'Romdhanisqsqs', 10, '11955470', 'ala.r0311@gmail.com', '29208660', 'Tunis', 'adresse', '3100', 'master1'),
(13, 'ala', 'romdhani', 11, '11955480', 'ala.rom0311@gmail.com', '55733554', 'Kairouan', 'adresse', '3100', 'master1'),
(14, 'ala ', 'membre', 3, '93744825', 'alaromdhani0311@gmail.tn', '29208660', 'aucun', 'aucun', '', 'Doctorat'),
(15, 'Abada', 'Balkis', 1, '14329482', 'Balkisabada@gmail.com', '55722905', 'ariana', 'adresse', '2094', 'master1');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_etudiants_activities`
--

DROP TABLE IF EXISTS `28_2024_etudiants_activities`;
CREATE TABLE IF NOT EXISTS `28_2024_etudiants_activities` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etudiant_id` int NOT NULL,
  `activity_id` int NOT NULL,
  `document` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  `created_at` date NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_etudiants_activities`
--

INSERT INTO `28_2024_etudiants_activities` (`id`, `etudiant_id`, `activity_id`, `document`, `created_at`) VALUES
(2, 16, 2, 'candidature_C1_2024_032507.pdf', '2024-11-21'),
(3, 16, 5, 'devoir.pdf', '2024-11-22');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_etudiants_projetcs`
--

DROP TABLE IF EXISTS `28_2024_etudiants_projetcs`;
CREATE TABLE IF NOT EXISTS `28_2024_etudiants_projetcs` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etudiant_id` int NOT NULL,
  `project_id` int NOT NULL,
  `cv` varchar(255) NOT NULL,
  `old_promotion` varchar(255) DEFAULT NULL,
  `porteur_project` tinyint(1) NOT NULL,
  `old_niveau` varchar(255) NOT NULL,
  `current_niveau` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'en_attente',
  `bilan_initiateur_ready` tinyint(1) NOT NULL DEFAULT '0',
  `bilan_innovateur_ready` tinyint(1) NOT NULL DEFAULT '0',
  `bilan_promoteur_ready` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_etudiants_projetcs`
--

INSERT INTO `28_2024_etudiants_projetcs` (`id`, `etudiant_id`, `project_id`, `cv`, `old_promotion`, `porteur_project`, `old_niveau`, `current_niveau`, `bilan_initiateur_ready`, `bilan_innovateur_ready`, `bilan_promoteur_ready`) VALUES
(1, 4, 7, 'rapport-ens.pdf', '1ére promotion', 0, 'Innovateur', 'en_attente', 0, 0, 0),
(2, 1, 7, 'cv-ala.pdf', NULL, 1, '', 'initiateur', 1, 1, 1),
(3, 5, 8, 'rapport-ala (1).pdf', '1ére promotion', 0, 'Initiateur', 'en_attente', 0, 0, 0),
(4, 6, 8, 'note_ped (1).pdf', '2éme Promotion', 0, 'Innovateur', 'en_attente', 0, 0, 0),
(5, 1, 8, 'cv-ala.pdf', '1ére promotion', 1, 'Promoteur', 'en_attente', 0, 0, 0),
(6, 7, 9, '', NULL, 0, '', 'en_attente', 0, 0, 0),
(7, 1, 9, 'CCAP - Conception développement et maintenance du site de la Philharmonie de Paris.pdf', NULL, 1, '', 'non', 0, 0, 0),
(8, 8, 11, 'attestation_de_revenu.pdf', '1ére promotion', 1, 'Initiateur', 'en_attente', 0, 0, 0),
(9, 10, 12, 'cv_membre1.pdf', '1ére promotion', 0, 'Initiateur', 'en_attente', 0, 0, 0),
(10, 9, 12, 'cv_etudiant_de_test.pdf', NULL, 1, '', 'en_attente', 1, 1, 1),
(11, 11, 13, 'fiche_suivie_aal_rom (1) (1).pdf', '1ére promotion', 0, 'Initiateur', 'en_attente', 0, 0, 0),
(12, 9, 13, 'fiche_suivie_aal_rom (1) (1).pdf', '1ére promotion', 1, 'Initiateur', 'en_attente', 1, 0, 0),
(13, 14, 14, 'fiche_suivie_etudiant_detest (2) (1).pdf', '1ére promotion', 0, 'Initiateur', 'en_attente', 0, 0, 0),
(14, 13, 14, 'cv.pdf', '1ére promotion', 1, 'Initiateur', 'en_attente', 1, 0, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_evenement`
--

DROP TABLE IF EXISTS `28_2024_evenement`;
CREATE TABLE IF NOT EXISTS `28_2024_evenement` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date NOT NULL,
  `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `photo` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_evenement`
--

INSERT INTO `28_2024_evenement` (`id`, `nom`, `date_debut`, `date_fin`, `description`, `photo`) VALUES
(2, 'Journée PEESo', '2024-10-28', '2024-11-05', 'Journée PEESo ', 'https://esen-sci-compet.com/eesu/assets/assets/images/event_peeso.jpg'),
(6, 'evennnnt', '2024-11-25', '2024-11-27', 'évènement pendant 3 jours', 'https://esen-sci-compet.com/eesu/assets/assets/images/peeso1.jpg');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_formations`
--

DROP TABLE IF EXISTS `28_2024_formations`;
CREATE TABLE IF NOT EXISTS `28_2024_formations` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `thumbnail` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  `rating` int NOT NULL DEFAULT '0',
  `created_at` date NOT NULL,
  `referent_id` int DEFAULT NULL,
  `publie` tinyint(1) NOT NULL DEFAULT '0',
  `nb_seance` int NOT NULL DEFAULT '0',
  `date_overture` date DEFAULT NULL,
  `date_debut` date DEFAULT NULL,
  `date_fin` date DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_formations`
--

INSERT INTO `28_2024_formations` (`id`, `nom`, `thumbnail`, `rating`, `created_at`, `referent_id`, `publie`, `nb_seance`, `date_overture`, `date_debut`, `date_fin`) VALUES
(2, 'Making Music with Other People', 'music.jfif', 0, '2024-11-14', NULL, 0, 0, NULL, '2024-11-30', '2024-12-31'),
(3, 'Formation De Management', 'Project-Scope-Management-Cover.avif', 0, '2024-11-15', 11, 0, 0, NULL, NULL, NULL),
(4, 'Formation DEV', NULL, 0, '2024-11-18', 11, 0, 0, NULL, NULL, NULL),
(7, 'Formation Devops', 'devops.jfif', 0, '2024-11-18', 5, 1, 0, NULL, '2024-11-30', '2024-12-31'),
(8, '3D Painting', NULL, 0, '2024-11-25', 2, 0, 0, NULL, NULL, NULL),
(9, 'painting', NULL, 0, '2024-11-25', 1, 0, 0, NULL, '2024-11-26', '2024-12-08'),
(10, 'painting', NULL, 0, '2024-11-25', 1, 0, 3, NULL, '2024-11-29', '2024-12-07'),
(11, 'Testing', NULL, 0, '2024-11-27', 3, 0, 0, NULL, '2024-11-28', '2024-11-30'),
(12, 'Testing', NULL, 0, '2024-11-27', 3, 0, 0, NULL, '2024-11-28', '2024-11-30'),
(13, 'Testing', NULL, 0, '2024-11-27', 3, 0, 3, NULL, '2024-11-28', '2024-11-30'),
(14, 'Testing', NULL, 0, '2024-11-27', 3, 0, 2, NULL, '2024-11-28', '2024-11-29'),
(15, 'Testing', NULL, 0, '2024-11-27', 1, 0, 2, NULL, '2024-11-28', '2024-11-29');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_formations_domaines`
--

DROP TABLE IF EXISTS `28_2024_formations_domaines`;
CREATE TABLE IF NOT EXISTS `28_2024_formations_domaines` (
  `id` int NOT NULL AUTO_INCREMENT,
  `formation_id` int NOT NULL,
  `domaine_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_formations_domaines`
--

INSERT INTO `28_2024_formations_domaines` (`id`, `formation_id`, `domaine_id`) VALUES
(4, 2, 3),
(5, 3, 7),
(6, 3, 12),
(7, 4, 10),
(14, 7, 10);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_formations_groupes`
--

DROP TABLE IF EXISTS `28_2024_formations_groupes`;
CREATE TABLE IF NOT EXISTS `28_2024_formations_groupes` (
  `id` int NOT NULL AUTO_INCREMENT,
  `formation_id` int NOT NULL,
  `groupe_id` int NOT NULL,
  `progression` int NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_formations_groupes`
--

INSERT INTO `28_2024_formations_groupes` (`id`, `formation_id`, `groupe_id`, `progression`) VALUES
(1, 3, 2, 0),
(2, 7, 3, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_formations_students`
--

DROP TABLE IF EXISTS `28_2024_formations_students`;
CREATE TABLE IF NOT EXISTS `28_2024_formations_students` (
  `id` int NOT NULL AUTO_INCREMENT,
  `formation_id` int NOT NULL,
  `student_id` int NOT NULL,
  `progression` int NOT NULL DEFAULT '0',
  `fiche_chaude_ready` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_formations_students`
--

INSERT INTO `28_2024_formations_students` (`id`, `formation_id`, `student_id`, `progression`, `fiche_chaude_ready`) VALUES
(5, 7, 8, 0, 0),
(6, 9, 8, 0, 0),
(7, 10, 8, 0, 0),
(8, 13, 8, 0, 0),
(9, 14, 8, 0, 0),
(10, 15, 8, 0, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_google_info`
--

DROP TABLE IF EXISTS `28_2024_google_info`;
CREATE TABLE IF NOT EXISTS `28_2024_google_info` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) DEFAULT NULL,
  `lastname` varchar(255) DEFAULT NULL,
  `email` varchar(255) NOT NULL,
  `access_token` text,
  `expires_in` int DEFAULT NULL,
  `created_at` datetime NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_google_info`
--

INSERT INTO `28_2024_google_info` (`id`, `name`, `lastname`, `email`, `access_token`, `expires_in`, `created_at`) VALUES
(4, 'Ala ROMDHANI', 'Ala ROMDHANI', 'ala.rom0311@gmail.com', 'ya29.a0AeDClZDxT8mEoz3jZKkcOPQn1LT0R3hoHibw9gZuZYMQLRPyVW88Fo5DXgSdiaTN9iLAZLNo2Ahc0Aky9BLR_5QadpHPbeM-wyBUOpbuI_gV31rhRhPR6dV2MEyxhKZ3M1CQCiofU8WfLgnGXdh-4MViXVZ3Ec-ElxUaCgYKAcQSARISFQHGX2Miom2WYZlZSurtJfVKdRxvCQ0170', 3599, '2024-11-04 17:06:04'),
(5, 'ala romdhani', 'ala romdhani', 'ala.romdhani@esprit.tn', 'ya29.a0AeDClZBzL128IteChoAYlGJjKTiC6l8cAloflTyJJsJyX_l_5sSgqwdHDFudXTNDzcpTFQAhcfSUZAVJ1mvrPpcMxi5IJ6qDvRfQiPhqMaaHr7v8hsaAkVLc1xp6wzEfRuONCUg-I23wbSxR3AS1T1lkSL7b3Gk4yDYaCgYKAbQSARASFQHGX2MicqFlz5m7rpr6yplfykEctQ0170', 3599, '2024-10-31 10:03:18'),
(6, 'rom ala', 'rom ala', 'ala.rom0311@gmail.com', 'ya29.a0AeDClZDrne1oRXzwAUZtrFrRaXrbSuGO-nSHIQBDZzft-f81uJnUgq4B8vahNiq1Ro4FZ6K6-hXwy29c3EOCT1PL0f5_-OCEjvNk_UnvzEiwKme4w30z9rlsc6BoZI8kO5OblvHFinTzaicei_mLdjqPeygOyunpcMgaCgYKAaESARISFQHGX2MihVXlUm9RBl89fni3p_JSHA0170', 3599, '2024-10-31 14:34:21'),
(7, 'rom ala', 'rom ala', 'ala.rom0311@gmail.com', 'ya29.a0AeDClZAAffE5ORO0_u1UI8GlTcuhoZ3p-xm7ouHzqlRURZpB-b9DnyPjhfJYewk63FM1v6BmjyOFFfy81Dcky0JwDC76gCtTjPD--5gSyTC2eNe69IKzGvVZyQ734kd7kLBR8oW87g2UE0_8AllGb-iBQe0w2AzfUgaCgYKAbkSARISFQHGX2MiemiB9psKaMHHqMf3cgzBTA0169', 3599, '2024-11-08 00:51:21'),
(8, 'ala romdhani', 'ala romdhani', 'alaromdhani0311@gmail.com', 'ya29.a0AeDClZB8K2FlL2Z-lqtJqv1tn7eNJQ-VYCZW1lWRK5LD5W2Deri-V_wle6p0-R79tLDHitF0p1AHsFrvn8uZsbyiLsia8CFLRmPKDkGjJHz5wWLE1kR-Hbay4CdU2RI_AtSa_-Vh8keR7TXU1LlHwmw_iTg9jH3QOiIaCgYKAUMSARASFQHGX2MiMEwdFrNzOfA06iin_1trQg0170', 3599, '2024-11-08 17:18:38'),
(9, 'rom ala', 'rom ala', 'ala.rom0311@gmail.com', 'ya29.a0AeDClZDXWlKTy7FCM4sujG00Svcc8EIU3XyxAJn82pvkXkTvPyGJ5NeSG8JyEFsOnWz4sDobQOE9rnF8oEmKJ9NGXsLi2FTe8uf9lH-vmUat7s3dZCxMHKUkSLa3BAO0ZmVMfU24JS43AjheKhyDb16Q_jFPWNYC0DUaCgYKAXYSARISFQHGX2MipvyMJvW53FMi7cKO-lWXnA0170', 3599, '2024-11-08 17:20:10');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_grille_evaluation`
--

DROP TABLE IF EXISTS `28_2024_grille_evaluation`;
CREATE TABLE IF NOT EXISTS `28_2024_grille_evaluation` (
  `id` int NOT NULL AUTO_INCREMENT,
  `porteur` enum('oui','non') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL DEFAULT 'oui',
  `membre` enum('oui','non') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL DEFAULT 'oui',
  `solide` enum('oui','non') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL,
  `depot` enum('oui','non') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL,
  `creative` int NOT NULL,
  `explore` int NOT NULL,
  `communication` int NOT NULL,
  `travail_equipe` int NOT NULL,
  `commentaire_critere` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `comprehension_problem` int NOT NULL,
  `offre` int NOT NULL,
  `innovation` int NOT NULL,
  `commentaire_idee` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `motivation` int NOT NULL,
  `commentaire_motivation` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `total` int NOT NULL,
  `stat_nat` enum('oui','non','en_attente') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL,
  `commentaire_avis` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `niveau` enum('en_attente','non','initiateur','innovateur','promoteur') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL,
  `id_enseignant` int NOT NULL,
  `id_projet` int NOT NULL,
  `signature_tuteur` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_grille_evaluation`
--

INSERT INTO `28_2024_grille_evaluation` (`id`, `porteur`, `membre`, `solide`, `depot`, `creative`, `explore`, `communication`, `travail_equipe`, `commentaire_critere`, `comprehension_problem`, `offre`, `innovation`, `commentaire_idee`, `motivation`, `commentaire_motivation`, `total`, `stat_nat`, `commentaire_avis`, `niveau`, `id_enseignant`, `id_projet`, `signature_tuteur`) VALUES
(2, 'oui', 'non', 'oui', 'oui', 5, 5, 5, 5, 'très bien', 5, 5, 5, 'très bien', 15, 'très bien', 50, 'oui', 'très bien', 'initiateur', 1, 8, ''),
(3, 'oui', 'non', 'oui', 'oui', 5, 5, 5, 5, 'Bien', 5, 5, 5, 'Bien', 15, 'Bien', 50, 'oui', 'Bien Trés Bien', 'initiateur', 2, 7, 'signature-401203.png'),
(4, 'oui', 'non', 'oui', 'oui', 3, 3, 4, 5, 'fds', 3, 4, 3, 'yteyryryryryryryr', 11, 'fyheryhryryr\'yryryryryryryryr', 35, 'non', 'u(utututututu', 'non', 2, 9, 'logo_time2022-11-23_033629.png'),
(5, 'non', 'oui', 'oui', 'oui', 5, 5, 5, 5, 'wfhgshdghgfh', 5, 5, 5, 'hfgfhdgjhfdgh', 15, 'dhgfhghgdhgfh', 50, 'oui', 'hsgfjhdfghfgh', 'initiateur', 2, 8, 'signature-401382.png'),
(6, 'non', 'oui', 'oui', 'oui', 4, 4, 4, 5, 'xbgdhfgshgfjhgfh', 4, 5, 4, 'hsghhjfjhjg', 15, 'gdfgshfghdjdj', 46, 'oui', 'hhgfjhjhdgjdhg', 'initiateur', 1, 11, 'signature-401385.png'),
(7, 'non', 'oui', 'oui', 'oui', 4, 4, 5, 4, 'egdsfhdjhgjfg', 4, 5, 5, 'jdfjhdgjhdgjhdgjdhgj', 15, 'hsfhgfhgfhjdfgf', 47, 'oui', 'shghfhgfhsghgfh', 'initiateur', 2, 11, 'signature-401206.png'),
(8, 'oui', 'oui', 'oui', 'oui', 4, 4, 4, 5, 'Observations et Commentaires', 4, 4, 4, 'Observations et Commentaires', 11, 'Observations et Commentaires', 40, 'oui', 'Observations et Commentaires', 'initiateur', 5, 12, 'signature4.png'),
(9, 'oui', 'oui', 'oui', 'oui', 4, 4, 5, 5, 'Observations et Commentaires', 3, 4, 4, 'Observations et Commentaires', 6, 'Observations et Commentaires', 34, 'oui', 'Observations et Commentaires', 'innovateur', 5, 10, 'signature6.png'),
(10, 'oui', 'oui', 'oui', 'oui', 4, 4, 5, 4, 'Observations et Commentaires', 3, 4, 5, 'Observations et Commentaires', 8, 'Observations et Commentaires', 38, 'oui', 'Observations et Commentaires', 'initiateur', 5, 11, 'signature8.png'),
(11, 'oui', 'oui', 'oui', 'oui', 4, 4, 3, 4, 'Observations et Commentaires', 3, 4, 3, 'Observations et Commentaires', 14, 'Observations et Commentaires', 38, 'oui', 'Observations et Commentaires', 'initiateur', 5, 13, 'signature9.png'),
(12, 'oui', 'oui', 'oui', 'oui', 2, 2, 4, 3, 'Observations et Commentaires', 4, 4, 3, 'Observations et Commentaires', 7, 'Observations et Commentaires', 30, 'oui', 'Observations et Commentaires', 'initiateur', 2, 14, 'signature10.png');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_inscriptions`
--

DROP TABLE IF EXISTS `28_2024_inscriptions`;
CREATE TABLE IF NOT EXISTS `28_2024_inscriptions` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etudiant_id` int NOT NULL,
  `appel_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_inscriptions`
--

INSERT INTO `28_2024_inscriptions` (`id`, `etudiant_id`, `appel_id`) VALUES
(1, 4, 5),
(2, 1, 5),
(3, 5, 4),
(4, 6, 4),
(5, 1, 4),
(6, 1, 1),
(7, 7, 1),
(8, 8, 7),
(9, 9, 7),
(10, 10, 7),
(11, 9, 6),
(12, 11, 6),
(13, 13, 1),
(14, 14, 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_langues`
--

DROP TABLE IF EXISTS `28_2024_langues`;
CREATE TABLE IF NOT EXISTS `28_2024_langues` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `niveau` varchar(255) NOT NULL,
  `etudiant_id` int DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=23 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_langues`
--

INSERT INTO `28_2024_langues` (`id`, `nom`, `niveau`, `etudiant_id`) VALUES
(15, 'Français', 'Bien', 1),
(16, 'Englais', 'Trés Bien', 1),
(17, 'Allemend', 'Bien', 1),
(18, 'Français', 'Passable', 8),
(19, 'Arabic', 'Trés Bien', 8),
(20, 'Englais', 'Bien', 8),
(21, 'Englais', 'Bien', 15),
(22, 'Français', 'Trés Bien', 15);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_livrable_initiateur`
--

DROP TABLE IF EXISTS `28_2024_livrable_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_livrable_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `livrable` varchar(255) DEFAULT NULL,
  `seance_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_livrable_initiateur`
--

INSERT INTO `28_2024_livrable_initiateur` (`id`, `name`, `livrable`, `seance_id`) VALUES
(1, 'Etude documentaire sur le secteur d’activité de son projet', 'fiche_suivie_aal_rom (7).pdf', 1),
(2, 'Description du problème, de la solution et de l’idée du projet.', 'fiche_accompagnementaal_rom_seance_1 (11).pdf', 1),
(3, 'Etude documentaire sur le secteur d’activité de son projet\r\n', 'fiche_suivie_aal_rom (1).pdf', 39),
(4, 'Description du problème, de la solution et de l’idée du projet.', 'fiche_suivie_aal_rom (4).pdf', 39),
(5, 'Le Business Model Canvas/Le Business Lean Canvas', 'fiche_suivie_aal_rom (1) (1).pdf', 41),
(6, 'Etude documentaire sur le secteur d’activité de son projet\r\n', 'fiche_suivie_etudiant_detest (2).pdf', 63),
(7, 'Description du problème, de la solution et de l’idée du projet.', 'fiche_suivie_etudiant_detest (1).pdf', 63),
(8, 'Etude documentaire sur le secteur d’activité de son projet\r\n', 'fiche_suivie_etudiant_detest (5).pdf', 75),
(9, 'Description du problème, de la solution et de l’idée du projet.', 'fiche_suivie_etudiant_detest (5).pdf', 75);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_livrable_innovateur`
--

DROP TABLE IF EXISTS `28_2024_livrable_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_livrable_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `livrable` varchar(255) DEFAULT NULL,
  `seance_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_livrable_innovateur`
--

INSERT INTO `28_2024_livrable_innovateur` (`id`, `name`, `livrable`, `seance_id`) VALUES
(1, 'Etude documentaire sur les étapes de la planification stratégique d’un projet\r\n(Vision, Mission, Objectifs, Stratégie, Tactiques : VMOST)', 'fiche_suivie_aal_rom (1) (1) (1).pdf', 20),
(2, 'Etude documentaire sur les étapes de la planification stratégique d’un projet\r\n(Vision, Mission, Objectifs, Stratégie, Tactiques : VMOST)', 'signature.png', 39);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_livrable_promoteur`
--

DROP TABLE IF EXISTS `28_2024_livrable_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_livrable_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `livrable` varchar(255) DEFAULT NULL,
  `seance_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_livrable_promoteur`
--

INSERT INTO `28_2024_livrable_promoteur` (`id`, `name`, `livrable`, `seance_id`) VALUES
(1, 'Faire une cartographie des parties prenantes', 'livrable_seance_1_promoteur.pdf', 17),
(2, 'Faire une cartographie des parties prenantes', 'fiche_accompagnementetudiant_detest_seance_1 (3).pdf', 25),
(3, 'Faire une cartographie des parties prenantes', 'fiche_suivie_etudiant_detest (5).pdf', 33);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_messages_peeso`
--

DROP TABLE IF EXISTS `28_2024_messages_peeso`;
CREATE TABLE IF NOT EXISTS `28_2024_messages_peeso` (
  `id` int NOT NULL AUTO_INCREMENT,
  `sender_id` int NOT NULL,
  `content` text NOT NULL,
  `sujet` text NOT NULL,
  `created_at` date NOT NULL,
  `type_sender` varchar(255) NOT NULL,
  `type_reciever` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_messages_peeso`
--

INSERT INTO `28_2024_messages_peeso` (`id`, `sender_id`, `content`, `sujet`, `created_at`, `type_sender`, `type_reciever`) VALUES
(1, 1, ' this is a test message for all coachs', 'hello', '2024-10-09', 'Etudiant', 'coach'),
(2, 1, ' this is a test message for all coachs', 'hello', '2024-10-09', 'Etudiant', 'coach');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_messages_users_peeso`
--

DROP TABLE IF EXISTS `28_2024_messages_users_peeso`;
CREATE TABLE IF NOT EXISTS `28_2024_messages_users_peeso` (
  `id` int NOT NULL AUTO_INCREMENT,
  `user_id` int NOT NULL,
  `message_id` int NOT NULL,
  `seen` tinyint(1) NOT NULL DEFAULT '0',
  `type_sender` varchar(255) NOT NULL,
  `type_reciever` varchar(255) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_messages_users_peeso`
--

INSERT INTO `28_2024_messages_users_peeso` (`id`, `user_id`, `message_id`, `seen`, `type_sender`, `type_reciever`) VALUES
(1, 3, 2, 0, 'Etudiant', 'coach');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_modules`
--

DROP TABLE IF EXISTS `28_2024_modules`;
CREATE TABLE IF NOT EXISTS `28_2024_modules` (
  `id` int NOT NULL AUTO_INCREMENT,
  `seance_id` int NOT NULL,
  `document` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  `titre` varchar(255) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_modules`
--

INSERT INTO `28_2024_modules` (`id`, `seance_id`, `document`, `titre`) VALUES
(3, 8, 'outils-devops.pdf', 'Outils De Devops'),
(4, 8, 'continuous devlivery.pdf', 'Outils D\'integration Continue'),
(5, 10, 'docker.pdf', 'Docker'),
(6, 10, 'docker.pdf', 'Docker-compose'),
(7, 9, 'maintenance.pdf', 'Maintenance'),
(8, 15, 'intro.pdf', 'Intro');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_news`
--

DROP TABLE IF EXISTS `28_2024_news`;
CREATE TABLE IF NOT EXISTS `28_2024_news` (
  `id` int NOT NULL AUTO_INCREMENT,
  `content` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `titre` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `image` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `added_by` int NOT NULL,
  `publie` tinyint(1) NOT NULL,
  `created_at` date NOT NULL,
  `content-en` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `titre-en` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_news`
--

INSERT INTO `28_2024_news` (`id`, `content`, `titre`, `image`, `added_by`, `publie`, `created_at`, `content-en`, `titre-en`) VALUES
(1, 'L’Université de Sousse a l’honneur d\'annoncer que dans le cadre de la compétition internationale START CUP COMPETITION 2024 “Entrepreneurship in the Mediterranean” organisée par UNIMED – ETF – AUF  et Lebanese University (Coordinator of UNIMED SubNetwork on Employability) le 21 et 22 Octobre 2024 en Turkie à l\'université d\'Aydin, l’équipe du projet « Smart Re-Rustica » composée de Khouloud BACCOUCHE étudiante en deuxième année mastère en protection des plantes et environnement à l’Institut Supérieur d’agronomie de Chott Meriem  et Mohamed OURAGINI étudiant en troisième année licence en Ingénierie des systèmes informatiques à l’Institut Supérieur d’informatique et des Technologies de Communication de Sousse qui ont représenté la Tunisie après avoir été finalistes ont eu le prix du « Projet le plus honorable \"devant une communauté diversifiée de toute la région méditerranéenne.', ' Félicitations et bravo aux étudiants entrepreneurs', '465273456_965522068941966_5645469509170765210_n.jpg', 0, 1, '2024-10-09', ' Félicitations Congratulations et bravo aux étudiants entrepreneurs du PEESo  Pôle Etudiant Entrepreneur Université de Sousse', ' Félicitations Congratulations et bravo aux étudiants entrepreneurs du PEESo  Pôle Etudiant Entrepreneur Université de Sousse'),
(5, '<p><span style=\"background-color: rgb(255, 255, 255); color: rgb(8, 8, 9);\">Participation de </span><a href=\"https://www.facebook.com/universityofsousse?__cft__[0]=AZXmEwfN8wTw7Z6aNQfyLlro61JdvgOBkMm0-2Mk2RarHAbrg3kkPX0sFow1qwSql9qtmhm3z5F0rBDZn4FU9rUrZGF4asFGDpjXA0lkUAMxZlPWWLlARe-F_CaMvHu9DbG5_acFV-PuQl_wA9pqmrPE42I4f7LCr4MqU1YKIbVM-_QjxXlEV38_6uSyRuqOy-4&amp;__tn__=-]K-R\" rel=\"noopener noreferrer\" target=\"_blank\" style=\"background-color: transparent; color: var(--blue-link);\">Université de Sousse</a><span style=\"background-color: rgb(255, 255, 255); color: rgb(8, 8, 9);\"> au Salon des Universités et de la formation organisé par la Foire de Sousse du 24 au 26 Mai 2024 avec la réservation d\'un espace au profit de quelques porteurs de projets de l\'Institut Supérieur d\'agronomie de Chott Meriem et de l\'Institut Supérieur des Beaux-arts de Sousse</span></p>', 'Participation au Salon des Universités', '445372715_468098882408826_4093442829963163760_n.jpg', 0, 1, '2024-11-07', '<p>Participation of the University of Sousse in the University and Training Fair organized by the Sousse Fair from May 24 to 26, 2024 with the reservation of a space for the benefit of some project leaders from the Higher Institute of Agronomy of Chott Meriem and the Higher Institute of Fine Arts of Sousse</p>', 'Participation in the University Fair'),
(6, '<p><span style=\"background-color: rgb(255, 255, 255); color: rgb(8, 8, 9);\">Comité de sélection des étudiants entrepreneurs de l\'ISSAT de Sousse, l\'ISET de Sousse, l\'ISTLS de de Sousse candidats à la troisième cohorte du Statut National de l\'Etudiant Entrepreneur au siège du PEESo.</span></p><p><span style=\"background-color: rgb(255, 255, 255); color: rgb(8, 8, 9);\">Remerciement à tous les référents : M. Mounir FRIJA, Mme. Hanene Memmi , M. Mourad HADHRI, M. Ahmed Youssef El FEKIH, M. Hichem HASSINE Mme. Maissa CHAIBI et à Mme Feten Ayari présidente de l\'espace entreprendre de Sousse</span></p><p><span style=\"background-color: rgb(255, 255, 255); color: rgb(8, 8, 9);\">Bonne chance à tous les candidats</span></p><p><br></p>', 'Comité de sélection des étudiants entrepreneurs ', '433426891_424779836740731_2165581947807999109_n.jpg', 0, 1, '2024-11-07', '<p>Selection committee of student entrepreneurs of ISSAT of Sousse, ISET of Sousse, ISTLS of Sousse candidates for the third cohort of the National Status of Student Entrepreneur at the headquarters of PEESo.</p><p>Thanks to all the referents: Mr. Mounir FRIJA, Mrs. Hanene Memmi, Mr. Mourad HADHRI, Mr. Ahmed Youssef El FEKIH, Mr. Hichem HASSINE Mrs. Maissa CHAIBI and Mrs. Feten Ayari president of the Sousse entrepreneurship space</p><p>Good luck to all the candidates</p>', 'Selection committee of student entrepreneurs');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_newsletters`
--

DROP TABLE IF EXISTS `28_2024_newsletters`;
CREATE TABLE IF NOT EXISTS `28_2024_newsletters` (
  `id` int NOT NULL AUTO_INCREMENT,
  `subject` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `titre` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `image` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `content` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `withlink` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `lien` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_newsletters`
--

INSERT INTO `28_2024_newsletters` (`id`, `subject`, `titre`, `image`, `content`, `withlink`, `lien`, `created_at`) VALUES
(2, 'Test', 'Test', 'projet24.jpeg', '<p><strong>Test</strong></p><p><u>Cordiallement,</u></p><p>PEESo</p>', 'on', 'www.google.com', '2024-10-10 14:12:32'),
(4, 'Test 2 ', 'test2', 'admin5.png', '<p><strong>Test2 :)</strong></p>', 'on', 'https://google.com', '2024-10-10 14:22:11'),
(5, 'Test 3', 'test3', 'projet26.jpeg', '<p><strong>Test3:)</strong></p>', 'on', 'https://google.com', '2024-10-10 15:06:23'),
(7, 'projet ', 'nouveau projet', 'projet28.jpeg', '<h1>Projet</h1><p><strong>Bonjour!</strong></p><h1><br></h1>', 'on', 'https://www.ama-business.com/', '2024-10-14 09:57:07'),
(8, 'dfhgjhjhg', 'djhgjdhgj', '', '<p>d,jghj,gdhgfhdgfh</p>', NULL, '', '2024-11-06 11:34:49');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parameter_bilan_competance_initiateur`
--

DROP TABLE IF EXISTS `28_2024_parameter_bilan_competance_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_parameter_bilan_competance_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parameter_bilan_competance_initiateur`
--

INSERT INTO `28_2024_parameter_bilan_competance_initiateur` (`id`, `name`) VALUES
(1, 'Développer ses aptitudes de communication'),
(2, 'Maîtriser les techniques de la communication verbale et non verbale'),
(3, 'Gérer les conflits'),
(4, 'Maîtriser la gestion du temps'),
(5, 'Appliquer les méthodes efficaces pour la gestion d\'équipe'),
(6, 'Acquérir les compétences sociales nécessaires pour créer et/ou renforcer son réseau : perception sociale, adaptabilité sociale, persuasion et influence,\r\nintelligence émotionnelle, marketing de soi'),
(7, 'Mobiliser son réseau personnel (e.g. Famille, amis, membres futurs de son entreprise…)'),
(8, 'Créer son réseau professionnel et d’affaires (Ex. clients, fournisseurs, hommes d’affaires, conseillers et experts comptables…)'),
(9, 'Diversifier son réseau de support (e.g. ANETI, APII, Chambre de commerce, UTICA…)'),
(10, 'Préparer son réseau de financement (e.g. Investisseurs, Banques, les société de leasing…)'),
(11, 'Développer l’opportunité entrepreneuriale '),
(12, 'Avoir la capacité à innover/inventer/improviser et la capacité d’imaginer, de penser à des idées nouvelles/inédites/originales'),
(13, 'Développer l’opportunité entrepreneuriale \r\n\r\n');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parameter_bilan_competance_innovateur`
--

DROP TABLE IF EXISTS `28_2024_parameter_bilan_competance_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_parameter_bilan_competance_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=34 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parameter_bilan_competance_innovateur`
--

INSERT INTO `28_2024_parameter_bilan_competance_innovateur` (`id`, `name`) VALUES
(1, 'Coordonner les diverses composantes d’un projet (les fonctions de planification et d’exécution du projet et de contrôle du changement)\r\nconformément à des normes de qualité, pour établir un juste milieu entre la durée, le coût et la qualité du projet'),
(2, 'Veiller à ce que le projet soit réalisé dans les délais prescrits'),
(3, 'Veiller à ce que le projet soit réalisé dans les limites du budget alloué'),
(4, 'Veiller à ce que le produit/service réponde aux besoins'),
(5, 'Faire preuve d’un leadership pour assurer un travail d’équipe de qualité'),
(6, 'Identifier les éléments du contexte du projet utiles à la communication (parties prenantes, budget, temps alloué, les limitations, contraintes,\r\nopportunités...)'),
(7, 'Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise'),
(8, 'Analyser les différents groupes cibles (leurs importances, leurs implications dans le projet, leurs connaissances, leurs aptitudes, leurs attentes...)'),
(9, 'Déterminer les objectifs de communication ( les résultats à atteindre, les objectifs formulés SMART, les indicateurs...)'),
(10, 'Définir quels messages pour quels groupes cibles (les messages primaires, les messages secondaires adaptés en fonction des groupes cibles, Keep It\r\nShort and Simple, storytelling, elevator pitch...)'),
(11, 'Etablir le plan d’actions (plan de communication visuel, planning de communication: tableau de bord et suivi des actions...)'),
(12, 'Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits'),
(13, 'Faire le suivi et adapter : outil de travail qui doit évoluer en permanence basé sur un plan-do-chek-act'),
(14, 'Evaluer et débriefer (objectifs sont atteints partiellement ou totalement, échec du plan, pistes d’amélioration, mesures d\'impact des actions, tirer des\r\nconclusions...)'),
(15, 'Décrire l’activité avec précision (utilité des produits et/ou services, leurs caractéristiques, cible, lieu de vente...)'),
(16, 'Etudier le marché visé (zone géographique, saisonnier ou permanent, niveau de concentration , barrières à l’entrée, types de clientèles, état actuel du\r\nmarché et ses tendances...)'),
(17, 'Etudier la compatibilité de l\'innovateur avec le besoin du son projet (ses motivations, ses objectifs, son savoir-faire, son potentiel, s’il y a des\r\ncompétences manquantes et trouver des associés qui pourront les apporter...)'),
(18, 'Analyser les forces et les faiblesses de l’idée (avantages par rapport à la concurrence et les contraintes du projet...)'),
(19, 'Tester l’idée de création d’entreprise (sélectionner les testeurs, tester le produit/service, analyser les retours...)'),
(20, 'Choisir la structure juridique selon la nature et l\'importance de l’activité (entreprise individuelle, SUARL, SARL, SA...)'),
(21, 'Déterminer la forme de l’entreprise selon la nature de l’activité, la volonté de s’associer, l’organisation patrimoniale, l’engagement financier et la\r\ncrédibilité vis-à-vis des partenaires'),
(22, 'Finaliser les formalités de constitution (attestations de dépôt de déclaration auprès de l\'API, déclaration d’ouverture de patente au bureau des impôts,\r\nenregistrement des statuts à la recette d’enregistrement des actes de sociétés...)'),
(23, 'Faire une analyse stratégique du projet, recenser les opportunités de l’entreprise et les éventuelles difficultés à court, moyen ou long terme'),
(24, 'Elaborer un plan de financement du projet : évaluer les besoins financiers pour la mise en œuvre et trouver les moyens qui couvrent les besoins'),
(25, 'Donner une appréciation du chiffre d’affaires (hypothèse du taux d’utilisation de la capacité, production, prévision du chiffre d\'affaires...)'),
(26, 'Estimer les charges du projet : s’acquitter de plusieurs charges, recenser toutes ces charges sans restriction aucune et d’en faire une synthèse'),
(27, 'Etablir un compte de résultat prévisionnel : capacité à rembourser les emprunts et savoir s\'il y a suffisamment de bénéfices pour vivre du projet'),
(28, 'Faire une étude financière qui permet de surveiller l’évolution de la trésorerie (e.g. sur 12 mois)'),
(29, 'Créer un design du produit'),
(30, 'Créer un prototype non fonctionnel (concevoir des plans 3D, 2D, établir le cahier des charges...)'),
(31, 'Créer un prototype de produit fonctionnel (le chiffrage selon la quantité et les caractéristiques techniques...)'),
(32, 'Valider des différents éléments du projet'),
(33, 'Vérifier l’adéquation aux exigences préétablies (géométrie des prototypes, dimensions, caractéristiques visuelles/physiques /mécaniques, finition)');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parameter_bilan_competance_promoteur`
--

DROP TABLE IF EXISTS `28_2024_parameter_bilan_competance_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_parameter_bilan_competance_promoteur` (
  `name` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `id` int NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=35 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parameter_bilan_competance_promoteur`
--

INSERT INTO `28_2024_parameter_bilan_competance_promoteur` (`name`, `id`) VALUES
('Identifier les parties prenantes : avoir une liste détaillée de tous les partenaires effectifs qui se positionnent en amont ou en aval de l’entreprise', 1),
('Mesurer le niveau d’intérêt et de pouvoir sur le projet de chaque partie prenante', 2),
('Classifier les parties prenantes', 3),
('Communiquer avec les différentes parties prenantes et partenaires, les persuader et entretenir des relations durables avec eux', 4),
('Définir une stratégie de négociation appropriée pour chaque partie prenante afin de répondre aux attentes communes', 5),
('Elaborer la matrice RACI (responsible, accountable, consulted et informed) et/ou RAM (responsibility assignment matrix) : répartir les tâches, organiser\r\nles circuits d’information, de décision, de validation', 6),
('Définir le contenu des postes de travail en termes d’activités et de compétences et les positionner dans l’organigramme de l’entreprise', 7),
('Mener un entretien de recrutement en utilisant les techniques adéquates', 8),
('Installer un esprit et une dynamique d’équipe', 9),
('Mettre en œuvre une politique de motivation des membres de l\'équipe : mettre en place des structures incitatives, motiver ses collaborateurs...', 10),
('Mettre en œuvre une politique de rémunération en définissant les salaires et autres composantes de rémunération', 11),
('Faire preuve d’écoute vis-à-vis des collaborateurs et être attentif aux problèmes et aux conflits', 12),
('Faire preuve d’écoute vis-à-vis des collaborateurs, être attentifs aux problèmes (gestion des conflits)', 13),
('Mettre en œuvre une politique de communication interne efficace', 14),
('Elaborer une matrice d’analyse des risques', 15),
('Identifier tous les risques possibles (positifs et négatifs)', 16),
('Mesurer l\'importance du risque et classifier les risques selon l’importance', 17),
('Proposer les solutions pour affronter les risques (éliminer, réduire, transférer, accepter, pivoter...)', 18),
('Mettre en œuvre les mesures de protection intellectuelle', 19),
('Formaliser les contrats avec les partenaires d’affaire', 20),
('Engager les formalités de création de l’entreprise', 21),
('Identifier les différentes ressources de financement (autofinancement, dettes bancaires, fonds d’investissements, subventions, leasing...)', 22),
('Identifier les incitations des investissements (code d’incitation des investissements, lois d\'investissements...)', 23),
('Choisir la structure de financement optimale', 24),
('Respecter le planing de projet.', 25),
('Faire preuve d’efficience et d’efficacité: Définir et surveiller l’atteinte des indicateurs clés de performances du projet.', 26),
('Ajuster le plan d\'action: déterminer les écarts et les mesures de corrections nécessaires.', 27),
('Modifier le plan d\'action en présence de risques imprévus (covid, forces majeurs...).', 28),
('Être en mesure d’identifier la stratégie commerciale convenable (en fonction du produit/service créé): domination par les coûts, différenciation,\r\nconcentration...', 29),
('Savoir tenir un discours commercial efficace', 30),
('Savoir créer un lien de confiance et fidéliser la clientèle...', 31),
('Concevoir une stratégie de communication bien ciblée, cohérente avec des objectifs marketing clairs', 32),
('Comprendre les usages et les valeurs ajoutées des réseaux sociaux pour augmenter sa visibilité', 33),
('Valoriser ses profils et son activité en ligne : développer sa e-réputation; découvrir les notions d’identité numérique, d’empreinte numérique...', 34);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parameter_livrables_initiateur`
--

DROP TABLE IF EXISTS `28_2024_parameter_livrables_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_parameter_livrables_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `id_attribution_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parameter_livrables_initiateur`
--

INSERT INTO `28_2024_parameter_livrables_initiateur` (`id`, `name`, `id_attribution_seance`) VALUES
(1, 'Etude documentaire sur le secteur d’activité de son projet\r\n', 1),
(2, 'Description du problème, de la solution et de l’idée du projet.', 1),
(3, 'Le Business Model Canvas/Le Business Lean Canvas', 3),
(4, 'L’étude de marché : recherche documentaire sur le marché,\r\n\r\n', 6),
(5, 'Les clients Cibles\r\n', 6),
(6, 'Les concurrents\r\n\r\n', 6),
(7, 'Les résultats de l’étude de marché\r\n', 8),
(8, 'Le chiffre d’affaires prévisionnel\r\n', 8),
(9, 'Les éléments de la stratégie marketing (les 4P)', 8),
(10, 'Le processus de production ou la chaine de valeur\r\n', 9),
(11, 'Les besoins en équipements / matières premières / main d’œuvre\r\n', 9),
(12, 'Les devis', 9),
(13, 'Le coût de l’investissement (Les devis)\r\n', 10),
(14, 'Les moyens de financement adaptés au projet\r\n', 10),
(15, 'Prévisions des charges et des produits', 10),
(16, 'Le tableau des produits et des charges prévisionnelles\r\n', 11),
(17, 'Le Tableau de trésorerie prévisionnelle\r\n', 11),
(18, 'Le calcul de rentabilité', 11),
(19, 'Les statuts de l’entreprise', 12),
(20, 'Capsule vidéo pour le pitching du projet (3 minutes)\r\n', 13),
(21, 'Présentation PPT du projet', 13),
(22, 'Capsule vidéo\r\n', 14),
(23, 'Présentation PPT\r\n', 14),
(24, '', 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parameter_livrables_innovateur`
--

DROP TABLE IF EXISTS `28_2024_parameter_livrables_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_parameter_livrables_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `id_attribution_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=27 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parameter_livrables_innovateur`
--

INSERT INTO `28_2024_parameter_livrables_innovateur` (`id`, `name`, `id_attribution_seance`) VALUES
(1, 'Etude documentaire sur les étapes de la planification stratégique d’un projet\r\n(Vision, Mission, Objectifs, Stratégie, Tactiques : VMOST)', 1),
(2, 'Les compétences d’un bon leader\r\n', 2),
(3, 'Les compétences pour une bonne gestion d’équipe', 2),
(4, 'Les types de risques\r\n', 3),
(5, 'Les outils de résolution de problèmes', 3),
(6, 'Les acteurs de l’écosystème entrepreneurial\r\n', 4),
(7, 'Les acteurs et les partenaires relatifs à la chaîne de valeur de son projet', 4),
(8, 'Analyse et état des lieux des besoins en communication\r\n', 5),
(9, 'Une description des objectifs de communication', 5),
(10, 'Formuler les messages pour les différents cibles', 6),
(11, 'Déterminer les étapes d’un plan de communication', 7),
(12, 'une enquête de satisfaction (questionnaire)', 8),
(13, 'les différents types de prototypages', 9),
(14, 'Réaliser le prototype de l’idée du projet', 10),
(15, 'Faire une enquête auprès de 10 utilisateurs pour un feedback (satisfaction/recommandations)\r\n', 11),
(16, 'Valider du prototype V0 (MVP)', 11),
(17, 'Rédiger un guide d’entretien ou un questionnaire', 12),
(18, 'Analyse concurrentielle relative au projet', 13),
(19, 'Déterminer la stratégie commerciale pour le projet', 14),
(20, 'Élaborer le plan marketing du projet', 15),
(21, 'Choisir et justifier son statut juridique\r\n', 16),
(22, 'Constituer son carnet d’adresse utile pour la constitution de son entreprise', 16),
(23, 'Choisir et justifier son statut juridique', 17),
(24, 'Constituer son carnet d’adresse utile pour la constitution de son entreprise', 17),
(25, 'Remplir le template pour dégager le schéma d’investissement et de financement de son projet', 18),
(26, 'Remplir le template pour dégager le taux de rentabilité du projet et présenter sa décision\r\nd’investissement', 19);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parameter_livrables_promoteur`
--

DROP TABLE IF EXISTS `28_2024_parameter_livrables_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_parameter_livrables_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `id_attribution_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parameter_livrables_promoteur`
--

INSERT INTO `28_2024_parameter_livrables_promoteur` (`id`, `name`, `id_attribution_seance`) VALUES
(1, 'Faire une cartographie des parties prenantes', 1),
(2, 'Contacter les parties prenantes et concrétiser les partenariats', 2),
(3, 'Elaborer les fiches de postes et l’organigramme de l’entreprise', 3),
(4, 'Définir les tests et les techniques d’embauche\r\n', 4),
(5, 'Elaborer la grille de salaire', 4),
(6, 'Elaborer la charte de communication interne (1ère version)', 5),
(7, 'Elaborer un registre des risques relatif au projet (catégories de risques : internes ou externes,\r\nniveau d’importance)', 6),
(8, 'Choisir l’approche de gestion des risques appropriée', 7),
(9, 'Dépôt du brevet d’invention (ou certificat ou autres)', 8);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_parcours`
--

DROP TABLE IF EXISTS `28_2024_parcours`;
CREATE TABLE IF NOT EXISTS `28_2024_parcours` (
  `id` int NOT NULL AUTO_INCREMENT,
  `etablissement_id` int DEFAULT NULL,
  `diplome` varchar(255) NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date NOT NULL,
  `etudiant_id` int DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_parcours`
--

INSERT INTO `28_2024_parcours` (`id`, `etablissement_id`, `diplome`, `date_debut`, `date_fin`, `etudiant_id`) VALUES
(5, 6, 'Master', '2018-06-17', '2024-10-30', 1),
(6, 6, 'BTP', '2024-10-17', '2024-10-30', 1),
(7, 7, 'LMD', '2020-01-01', '2023-06-02', 8),
(9, 1, 'BTP', '2020-09-15', '2023-06-06', 15);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_phases_initiateur`
--

DROP TABLE IF EXISTS `28_2024_phases_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_phases_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `num` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_phases_initiateur`
--

INSERT INTO `28_2024_phases_initiateur` (`id`, `nom`, `num`) VALUES
(1, 'Profiling', 1),
(2, 'Validation de l’idée du projet', 2),
(3, 'Elaboration du Business Model', 3),
(4, 'Elaboration du plan d’affaires', 4),
(5, 'Pitching', 5);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_phases_innovateur`
--

DROP TABLE IF EXISTS `28_2024_phases_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_phases_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `num` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_phases_innovateur`
--

INSERT INTO `28_2024_phases_innovateur` (`id`, `nom`, `num`) VALUES
(1, 'Gestion du projet', 1),
(2, 'Elaboration de plan de communication', 2),
(3, 'Validation du Prototype', 3),
(4, 'Stratégie commerciale', 4),
(5, 'Elaboration du schéma de financement et d’investissement', 5);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_phases_promoteur`
--

DROP TABLE IF EXISTS `28_2024_phases_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_phases_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) NOT NULL,
  `num` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_phases_promoteur`
--

INSERT INTO `28_2024_phases_promoteur` (`id`, `nom`, `num`) VALUES
(1, 'Partenariat et collaboration avec les\r\nparties prenantes', 1),
(2, 'Management de l’équipe', 2),
(3, 'Gestion des risques', 3),
(4, 'Démarches juridiques', 4),
(5, 'Accompagnement à l’incubation', 5);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_pieces_jointes`
--

DROP TABLE IF EXISTS `28_2024_pieces_jointes`;
CREATE TABLE IF NOT EXISTS `28_2024_pieces_jointes` (
  `id` int NOT NULL AUTO_INCREMENT,
  `path` varchar(255) NOT NULL,
  `type` varchar(255) NOT NULL,
  `project_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_pieces_jointes`
--

INSERT INTO `28_2024_pieces_jointes` (`id`, `path`, `type`, `project_id`) VALUES
(1, 'note_ped (1).pdf', 'application/pdf', 2),
(2, 'payment.PNG', 'image/png', 2),
(3, 'note_ped (1).pdf', 'application/pdf', 3),
(7, 'logoESEN.png', 'image/png', 3),
(8, 'peeso.jpg', 'image/jpeg', 4),
(9, '1esen4 (1).pdf', 'application/pdf', 4),
(10, 'cas.pdf', 'application/pdf', 4);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_projects`
--

DROP TABLE IF EXISTS `28_2024_projects`;
CREATE TABLE IF NOT EXISTS `28_2024_projects` (
  `id` int NOT NULL AUTO_INCREMENT,
  `domaine_id` int DEFAULT NULL,
  `description` text NOT NULL,
  `business_plan` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  `id_appel_condidature` int NOT NULL,
  `titre` varchar(255) NOT NULL,
  `id_tuteur` int DEFAULT NULL,
  `affecte` tinyint(1) NOT NULL,
  `date_envoi` date DEFAULT NULL,
  `isGroup` tinyint(1) NOT NULL DEFAULT '0',
  `besoin_project` text NOT NULL,
  `solution_project` text NOT NULL,
  `clients_potentiels` text NOT NULL,
  `offre` int NOT NULL,
  `genre_project` varchar(255) NOT NULL,
  `link_vid` varchar(255) NOT NULL,
  `creation_step` varchar(255) NOT NULL,
  `programme_accompagnent` varchar(255) CHARACTER SET latin1 COLLATE latin1_swedish_ci DEFAULT NULL,
  `motivation_letter` text NOT NULL,
  `etudiant_id` int NOT NULL,
  `is_finished` tinyint(1) NOT NULL DEFAULT '0',
  `date_finition` date NOT NULL,
  `niveau` varchar(255) NOT NULL DEFAULT 'en_attente',
  `1er_validation` tinyint(1) NOT NULL DEFAULT '0',
  `date_comite` datetime NOT NULL,
  `lieu_comite` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `bilan_initiateur_ready` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_projects`
--

INSERT INTO `28_2024_projects` (`id`, `domaine_id`, `description`, `business_plan`, `id_appel_condidature`, `titre`, `id_tuteur`, `affecte`, `date_envoi`, `isGroup`, `besoin_project`, `solution_project`, `clients_potentiels`, `offre`, `genre_project`, `link_vid`, `creation_step`, `programme_accompagnent`, `motivation_letter`, `etudiant_id`, `is_finished`, `date_finition`, `niveau`, `1er_validation`, `date_comite`, `lieu_comite`, `bilan_initiateur_ready`) VALUES
(7, NULL, '', NULL, 5, 'Mon Projet', 2, 1, NULL, 1, 'A quel besoin votre projet répond-il ? : \r\n                    ', 'Quelle solution apporte votre projet à ce besoin ? :   ', 'Qui sont vos clients potentiels? :\r\n                    ', 0, 'Transmission d\'activité familiale', 'https://www.youtube.com/watch?v=XoXA8nSKHr0&t=5s', 'le business plan est élaboré', NULL, 'Quelle est votre motivation pour intégrer le PEESo (100 mots au maximum) ? : Je suis Trés Motivé\r\n                    ', 1, 0, '2024-10-31', 'initiateur', 1, '0000-00-00 00:00:00', '', 0),
(9, NULL, '', NULL, 1, 'nouveau projetc ,ew', NULL, 1, NULL, 0, 'psum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also t\r\n                    ', 'psum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also t\r\n                    ', 'psum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also t\r\n                    ', 0, 'Transmission d\'activité familiale', 'youtub/com', 'le business plan est élaboré', NULL, 'psum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also t\r\n                    ', 1, 0, '0000-00-00', 'non', 1, '0000-00-00 00:00:00', '', 0),
(10, NULL, '', NULL, 6, 'Projet Agriculture', NULL, 0, NULL, 0, '\r\n               testtttttttttttttttttt     ', '\r\n                    testttttttttttttttttttttttttt', '\r\n                    testtttttttttttttttttttttttt', 0, 'Une création d\'activité', 'https://www.youtube.com/shorts/TBM4KBObuBg', 'le business plan est élaboré', NULL, '\r\n                    testtttttttttttt tewttttttttttttttttttt testtttttttttttttttttt testtttttttttttttt testtttttttttttttt testttttttttttttttttttttttt tetstttttttttttttt testttttttttt testttttttttttttttttt testtttttttttttttttttt testtttttttttttttttttttttttttttttttt', 8, 0, '0000-00-00', 'non', 1, '0000-00-00 00:00:00', '', 0),
(11, NULL, '', NULL, 7, 'Projet test', 5, 1, NULL, 0, '\r\n                    tesththdfjfhfhghgf', '\r\n                    testhfghfghgdhdghdgh', '\r\n                    testhfghgfjhgkjgkfjkjk', 0, 'Transmission d\'activité familiale', 'www.testryshstdh.com', 'juste une idée', NULL, '\r\n                    tfyreygvubftffg', 8, 0, '0000-00-00', 'en_attente', 1, '0000-00-00 00:00:00', '', 0),
(12, NULL, '', NULL, 7, 'Project de Musique', 5, 1, NULL, 0, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled\r\n                    ', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled\r\n                    ', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled\r\n                    ', 0, 'Une création d\'activité', 'https://www.youtube.com/watch?v=XoXA8nSKHr0&t=5s', 'le business plan est élaboré', NULL, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled\r\n                    ', 9, 0, '0000-00-00', 'innovateur', 1, '0000-00-00 00:00:00', '', 0),
(13, NULL, '', NULL, 6, 'Project De Test 1', 5, 1, NULL, 0, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It ', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It \r\n                    ', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It \r\n                    ', 0, 'Transmission d\'activité familiale', 'https://www.youtube.com/watch?v=QFaFIcGhPoM', 'le business plan est élaboré', NULL, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It \r\n                    ', 9, 0, '0000-00-00', 'promoteur', 1, '0000-00-00 00:00:00', '', 0),
(14, NULL, '', NULL, 1, 'Projet De Ala', 5, 1, NULL, 0, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essent', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essent\r\n                    ', '\r\n     Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essent               ', 0, 'Transmission d\'activité familiale', 'https://www.youtube.com/watch?v=QFaFIcGhPoM', 'le business plan est élaboré', NULL, 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essent\r\n                    ', 13, 0, '0000-00-00', 'incubateur', 1, '0000-00-00 00:00:00', '', 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_projects_domaines`
--

DROP TABLE IF EXISTS `28_2024_projects_domaines`;
CREATE TABLE IF NOT EXISTS `28_2024_projects_domaines` (
  `id` int NOT NULL AUTO_INCREMENT,
  `project_id` int NOT NULL,
  `domaine_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_projects_domaines`
--

INSERT INTO `28_2024_projects_domaines` (`id`, `project_id`, `domaine_id`) VALUES
(16, 3, 1),
(17, 3, 2),
(18, 4, 1),
(19, 5, 1),
(20, 6, 1),
(21, 7, 1),
(22, 8, 2),
(23, 9, 2),
(24, 9, 3),
(25, 10, 1),
(26, 11, 3),
(27, 12, 3),
(28, 12, 9),
(29, 13, 2),
(30, 14, 3);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_randez_vous`
--

DROP TABLE IF EXISTS `28_2024_randez_vous`;
CREATE TABLE IF NOT EXISTS `28_2024_randez_vous` (
  `id` int NOT NULL AUTO_INCREMENT,
  `titre` varchar(255) NOT NULL,
  `description` text NOT NULL,
  `date_debut` datetime NOT NULL,
  `date_fin` datetime NOT NULL,
  `created_at` date NOT NULL,
  `meet_link` varchar(255) DEFAULT NULL,
  `calender_link` varchar(255) DEFAULT NULL,
  `seance_id` int DEFAULT NULL,
  `programme_id` int DEFAULT NULL,
  `lieu` varchar(255) DEFAULT NULL,
  `added_by` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_randez_vous`
--

INSERT INTO `28_2024_randez_vous` (`id`, `titre`, `description`, `date_debut`, `date_fin`, `created_at`, `meet_link`, `calender_link`, `seance_id`, `programme_id`, `lieu`, `added_by`) VALUES
(1, 'Seance Devops', 'Introduction a CI\\CD', '2024-11-30 01:43:00', '2024-11-30 05:43:00', '0000-00-00', 'https://meet.google.com/fsu-botf-oby', 'https://www.google.com/calendar/event?eid=cmU1ZDdkM3JudjU1djEwdHQydGtiamZpZTRfMjAyNDEyMzBUMDgwMDAwWiBhbGFyb21kaGFuaTAzMTFAbQ', NULL, NULL, 'Tunis', 13),
(2, 'Seance Devops', 'Seance Devops Seance Devops', '2024-11-23 18:21:00', '2024-11-23 18:25:00', '0000-00-00', 'https://meet.google.com/hat-pubt-cvb', 'https://www.google.com/calendar/event?eid=MmJpM3UzNHEwNzd1NmJyMzc2bjgyMmdraGdfMjAyNDEyMzBUMDgwMDAwWiBhbGEucm9tMDMxMUBt', 8, NULL, 'Tunis', 13),
(3, 'Seance 1', 'Seance 1 Intro pour  test', '2024-11-23 15:27:00', '2024-11-23 21:23:00', '0000-00-00', 'https://meet.google.com/jyf-razg-yse', 'https://www.google.com/calendar/event?eid=ODlwMHBlbW1rYmp0NTJuazZpbWg4Y2txbWdfMjAyNDEyMzBUMDgwMDAwWiBhbGFyb21kaGFuaTAzMTFAbQ', 9, NULL, 'Tunis', 13),
(4, 'Séance1', '1er séance de testing', '2024-11-28 10:00:00', '2024-11-28 12:00:00', '0000-00-00', NULL, NULL, 27, NULL, 'Sousse', 2);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_randez_vous_users`
--

DROP TABLE IF EXISTS `28_2024_randez_vous_users`;
CREATE TABLE IF NOT EXISTS `28_2024_randez_vous_users` (
  `id` int NOT NULL AUTO_INCREMENT,
  `user_id` int NOT NULL,
  `randez_vous_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_randez_vous_users`
--

INSERT INTO `28_2024_randez_vous_users` (`id`, `user_id`, `randez_vous_id`) VALUES
(1, 22, 1),
(2, 27, 1),
(3, 22, 2),
(4, 22, 3),
(5, 27, 3);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_seances`
--

DROP TABLE IF EXISTS `28_2024_seances`;
CREATE TABLE IF NOT EXISTS `28_2024_seances` (
  `id` int NOT NULL AUTO_INCREMENT,
  `formation_id` int NOT NULL,
  `nom` varchar(255) NOT NULL,
  `ordre` int NOT NULL,
  `date_debut` datetime DEFAULT NULL,
  `date_fin` datetime DEFAULT NULL,
  `ended` tinyint(1) NOT NULL DEFAULT '0',
  `coaching_id` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_seances`
--

INSERT INTO `28_2024_seances` (`id`, `formation_id`, `nom`, `ordre`, `date_debut`, `date_fin`, `ended`, `coaching_id`) VALUES
(8, 10, 'Introduction', 1, '2024-11-30 13:21:00', '2024-11-21 15:27:00', 0, 0),
(9, 11, 'Introduction', 1, '2024-11-21 14:40:00', '2024-11-22 23:40:00', 0, 0),
(10, 10, 'Integration Continue /Livraison Contenu', 2, '2024-11-21 14:40:00', '2024-11-21 14:40:00', 0, 0),
(11, 12, 'Introduction', 1, '2024-11-21 13:41:00', '2024-11-21 18:41:00', 0, 0),
(12, 13, 'Introduction', 1, '2025-01-22 13:47:00', '2025-01-01 13:47:00', 0, 0),
(13, 13, 'Talking To A client', 2, '2024-11-01 15:48:00', '2024-11-30 17:48:00', 0, 0),
(14, 13, 'Marché Client', 3, '2024-11-30 18:48:00', '2024-11-30 19:53:00', 0, 0),
(15, 14, 'Introduction', 1, '2024-11-22 17:04:00', '2024-11-22 14:05:00', 0, 0),
(16, 14, 'Peeso C\'est Quoi', 2, '2024-11-22 15:05:00', '2024-11-22 20:05:00', 0, 0),
(17, 14, 'Seance 3', 3, '2024-11-22 18:06:00', '2024-11-22 18:06:00', 0, 0),
(18, 14, 'Seance4', 4, '2024-11-23 19:11:00', '2024-11-23 20:11:00', 0, 0),
(19, 10, '1er séance', 1, '2024-11-30 21:18:00', '2024-11-30 13:18:00', 0, 0),
(20, 10, '2eme séance', 2, '2024-12-03 22:18:00', '2024-12-03 12:19:00', 0, 0),
(21, 10, '3eme séance', 3, '2024-12-06 22:19:00', '2024-12-06 12:19:00', 0, 0),
(22, 13, 'séance1', 1, '2024-11-28 10:00:00', '2024-11-28 12:00:00', 0, 0),
(23, 13, 'séance2', 2, '2024-11-29 10:00:00', '2024-11-29 12:00:00', 0, 0),
(24, 13, 'séance3', 3, '2024-11-30 10:00:00', '2024-11-30 12:00:00', 0, 0),
(25, 14, 'séance1', 1, '2024-11-28 10:00:00', '2024-11-28 12:00:00', 0, 0),
(26, 14, 'séance2', 2, '2024-11-29 10:00:00', '2024-11-29 12:00:00', 0, 0),
(27, 15, 'séance1', 1, '2024-11-28 10:00:00', '2024-11-28 12:00:00', 0, 0),
(28, 15, 'séance2', 2, '2024-11-29 10:00:00', '2024-11-29 12:00:00', 0, 0),
(29, 0, 'séance1', 1, '2024-11-30 12:00:00', '2024-11-30 14:00:00', 0, 1),
(30, 0, 'séance2', 2, '2024-12-01 12:00:00', '2024-12-01 14:00:00', 0, 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_seance_group`
--

DROP TABLE IF EXISTS `28_2024_seance_group`;
CREATE TABLE IF NOT EXISTS `28_2024_seance_group` (
  `id` int NOT NULL AUTO_INCREMENT,
  `seance_id` int NOT NULL,
  `group_id` int NOT NULL,
  `ended` tinyint(1) NOT NULL DEFAULT '0',
  `started` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_seance_group`
--

INSERT INTO `28_2024_seance_group` (`id`, `seance_id`, `group_id`, `ended`, `started`) VALUES
(1, 5, 3, 0, 1),
(2, 6, 3, 0, 1),
(3, 7, 3, 0, 1);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_seance_initiateur`
--

DROP TABLE IF EXISTS `28_2024_seance_initiateur`;
CREATE TABLE IF NOT EXISTS `28_2024_seance_initiateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_attribution_seance` int NOT NULL,
  `id_projet` int NOT NULL,
  `id_tuteur` int DEFAULT NULL,
  `date` date DEFAULT NULL,
  `lieu` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `observation` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci,
  `signature` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `realise` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=327 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_seance_initiateur`
--

INSERT INTO `28_2024_seance_initiateur` (`id`, `id_attribution_seance`, `id_projet`, `id_tuteur`, `date`, `lieu`, `observation`, `signature`, `realise`) VALUES
(1, 1, 7, 2, '2024-10-14', 'Manzah 6', '\r\n                             \r\n                             Trés Bien\r\n                                                                                                      ', '', 1),
(2, 2, 7, 2, '2024-10-16', 'Parc mana3rech wiin', 'Vous pouver se concentrer sur l\'identification des Problémes\r\n                                                    ', 'signature.png', 1),
(3, 3, 7, 2, '2024-10-17', 'tunis', '\r\n                             mlkjhgfds                       ', 'logo time2022-11-23 033629.png', 1),
(6, 6, 7, 2, '2024-11-12', 'Tunis', 'Seance Good\r\n                                                    ', 'signature.png', 1),
(7, 7, 7, 2, '2024-11-13', 'Tunis', 'Tunis Good\r\n                                                    ', 'signature.png', 1),
(8, 8, 7, 2, '2024-10-30', 'Tunis', 'Very Good', 'signature.png', 1),
(9, 9, 7, 2, '2024-11-13', 'Tunis', 'Very good\r\n                                                    ', 'signature.png', 1),
(10, 10, 7, 2, '2024-11-13', 'Tunis', 'Very good\r\n                                                    ', 'signature.png', 1),
(11, 11, 7, 2, '2024-11-12', 'Tunis', 'Very good\r\n                                                    ', 'signature.png', 1),
(12, 12, 7, 2, NULL, NULL, NULL, NULL, 0),
(13, 13, 7, 2, NULL, NULL, NULL, NULL, 0),
(14, 14, 7, 2, NULL, NULL, NULL, NULL, 0),
(15, 1, 10, 1, NULL, NULL, NULL, NULL, 0),
(16, 2, 10, 1, NULL, NULL, NULL, NULL, 0),
(17, 3, 10, 1, NULL, NULL, NULL, NULL, 0),
(18, 6, 10, 1, NULL, NULL, NULL, NULL, 0),
(19, 7, 10, 1, NULL, NULL, NULL, NULL, 0),
(20, 8, 10, 1, NULL, NULL, NULL, NULL, 0),
(21, 9, 10, 1, NULL, NULL, NULL, NULL, 0),
(22, 10, 10, 1, NULL, NULL, NULL, NULL, 0),
(23, 11, 10, 1, NULL, NULL, NULL, NULL, 0),
(24, 12, 10, 1, NULL, NULL, NULL, NULL, 0),
(25, 13, 10, 1, NULL, NULL, NULL, NULL, 0),
(26, 14, 10, 1, NULL, NULL, NULL, NULL, 0),
(39, 1, 12, 5, '2024-11-11', 'Tunis', '\r\n    Observation et Avis :                                                ', 'signature.png', 1),
(40, 2, 12, 5, '2024-11-14', 'Date', '\r\n                             Date\r\n                                                                             ', 'signature.png', 1),
(41, 3, 12, 5, '2024-11-13', 'Date', '\r\n                             \r\n                            Date                                                 ', 'signature.png', 1),
(42, 6, 12, 5, NULL, NULL, NULL, NULL, 0),
(43, 7, 12, 5, NULL, NULL, NULL, NULL, 0),
(44, 8, 12, 5, NULL, NULL, NULL, NULL, 0),
(45, 9, 12, 5, NULL, NULL, NULL, NULL, 0),
(46, 10, 12, 5, NULL, NULL, NULL, NULL, 0),
(47, 11, 12, 5, NULL, NULL, NULL, NULL, 0),
(48, 12, 12, 5, NULL, NULL, NULL, NULL, 0),
(49, 13, 12, 5, NULL, NULL, NULL, NULL, 0),
(50, 14, 12, 5, NULL, NULL, NULL, NULL, 0),
(51, 1, 11, 5, '2024-11-05', 'Tunis', 'Observation et Avis :\r\n                                                    ', 'signature.png', 0),
(52, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(53, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(54, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(55, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(56, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(57, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(58, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(59, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(60, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(61, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(62, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(63, 1, 13, 5, '2024-11-06', 'Tunis', '\r\n                               Observation et Avis :                                                ', 'signature.png', 1),
(64, 2, 13, 5, NULL, NULL, NULL, NULL, 0),
(65, 3, 13, 5, NULL, NULL, NULL, NULL, 0),
(66, 6, 13, 5, NULL, NULL, NULL, NULL, 0),
(67, 7, 13, 5, NULL, NULL, NULL, NULL, 0),
(68, 8, 13, 5, NULL, NULL, NULL, NULL, 0),
(69, 9, 13, 5, NULL, NULL, NULL, NULL, 0),
(70, 10, 13, 5, NULL, NULL, NULL, NULL, 0),
(71, 11, 13, 5, NULL, NULL, NULL, NULL, 0),
(72, 12, 13, 5, NULL, NULL, NULL, NULL, 0),
(73, 13, 13, 5, NULL, NULL, NULL, NULL, 0),
(74, 14, 13, 5, NULL, NULL, NULL, NULL, 0),
(75, 1, 14, 5, '2024-11-21', 'Tunis', '\r\n           Observation et Avis :                                         ', 'signature.png', 1),
(76, 2, 14, 5, NULL, NULL, NULL, NULL, 0),
(77, 3, 14, 5, NULL, NULL, NULL, NULL, 0),
(78, 6, 14, 5, NULL, NULL, NULL, NULL, 0),
(79, 7, 14, 5, NULL, NULL, NULL, NULL, 0),
(80, 8, 14, 5, NULL, NULL, NULL, NULL, 0),
(81, 9, 14, 5, NULL, NULL, NULL, NULL, 0),
(82, 10, 14, 5, NULL, NULL, NULL, NULL, 0),
(83, 11, 14, 5, NULL, NULL, NULL, NULL, 0),
(84, 12, 14, 5, NULL, NULL, NULL, NULL, 0),
(85, 13, 14, 5, NULL, NULL, NULL, NULL, 0),
(86, 14, 14, 5, NULL, NULL, NULL, NULL, 0),
(87, 1, 10, NULL, NULL, NULL, NULL, NULL, 0),
(88, 2, 10, NULL, NULL, NULL, NULL, NULL, 0),
(89, 3, 10, NULL, NULL, NULL, NULL, NULL, 0),
(90, 6, 10, NULL, NULL, NULL, NULL, NULL, 0),
(91, 7, 10, NULL, NULL, NULL, NULL, NULL, 0),
(92, 8, 10, NULL, NULL, NULL, NULL, NULL, 0),
(93, 9, 10, NULL, NULL, NULL, NULL, NULL, 0),
(94, 10, 10, NULL, NULL, NULL, NULL, NULL, 0),
(95, 11, 10, NULL, NULL, NULL, NULL, NULL, 0),
(96, 12, 10, NULL, NULL, NULL, NULL, NULL, 0),
(97, 13, 10, NULL, NULL, NULL, NULL, NULL, 0),
(98, 14, 10, NULL, NULL, NULL, NULL, NULL, 0),
(99, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(100, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(101, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(102, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(103, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(104, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(105, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(106, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(107, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(108, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(109, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(110, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(111, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(112, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(113, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(114, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(115, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(116, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(117, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(118, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(119, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(120, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(121, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(122, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(123, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(124, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(125, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(126, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(127, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(128, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(129, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(130, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(131, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(132, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(133, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(134, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(135, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(136, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(137, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(138, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(139, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(140, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(141, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(142, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(143, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(144, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(145, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(146, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(147, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(148, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(149, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(150, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(151, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(152, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(153, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(154, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(155, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(156, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(157, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(158, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(159, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(160, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(161, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(162, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(163, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(164, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(165, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(166, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(167, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(168, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(169, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(170, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(171, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(172, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(173, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(174, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(175, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(176, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(177, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(178, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(179, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(180, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(181, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(182, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(183, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(184, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(185, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(186, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(187, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(188, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(189, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(190, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(191, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(192, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(193, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(194, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(195, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(196, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(197, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(198, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(199, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(200, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(201, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(202, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(203, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(204, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(205, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(206, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(207, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(208, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(209, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(210, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(211, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(212, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(213, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(214, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(215, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(216, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(217, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(218, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(219, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(220, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(221, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(222, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(223, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(224, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(225, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(226, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(227, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(228, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(229, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(230, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(231, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(232, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(233, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(234, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(235, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(236, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(237, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(238, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(239, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(240, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(241, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(242, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(243, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(244, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(245, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(246, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(247, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(248, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(249, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(250, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(251, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(252, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(253, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(254, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(255, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(256, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(257, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(258, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(259, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(260, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(261, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(262, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(263, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(264, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(265, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(266, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(267, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(268, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(269, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(270, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(271, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(272, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(273, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(274, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(275, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(276, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(277, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(278, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(279, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(280, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(281, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(282, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(283, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(284, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(285, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(286, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(287, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(288, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(289, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(290, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(291, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(292, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(293, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(294, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(295, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(296, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(297, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(298, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(299, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(300, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(301, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(302, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(303, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(304, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(305, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(306, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(307, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(308, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(309, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(310, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(311, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(312, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(313, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(314, 14, 11, 5, NULL, NULL, NULL, NULL, 0),
(315, 1, 11, 5, NULL, NULL, NULL, NULL, 0),
(316, 2, 11, 5, NULL, NULL, NULL, NULL, 0),
(317, 3, 11, 5, NULL, NULL, NULL, NULL, 0),
(318, 6, 11, 5, NULL, NULL, NULL, NULL, 0),
(319, 7, 11, 5, NULL, NULL, NULL, NULL, 0),
(320, 8, 11, 5, NULL, NULL, NULL, NULL, 0),
(321, 9, 11, 5, NULL, NULL, NULL, NULL, 0),
(322, 10, 11, 5, NULL, NULL, NULL, NULL, 0),
(323, 11, 11, 5, NULL, NULL, NULL, NULL, 0),
(324, 12, 11, 5, NULL, NULL, NULL, NULL, 0),
(325, 13, 11, 5, NULL, NULL, NULL, NULL, 0),
(326, 14, 11, 5, NULL, NULL, NULL, NULL, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_seance_innovateur`
--

DROP TABLE IF EXISTS `28_2024_seance_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_seance_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_attribution_seance` int NOT NULL,
  `id_projet` int NOT NULL,
  `id_tuteur` int DEFAULT NULL,
  `date` datetime DEFAULT NULL,
  `lieu` varchar(255) DEFAULT NULL,
  `observation` text,
  `signature` varchar(255) DEFAULT NULL,
  `realise` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=77 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_seance_innovateur`
--

INSERT INTO `28_2024_seance_innovateur` (`id`, `id_attribution_seance`, `id_projet`, `id_tuteur`, `date`, `lieu`, `observation`, `signature`, `realise`) VALUES
(1, 1, 7, 2, '2024-11-06 00:00:00', 'Lieu', 'Rnadez Vous Seance realise\r\n                                                    ', 'signature.png', 1),
(2, 2, 7, 2, '2024-11-20 12:29:00', 'Tunis', 'Observation\r\n                                                    ', 'signature.png', 1),
(3, 3, 7, 2, '2024-11-21 12:31:00', 'Tunis', 'Observation\r\n                                                    ', 'signature.png', 1),
(4, 4, 7, 2, '2024-11-13 23:38:00', 'Tunis', 'Observation seance 4\r\n                                                    ', 'logo_meet.png', 1),
(5, 5, 7, 2, NULL, NULL, NULL, NULL, 0),
(6, 6, 7, 2, NULL, NULL, NULL, NULL, 0),
(7, 7, 7, 2, NULL, NULL, NULL, NULL, 0),
(8, 8, 7, 2, NULL, NULL, NULL, NULL, 0),
(9, 9, 7, 2, NULL, NULL, NULL, NULL, 0),
(10, 10, 7, 2, NULL, NULL, NULL, NULL, 0),
(11, 11, 7, 2, NULL, NULL, NULL, NULL, 0),
(12, 12, 7, 2, NULL, NULL, NULL, NULL, 0),
(13, 13, 7, 2, NULL, NULL, NULL, NULL, 0),
(14, 14, 7, 2, NULL, NULL, NULL, NULL, 0),
(15, 15, 7, 2, NULL, NULL, NULL, NULL, 0),
(16, 16, 7, 2, NULL, NULL, NULL, NULL, 0),
(17, 17, 7, 2, NULL, NULL, NULL, NULL, 0),
(18, 18, 7, 2, NULL, NULL, NULL, NULL, 0),
(19, 19, 7, 2, NULL, NULL, NULL, NULL, 0),
(20, 1, 12, 5, '2024-11-07 22:35:00', 'Tunis', 'Observation et Avis :\r\n                                                    ', 'signature.png', 1),
(21, 2, 12, 5, NULL, NULL, NULL, NULL, 0),
(22, 3, 12, 5, NULL, NULL, NULL, NULL, 0),
(23, 4, 12, 5, NULL, NULL, NULL, NULL, 0),
(24, 5, 12, 5, NULL, NULL, NULL, NULL, 0),
(25, 6, 12, 5, NULL, NULL, NULL, NULL, 0),
(26, 7, 12, 5, NULL, NULL, NULL, NULL, 0),
(27, 8, 12, 5, NULL, NULL, NULL, NULL, 0),
(28, 9, 12, 5, NULL, NULL, NULL, NULL, 0),
(29, 10, 12, 5, NULL, NULL, NULL, NULL, 0),
(30, 11, 12, 5, NULL, NULL, NULL, NULL, 0),
(31, 12, 12, 5, NULL, NULL, NULL, NULL, 0),
(32, 13, 12, 5, NULL, NULL, NULL, NULL, 0),
(33, 14, 12, 5, NULL, NULL, NULL, NULL, 0),
(34, 15, 12, 5, NULL, NULL, NULL, NULL, 0),
(35, 16, 12, 5, NULL, NULL, NULL, NULL, 0),
(36, 17, 12, 5, NULL, NULL, NULL, NULL, 0),
(37, 18, 12, 5, NULL, NULL, NULL, NULL, 0),
(38, 19, 12, 5, NULL, NULL, NULL, NULL, 0),
(39, 1, 13, 5, '2024-10-31 00:23:00', 'Tunis', 'Observation et Avis :Observation et Avis :\r\n                                                    ', 'signature.png', 1),
(40, 2, 13, 5, NULL, NULL, NULL, NULL, 0),
(41, 3, 13, 5, NULL, NULL, NULL, NULL, 0),
(42, 4, 13, 5, NULL, NULL, NULL, NULL, 0),
(43, 5, 13, 5, NULL, NULL, NULL, NULL, 0),
(44, 6, 13, 5, NULL, NULL, NULL, NULL, 0),
(45, 7, 13, 5, NULL, NULL, NULL, NULL, 0),
(46, 8, 13, 5, NULL, NULL, NULL, NULL, 0),
(47, 9, 13, 5, NULL, NULL, NULL, NULL, 0),
(48, 10, 13, 5, NULL, NULL, NULL, NULL, 0),
(49, 11, 13, 5, NULL, NULL, NULL, NULL, 0),
(50, 12, 13, 5, NULL, NULL, NULL, NULL, 0),
(51, 13, 13, 5, NULL, NULL, NULL, NULL, 0),
(52, 14, 13, 5, NULL, NULL, NULL, NULL, 0),
(53, 15, 13, 5, NULL, NULL, NULL, NULL, 0),
(54, 16, 13, 5, NULL, NULL, NULL, NULL, 0),
(55, 17, 13, 5, NULL, NULL, NULL, NULL, 0),
(56, 18, 13, 5, NULL, NULL, NULL, NULL, 0),
(57, 19, 13, 5, NULL, NULL, NULL, NULL, 0),
(58, 1, 14, 5, NULL, NULL, NULL, NULL, 0),
(59, 2, 14, 5, NULL, NULL, NULL, NULL, 0),
(60, 3, 14, 5, NULL, NULL, NULL, NULL, 0),
(61, 4, 14, 5, NULL, NULL, NULL, NULL, 0),
(62, 5, 14, 5, NULL, NULL, NULL, NULL, 0),
(63, 6, 14, 5, NULL, NULL, NULL, NULL, 0),
(64, 7, 14, 5, NULL, NULL, NULL, NULL, 0),
(65, 8, 14, 5, NULL, NULL, NULL, NULL, 0),
(66, 9, 14, 5, NULL, NULL, NULL, NULL, 0),
(67, 10, 14, 5, NULL, NULL, NULL, NULL, 0),
(68, 11, 14, 5, NULL, NULL, NULL, NULL, 0),
(69, 12, 14, 5, NULL, NULL, NULL, NULL, 0),
(70, 13, 14, 5, NULL, NULL, NULL, NULL, 0),
(71, 14, 14, 5, NULL, NULL, NULL, NULL, 0),
(72, 15, 14, 5, NULL, NULL, NULL, NULL, 0),
(73, 16, 14, 5, NULL, NULL, NULL, NULL, 0),
(74, 17, 14, 5, NULL, NULL, NULL, NULL, 0),
(75, 18, 14, 5, NULL, NULL, NULL, NULL, 0),
(76, 19, 14, 5, NULL, NULL, NULL, NULL, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_seance_promoteur`
--

DROP TABLE IF EXISTS `28_2024_seance_promoteur`;
CREATE TABLE IF NOT EXISTS `28_2024_seance_promoteur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_attribution_seance` int NOT NULL,
  `id_projet` int NOT NULL,
  `date` datetime DEFAULT NULL,
  `lieu` varchar(255) DEFAULT NULL,
  `observation` text,
  `signature` varchar(255) DEFAULT NULL,
  `realise` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=57 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_seance_promoteur`
--

INSERT INTO `28_2024_seance_promoteur` (`id`, `id_attribution_seance`, `id_projet`, `date`, `lieu`, `observation`, `signature`, `realise`) VALUES
(17, 1, 7, '2024-11-15 22:43:00', 'Place pasteur', 'observation\r\n                                                    ', 'signature.png', 1),
(18, 2, 7, '2024-11-11 01:58:00', 'ssxsx', '\r\n       Observation et Avis :                                             ', 'signature.png', 1),
(19, 3, 7, '2024-11-28 10:59:00', 'drggd', '\r\n                             \r\n       Observation et Avis :                                                                      ', 'signature.png', 0),
(20, 4, 7, NULL, NULL, NULL, NULL, 0),
(21, 5, 7, NULL, NULL, NULL, NULL, 0),
(22, 6, 7, NULL, NULL, NULL, NULL, 0),
(23, 7, 7, NULL, NULL, NULL, NULL, 0),
(24, 8, 7, NULL, NULL, NULL, NULL, 0),
(25, 1, 12, NULL, NULL, NULL, NULL, 0),
(26, 2, 12, NULL, NULL, NULL, NULL, 0),
(27, 3, 12, NULL, NULL, NULL, NULL, 0),
(28, 4, 12, NULL, NULL, NULL, NULL, 0),
(29, 5, 12, NULL, NULL, NULL, NULL, 0),
(30, 6, 12, NULL, NULL, NULL, NULL, 0),
(31, 7, 12, NULL, NULL, NULL, NULL, 0),
(32, 8, 12, NULL, NULL, NULL, NULL, 0),
(41, 1, 13, NULL, NULL, NULL, NULL, 0),
(42, 2, 13, NULL, NULL, NULL, NULL, 0),
(43, 3, 13, NULL, NULL, NULL, NULL, 0),
(44, 4, 13, NULL, NULL, NULL, NULL, 0),
(45, 5, 13, NULL, NULL, NULL, NULL, 0),
(46, 6, 13, NULL, NULL, NULL, NULL, 0),
(47, 7, 13, NULL, NULL, NULL, NULL, 0),
(48, 8, 13, NULL, NULL, NULL, NULL, 0),
(49, 1, 14, NULL, NULL, NULL, NULL, 0),
(50, 2, 14, NULL, NULL, NULL, NULL, 0),
(51, 3, 14, NULL, NULL, NULL, NULL, 0),
(52, 4, 14, NULL, NULL, NULL, NULL, 0),
(53, 5, 14, NULL, NULL, NULL, NULL, 0),
(54, 6, 14, NULL, NULL, NULL, NULL, 0),
(55, 7, 14, NULL, NULL, NULL, NULL, 0),
(56, 8, 14, NULL, NULL, NULL, NULL, 0);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_signature_etudiant`
--

DROP TABLE IF EXISTS `28_2024_signature_etudiant`;
CREATE TABLE IF NOT EXISTS `28_2024_signature_etudiant` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_seance` int NOT NULL,
  `id_etudiant` int NOT NULL,
  `signature` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_signature_etudiant`
--

INSERT INTO `28_2024_signature_etudiant` (`id`, `id_seance`, `id_etudiant`, `signature`) VALUES
(1, 1, 1, 'data:image/png;base64,data:image/png;base64,'),
(2, 2, 1, 'data:image/png;base64,');
INSERT INTO `28_2024_signature_etudiant` (`id`, `id_seance`, `id_etudiant`, `signature`) VALUES
(8, 1, 4, 'data:image/png;base64,'),
(9, 2, 4, 'data:image/png;base64,');
INSERT INTO `28_2024_signature_etudiant` (`id`, `id_seance`, `id_etudiant`, `signature`) VALUES
(10, 3, 1, 'data:image/png;base64,'),
(11, 6, 1, 'data:image/png;base64,'),
(12, 7, 1, 'data:image/png;base64,');
INSERT INTO `28_2024_signature_etudiant` (`id`, `id_seance`, `id_etudiant`, `signature`) VALUES
(13, 39, 9, 'data:image/png;base64,'),
(14, 63, 9, 'data:image/png;base64,'),
(15, 75, 13, 'data:image/png;base64,');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_signature_etudiant_innovateur`
--

DROP TABLE IF EXISTS `28_2024_signature_etudiant_innovateur`;
CREATE TABLE IF NOT EXISTS `28_2024_signature_etudiant_innovateur` (
  `id` int NOT NULL AUTO_INCREMENT,
  `signature` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `id_etudiant` int NOT NULL,
  `id_seance` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_signature_etudiant_innovateur`
--

INSERT INTO `28_2024_signature_etudiant_innovateur` (`id`, `signature`, `id_etudiant`, `id_seance`) VALUES
(1, 'data:image/png;base64,', 1, 1),
(2, 'data:image/png;base64,', 9, 20),
(3, 'data:image/png;base64,', 9, 39);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_stage_emploi`
--

DROP TABLE IF EXISTS `28_2024_stage_emploi`;
CREATE TABLE IF NOT EXISTS `28_2024_stage_emploi` (
  `id` int NOT NULL AUTO_INCREMENT,
  `entreprise` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `titre` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `adresse` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `date_publication` date NOT NULL,
  `description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `type` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_stage_emploi`
--

INSERT INTO `28_2024_stage_emploi` (`id`, `entreprise`, `titre`, `adresse`, `date_publication`, `description`, `type`) VALUES
(1, 'Test Entreprise', 'Test Stage', 'test adresse 123', '2024-11-27', 'Offre de stage ', 'stage'),
(2, 'emploi entreprise', 'test emploi', 'adresse 123', '2024-11-27', 'c\'est une offre d\'emploi ', 'emploi'),
(3, 'pfe entreprise', 'test pfe', 'adresse 456', '2024-11-27', 'c\'est une offre de pfe test description', 'PFE');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_structure_appui`
--

DROP TABLE IF EXISTS `28_2024_structure_appui`;
CREATE TABLE IF NOT EXISTS `28_2024_structure_appui` (
  `id` int NOT NULL AUTO_INCREMENT,
  `nom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `prenom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `raison_sociale` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `email` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `tel` int NOT NULL,
  `type_etab` enum('etab_public','etab_prive') NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_subscribers`
--

DROP TABLE IF EXISTS `28_2024_subscribers`;
CREATE TABLE IF NOT EXISTS `28_2024_subscribers` (
  `id` int NOT NULL AUTO_INCREMENT,
  `email` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_subscribers`
--

INSERT INTO `28_2024_subscribers` (`id`, `email`) VALUES
(1, 'nidhalabbassi9@gmail.com'),
(3, 'balkisabada@gmail.com');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_subscribers_competition`
--

DROP TABLE IF EXISTS `28_2024_subscribers_competition`;
CREATE TABLE IF NOT EXISTS `28_2024_subscribers_competition` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_competition` int NOT NULL,
  `nom_prenom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `email` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `message` varchar(255) CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL,
  `cv` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `signature` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `is_accepted` enum('en_attente','oui','non') CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL DEFAULT 'en_attente',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_subscribers_competition`
--

INSERT INTO `28_2024_subscribers_competition` (`id`, `id_competition`, `nom_prenom`, `email`, `message`, `cv`, `signature`, `is_accepted`) VALUES
(1, 1, 'Nidhal Abbassi', 'nidhalabbassi9@gmail.com', 'sdgfdgfhfghghdffhfghfgh', 'https://esen-sci-compet.com/eesu/assets/assets/images/CV_Nidhal_ABBASSI_20242.pdf', 'data:image/png;base64,', 'non'),
(2, 1, 'test', 'cherif@gmail.com', 'jg hhjgvyyvjg', 'https://esen-sci-compet.com/eesu/assets/assets/images/CV_Nidhal_ABBASSI_20243.pdf', 'data:image/png;base64,', 'oui'),
(3, 3, 'Nidhal Abbassi', 'nidhalabbassi9@gmail.com', 'qsfsdhghgfdgstrhtrhgrt', 'https://esen-sci-compet.com/peeso-28/assets/assets/images/candidature_C1_2024_032507.pdf', 'data:image/png;base64,', 'oui');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_subscribers_evenement`
--

DROP TABLE IF EXISTS `28_2024_subscribers_evenement`;
CREATE TABLE IF NOT EXISTS `28_2024_subscribers_evenement` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_evenement` int NOT NULL,
  `nom_prenom` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `email` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `message` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_subscribers_evenement`
--

INSERT INTO `28_2024_subscribers_evenement` (`id`, `id_evenement`, `nom_prenom`, `email`, `message`) VALUES
(1, 1, 'test test', 'test@mail.com', 'test test testtest test testtest test testtest test testtest test testtest test test'),
(2, 1, 'Nidhal Abbassi', 'nidhalabbassi9@gmail.com', 'pour améliorer mes compétences!'),
(3, 2, 'Nidhal Abbassi', 'nidhalabbassi9@gmail.com', 'testttt testttt testttt testttt');

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_tuteur_domaine`
--

DROP TABLE IF EXISTS `28_2024_tuteur_domaine`;
CREATE TABLE IF NOT EXISTS `28_2024_tuteur_domaine` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_tuteur` int NOT NULL,
  `id_domaine` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=23 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_tuteur_domaine`
--

INSERT INTO `28_2024_tuteur_domaine` (`id`, `id_tuteur`, `id_domaine`) VALUES
(8, 3, 7),
(9, 3, 8),
(11, 2, 3),
(12, 2, 5),
(17, 1, 4),
(18, 4, 13),
(19, 5, 2),
(20, 5, 3),
(21, 6, 4),
(22, 7, 14);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_tuteur_projet`
--

DROP TABLE IF EXISTS `28_2024_tuteur_projet`;
CREATE TABLE IF NOT EXISTS `28_2024_tuteur_projet` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_tuteur` int NOT NULL,
  `id_projet` int NOT NULL,
  `date_envoi` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `acceptation_status` int NOT NULL,
  `status` varchar(11) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `rapport` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `id_tuteur` (`id_tuteur`),
  KEY `id_projet` (`id_projet`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_tuteur_projet`
--

INSERT INTO `28_2024_tuteur_projet` (`id`, `id_tuteur`, `id_projet`, `date_envoi`, `acceptation_status`, `status`, `rapport`) VALUES
(6, 1, 4, '2024-10-14 11:33:14', 0, '0', NULL),
(7, 1, 3, '2024-10-14 13:49:42', 0, '0', NULL),
(8, 2, 4, '2024-10-14 16:17:44', 0, '0', NULL),
(12, 2, 9, '2024-10-31 15:52:55', 0, '1', NULL),
(13, 3, 8, '2024-11-01 10:27:42', 0, '0', NULL),
(14, 2, 7, '2024-11-04 10:27:42', 0, '0', NULL),
(15, 5, 12, '2024-11-07 11:38:54', 0, '0', NULL),
(16, 5, 11, '2024-11-07 13:41:22', 0, '0', NULL),
(17, 5, 13, '2024-11-07 23:42:36', 0, '0', NULL),
(18, 5, 14, '2024-11-08 15:59:48', 0, '0', NULL);

-- --------------------------------------------------------

--
-- Table structure for table `28_2024_users`
--

DROP TABLE IF EXISTS `28_2024_users`;
CREATE TABLE IF NOT EXISTS `28_2024_users` (
  `id` int NOT NULL AUTO_INCREMENT,
  `username` varchar(255) NOT NULL,
  `password` varchar(255) NOT NULL,
  `type` varchar(255) NOT NULL,
  `etudiant_id` int DEFAULT NULL,
  `enseignant_id` int DEFAULT NULL,
  `structure_id` int DEFAULT NULL,
  `is_blocked` tinyint(1) NOT NULL DEFAULT '0',
  `photo` varchar(255) DEFAULT NULL,
  `google_info` int DEFAULT NULL,
  `role_referent` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL,
  `compte_incube` tinyint(1) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=23 DEFAULT CHARSET=latin1;

--
-- Dumping data for table `28_2024_users`
--

INSERT INTO `28_2024_users` (`id`, `username`, `password`, `type`, `etudiant_id`, `enseignant_id`, `structure_id`, `is_blocked`, `photo`, `google_info`, `role_referent`, `compte_incube`) VALUES
(1, '11955435', 'Azerty123@@@', 'Etudiant', 1, 0, NULL, 0, 'ala.jpg', 5, '', 0),
(2, 'adminEesu', 'Eesu2024*', 'Administrateur', NULL, NULL, NULL, 0, 'admin.png', NULL, '', 1),
(3, 'nidhal', 'Azerty1@', 'Enseignant', NULL, 1, NULL, 0, NULL, NULL, 'academique', 0),
(4, 'alaaFormateur', 'alaaFormateur1@', 'Enseignant', NULL, 2, NULL, 0, 'teacher.png', 4, 'academique', 1),
(5, '22446688', 'Azerty123@@', 'Etudiant', 2, NULL, NULL, 0, NULL, NULL, '', 0),
(6, '80158240', '86326512', 'Etudiant', 4, NULL, NULL, 0, NULL, 6, '', 0),
(7, '99186482', '12395350', 'Etudiant', 5, NULL, NULL, 0, NULL, NULL, '', 0),
(8, '76095142', '28688426', 'Etudiant', 6, NULL, NULL, 0, NULL, NULL, '', 0),
(9, '18609872', '18109832', 'Etudiant', 7, NULL, NULL, 0, NULL, NULL, '', 0),
(11, '11111111', 'Azerty1@', 'Etudiant', 8, NULL, NULL, 0, 'avatar-3.jpg', NULL, '', 0),
(13, 'refrent123', 'Azerty123@@', 'Enseignant', NULL, 5, NULL, 0, 'admin.png', 8, 'academique', 1),
(14, '11955440', 'Azerty123@@', 'Etudiant', 9, NULL, NULL, 0, NULL, 7, '', 0),
(15, '51115305', '70792740', 'Etudiant', 10, NULL, NULL, 0, NULL, NULL, '', 0),
(16, '75109136', '52938434', 'Etudiant', 11, NULL, NULL, 0, NULL, NULL, '', 0),
(17, '11955470', 'Azerty123@@', 'Etudiant', 12, NULL, NULL, 0, NULL, NULL, '', 0),
(18, '11955480', 'Azerty123@@', 'Etudiant', 13, NULL, NULL, 0, NULL, 9, '', 1),
(19, '93744825', '19260871', 'Etudiant', 14, NULL, NULL, 0, NULL, NULL, '', 1),
(20, 'Balkis', 'Balkis55722**', 'Etudiant', 15, NULL, NULL, 0, '151.png', NULL, '', 0);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
