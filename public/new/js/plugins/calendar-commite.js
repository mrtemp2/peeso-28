function createCalander(callback){
    ...
    if(datepicker.length){
        ...
        callback(selectedDay.toISOString().split('T')[0])
        const etablissmentElement = document.querySelector('#etablissmentFilter')
        etablissmentElement.addEventListener('change',e=>{
            const etablissement =$('#etablissmentFilter').val()  
            if(etablissement!='0'){

                callback(selectedDay.toISOString().split('T')[0],etablissement)
            }
            else{
                callback(selectedDay.toISOString().split('T')[0],"")
